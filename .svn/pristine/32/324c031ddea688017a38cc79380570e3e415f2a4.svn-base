...
function cm_checkScrolled(obj){
	...
	if(obj.scrolledY!=obj.lastScrolledY){
		if(!obj.useframes){
			for(i=0;i<obj.l[0].num;i++){var sobj=obj.l[0].o[i].oBorder; sobj.moveY(sobj.y+(obj.scrolledY-obj.lastScrolledY))}
			if(obj.usebar) obj.oBar.moveY(obj.oBar.y+(obj.scrolledY-obj.lastScrolledY))
		}
		obj.lastScrolledY=obj.scrolledY; page.y=obj.scrolledY; page.y2=page.y2orig+obj.scrolledY
...
/* ****************************************************************	
	//   new addition  //  For X
***************************************************************** */
	if(bw.ns4 || bw.ns6) obj.scrolledX=obj.win.pageYOffset
	else obj.scrolledX=obj.win.document.body.scrollLeft
	if(obj.scrolledX!=obj.lastScrolledX){
		if(!obj.useframes){
			for(i=0;i<obj.l[0].num;i++){var sobj=obj.l[0].o[i].oBorder; sobj.moveX(sobj.x+(obj.scrolledX-obj.lastScrolledX))}
			if(obj.usebar) obj.oBar.moveX(obj.oBar.x+(obj.scrolledX-obj.lastScrolledX))
		}
		obj.lastScrolledX=obj.scrolledX; page.y=obj.scrolledX; page.x2=page.x2orig+obj.scrolledX
		if(!obj.useframes || bw.ie){ clearTimeout(obj.tim); obj.isover=0; obj.hideSubs(1,0)}
	}if((bw.ns4 || bw.ns6) && !obj.useframes) setTimeout("cm_checkScrolled("+obj.name+")",200)
...
function cm_makeObj(obj,name,level,win,nest,o){
	...
	this.moveIt=cm_moveIt; this.clipTo=cm_clipTo; if(name) this.parent=name; this.moveY=cm_moveY;   this.moveX=cm_moveX;this.l=level; this.clipOut=cm_clipOut; 
...
this.ref.close()}else this.evnt.innerHTML=text}}; function cm_moveY(y){this.y=y; this.css.top=y}; function cm_moveX(x){this.x=x; this.css.left=x}
...
function cm_makeMenu(name,parent,text,link,target,width,height,img1,img2,bgcoloroff,bgcoloron,textcolor,hovercolor,onclick,onmouseover,onmouseout){
	...
	else if(bw.ns4){text='<font size="'+(prop1.NS4fontSize||prop2.NS4fontSize)+'" face="'+(prop1.NS4font||prop2.NS4font)+'" color="'+(textcolor||this.l[l].tc)+'"><b>'+text+'</b></font>'}
...
function cm_getCoords(name,l,num,topalign,align,ln,border,cn,lev1b){
	...
	if((align==2||topalign==2)&&lev1b!=3){
		...
		if(this.useframes&&l==1){ y=page.y2 - this.m[name].totheight - this.l[l-1].offsetY*2;
			x=page.x2 - this.m[name].totwidth - this.l[l-1].offsetX*2
		}
		else { y-=this.m[name].totheight - this.l[l].offsetY*2; if(l!=1||lev1b==3) y+=this.m[name].height;
			x-=this.m[name].totwidth - this.l[l].offsetX*2; if(l!=1||lev1b==3) x+=this.m[name].width
		}
	}this.m[name].scrollY=this.lastScrolledY; this.m[name].subx=x+this.l[l-1].offsetX; 
	this.m[name].scrollX=this.lastScrolledX; this.m[name].suby=y+this.l[l-1].offsetY; 
	this.m[name].suby=y+this.l[l-1].offsetY; if(this.useframes&&l==1&&align!=3) this.m[name].suby+=this.lastScrolledY;
	this.m[name].subx=x+this.l[l-1].offsetX; if(this.useframes&&l==1&&align!=3) this.m[name].subx+=this.lastScrolledX;
...
function cm_showSubs(name,l,num,cl){
	...
	if((this.m[name].subx==-1 || this.m[name].suby==-1) || this.m[name].scrollY!=this.lastScrolledY || this.m[name].scrollX!=this.lastScrolledX || this.isresized){
...
function cm_construct(){
	...
	for(i=0;i<num;i++){
		if(i!=0) frstr+='<div id="div'+this.name+i+'b" style="'+this.ns6styleb[i]+ '<img alt="" src="arrows.gif" />' + '" class="cl'+this.name+i+'b">\n'
		for(j=0;j<this.l[i].maxnum;j++){
...
function cm_makeStyle(){
	...
	if(bw.ns4) str+="A.clNS4{text-decoration:none;font-weight:bold; padding:"+this.NS4padding+"}\n"; document.write(str+"\n</style>\n")
}
/************
...
function makeCoolMenu(name){
	coolFName=name; this.name=name; this.lastScrolledY=0;this.lastScrolledX=0;
