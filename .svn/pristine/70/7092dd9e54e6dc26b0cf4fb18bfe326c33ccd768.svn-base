 ...
 *         Window - Preferences - Java - Code Style - Code Templates
 */
public class ComputeIndexForm extends ActionForm {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	static Logger Logging = Logger.getLogger(ComputeIndexForm.class);
	String fromcomposition, basedate1, index, b_showChild, query,
			indexcalculated, indexid, user_id = null, role_id = null;

	String computeall, computesame, computecurrency, computetotalreturns,
			close, settlement, ComputeIndex;

	NewIndexForm newIndexForm = null, newIndexForm1 = null;
	...
	public void setNewIndexForm(NewIndexForm newIndexForm) {
		if (newIndexForm != null
				&& newIndexForm.getDatabaseConnection() != null) {
			connection = newIndexForm.getDatabaseConnection();
		}
	...
	public Collection getIndexcollection() {
		...
		try {
			...
			resourcepth = resourcepth.substring(6,
					(resourcepth.lastIndexOf("/WEB-INF/") + 8));
		...
		try {
			...
			if (use_user != null && use_user.equals("yes")) {
				if (getB_showChild() != null
					...
					query = prop1.getProperty("index_list");
				} else {
					query = prop1.getProperty("index_list_without_child");
				}
		...
		if (fromcomposition == null
			...
			try {
				try {
					...
					Logging.debug("connection is " + connection);
					connection = null;
					connection = ConnectInit.getConnect()
	...
	public String getIndex() {
		...
		System.out.println("Index in getIndex method *** " + index);
		return index;
	}
	...
	public NewIndexForm nullNewIndexForm() {
		newIndexForm = null;
		return null;
	}
	 ...
	 */
	// modified by samiksha parameter
	public void setComputeIndex(String request) {

		Logging.debug(indexid + "1 : ");
		...
		try {
			...
			settlement = "n";
			System.out.println("setComputeIndex *******" + e.getMessage());
			// TODO: handle exception
		}
		Logging.debug("2");
		System.out.println("ComputeIndexForm   11111  newIndexForm **********"
				+ newIndexForm);
		// null check condition and newIndexObject creation is added by samiksha
		if (newIndexForm == null) {
			try {
				newIndexForm = new NewIndexForm();
			} catch (Exception e) {
				// TODO Auto-generated catch block
				System.out
						.println("Exception occured while creating object ---"
								+ e.getMessage());
				e.printStackTrace();
			}
		}
		System.out.println("newIndexForm.getI_indexID() **** "
				+ newIndexForm.getI_indexID());
		if (newIndexForm.getI_indexID() != null) {
			index = new String(newIndexForm.getI_indexID());
		}
		// modified by samiksha
		ComputeIndex = cic.computeIndex(index, settlement, "n", "yes", null,
				null, basedate1, connection);
		// ComputeIndex = cic.computeIndex(newIndexForm.getI_indexID(),
		// settlement, "n", "yes", null, null, basedate1, connection);
		// hardcode
		// ComputeIndex = cic.computeIndex("3568",
		// settlement, "n", "yes", null, null, basedate1, connection);

		Logging.debug("3 newindex form : " + newIndexForm);
		System.out.println("***** After ComputeIndex method *****"
				+ ComputeIndex);
		try {
			try {
				Logging.debug(ComputeIndex);// by p
				Double tempIndexValue;
				// null check condition is added by samiksha
				if (ComputeIndex != null) {
					if ((ComputeIndex != "----"
							|| !ComputeIndex.equalsIgnoreCase("----"))) {
						tempIndexValue = Double.valueOf(ComputeIndex);
						Logging.debug(tempIndexValue);
				...
				System.out
						.println("ComputeIndexForm   indexid ---------------------"
								+ indexid);
				if (indexid != null) {
					index = new String(indexid);
				}
				Logging.debug("Commiting index creation and composition as index value found is proper : "
						+ ComputeIndex);
				
			} catch (Exception e) {
				indexcalculated = "---Pranay223";
				// TODO: handle exception
				// e.printStackTrace();
	...
	public synchronized void setClosingvalue(String closingvalue) {
		...
		if (index != null && !index.equals("0")) {
			query = ConnectInit.queries
					.getProperty("indexcomputation.todayclosing");
			try {
				if (con == null) {
	...
	public ActionErrors validate(ActionMapping mapping,
		...
		Logging.debug("base date in validate compute index is " + basedate1);
		...
		if ((request.getParameter("combo") != null && request
				.getParameter("combo").trim().equals("yes"))
				|| (request.getParameter("operation1") != null && request
						.getParameter("operation1").equals("Change"))) {
			indexcalculated = "";
			computeall = "";
			computecurrency = "";
			computesame = "";
		...
		if (diff2 > 0) {
			actionErrors.add(null, new ActionError(
					"compute.error.not_greater_then_current"));
			return actionErrors;
		}
		int diff3 = CompareDate(hist_Date, basedate1);
		if (diff3 < 0) {
			actionErrors.add(null, new ActionError(
					"compute.error.not_less_then_base"));
			return actionErrors;
		}
		if (diff1 <= 0 && diff2 != 0) {
			basedate1 = hist_Date;
		...
		if (request.getParameter("operation") != null
			...
			synchronized (this) {
				...
				if (!this.Stoprepitation.trim().equals("end")) {
					...
					Logging.debug("this.Stoprepitation" + this.Stoprepitation);
					this.Stoprepitation = this.Stoprepitation + " ";
					...
					if (request.getAttribute("tmcverror") != null
						...
						Logging.debug("setting zero mcv action error in bean");
						indexcalculated = "----";
						actionErrors.add(null, new ActionError(
					...
					try {
						try {
							...
							Logging.error("Roll backing index creation and composition as index value found is "
									+ ComputeIndex);
							connection.rollback();
							connection.setAutoCommit(true);
						}
	...
	public String getLastclosingvalue() {
		...
		Connection con = null;
		if (index != null && !index.equals("0")) {
			int cmpdate = CompareDate(hist_Date, (QueryClass.formatDate()));
			Logging.debug("hist_date is " + hist_Date
					+ " QueryClass.formatDate() is " + QueryClass.formatDate()
					+ " cmpdate is " + cmpdate);
			if (cmpdate == 0) {
				query = ConnectInit.queries
	...
	public void validateTotalReturns(ActionErrors errors) {
		...
		case 1:
			...
			break;
		case 2:
			errors.add(null, new ActionError("error.totalreturn.todaysdivisor"));
		...
		case 5:
			...
			errors.add(null, new ActionError("error.totalreturns.cashdividend"));
	...
	public StringBuffer getMessages(String message) {
		...
		String display = null;
		buffer1.append("<td width='75%'><b><font face='Arial' color='Red' size='2'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
				+ message + "</font></b></td> ");
		buffer1.append("<br> ");
		return buffer1;
	}
