 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class NCorp_Action extends Action{
	static Logger Logging = Logger.getLogger(NCorp_Action.class);
	public ActionForward execute(ActionMapping mapping, ActionForm form,
			...
			if(button!=null)
			{
				if(button.equals("Exc"))    //on selection of exchange
				{
					...
					return new ActionForward("/pages/NCorporateAction.jsp?flag=New");
				}
				...
				if(button.equals("Radio"))  //on selection of radio button
				{
					...
					if(butt==null)
					{
						if(radval.equals("s"))	//if user wants to stop futher processing 
						{
							...
							return new ActionForward("/pages/NCorporateAction.jsp?flag=New&ref_flag=1&corp_name="+corp.getCorpid());
						}
						if(radval.equals("c")) //if user wants to continue further processing on any alert 
						{
							...
							return new ActionForward("/pages/NCorporateAction.jsp?flag=New&check_flg=0&corp_name="+corp.getCorpid());
						}
					}
					else
					{
						if(butt.equals("Undo"))  //in case of undo any corporate action
						{
							...
							if(radval.equals("c"))  //if user wants to continue further processing on any alert
							{
								...
								return new ActionForward("/pages/NCorporateAction.jsp?flag=Exist&check_flg="+flg_val);
							}
						}
					}
				}
				if(button.equals("NApply"))   //on click of apply button,if it is a new action
				{
					...
					if(stock!=null)			//get stock details
					{
					    try{
					         ...
					         {					        	
					        	...
					        	rs=ListTypeClass1.getResult_apply(con,stk_qry,stock,apply);
					        	if(rs!=null && rs.next()){
					...
					if(chk_dt==0)				//if dates are same
					{
						try{							
							...
							String query2=ConnectInit.queries.getProperty("select_corporate_action_diary");
							ResultSet rs2=ListTypeClass1.getresp_cad(con,query2,corp.getCorpid(),stock,corp.getApply_date());
							...
							{	
								...
								return new ActionForward("/pages/NCorporateAction.jsp?flag=New&check_flg="+flg_val);
							}
							else   //if present
							{														
								return new ActionForward("/pages/CorporateDetails.jsp");
							}						
					...
					else      //if the entered date is less than current date then there is historic calculation
					{
						...
						if(corp_nm.equals("cashdividend")) //if cashdividend then check in system configuration table
						{
							try{
								...
								ResultSet rs1=ListTypeClass1.resultCorporate(con,query);
								rs1.next();
...
//									get corporate action id.
									...
									if(chk_exist==false)
									{
									    ...
									    qry=ConnectInit.queries.getProperty("get_resp_date_closevalue");					    
										rs=ListTypeClass1.getResult_apply(con,qry,corp.getStid(),corp.getApply_date());
										rs.next();
										corp.setNewLTP(rs.getString("adjusted_price"));
										if(rs!=null)
											rs.close();
										corp.setNcorp_button(null);
										return new ActionForward("/pages/NCorporateAction.jsp?flag=New&check_flg=0");
									}
									if(chk_exist==true)
									{
										return new ActionForward("/pages/CorporateDetails.jsp");
				...
				if(button.equals("Apply"))   //on click of apply button, if the data is come from diary
				{
					corp.setCommit_butt("1");
					String butt=corp.getButton();
					if(butt!=null)
						...
						//collect the data in bean
							...
							if(butt==null)     //if there is no undo  
							{
								...
								else //if not then historic calculation 
								{
									...
									return new ActionForward("/pages/HistoricAction.jsp?flag="+check);
								}
							...
							{								
								...
								corp.setNcorp_button(null);
								String query=null;
								query=ConnectInit.queries.getProperty("check_corp_existance");
								rs=ListTypeClass1.getresp_cad(con,query,Integer.toString(cid),stock,apply);
								
								if(rs.next())//if present
								{
									//then alert
									return new ActionForward("/pages/NCorporateAction.jsp?flag=Exist&check_flg=4");	
								}
								else    //if not present
								{									
				...
				{					
					...
					String dt=UpdateCorp.accept_date();   //get the current date
					String apply=corp.getApply_date();
					chk_dt=ComputeIndexForm.CompareDate(apply,dt);	//check for the current date and user's entered date
						}
						else
							chk_dt=0;
					...
					{						
						...
						{							
							if(butt==null)   //if there is no undo
							{
						        ...
						        ResultSet rs1 = ListTypeClass1.getResult12(con,query,affect);			            
								corp.setResult1(rs1);
							}
							else              //if there is undo
							{
								...
								ResultSet rs1=ListTypeClass1.getResult_apply(con,query1,affect,date);
								corp.setResult1(rs1);
							}
	...
	//							check from system configuration table whether to adjust price or not for cash dividend
					...
					corp.setNcorp_button(null);
					tmcv_div(corp);
			        return new ActionForward("/pages/NCorporateAction.jsp?check_flg=0");			                
				}
						...
						if(check!=null)
						{
							...
							if(check.equals("New"))  //if its new i.e's from action page directly
							{
								...
								if(chk==null)  //if it's new
								{
									...
									ResultSet rs = ListTypeClass1.resultCorporate(con,query);    
									ListTypeClass1.check_corp_name(rs,corp_act,corp);
									query=ConnectInit.queries.getProperty("insert_into_cad_values_with_time");
	...
	//					check from system configuration table whether to adjust price or not for cash dividend					
						...
						//update tis in stock master
							query=ConnectInit.queries.getProperty("update_tis_stockmaster");
							...
							{							
								...
								corp.setNewmcv(mcv);
							    query =ConnectInit.queries.getProperty("update_tis_after_ca");
							 	...
							 	if(!(corp_name.equals("stockdividend/bonus")))
							 	{
									...
									qry=ConnectInit.queries.getProperty("get_latest_index_value_index_wise");
							...
							{								
								...
								String undo_query=ConnectInit.queries.getProperty("get_undo_close_value");
								
								//update stock price daily for the particular date
								double mcv=cf.calMarketCap(Double.parseDouble(corp.getNewLTP()),Long.parseLong(corp.getMark_lot()),1,Long.parseLong(corp.getNewTIS()),Double.parseDouble(corp.getIwfstk()));
								corp.setNewmcv(mcv);
								query =ConnectInit.queries.getProperty("update_undo_stock_price");
	...
	//						 	update index value daily for the particular date
							...
							ResultSet rs1=ListTypeClass1.getAffected(con,query,corp.getStid());					
							qry=ConnectInit.queries.getProperty("update_index");
						...
						{			
							...
							qry=ConnectInit.queries.getProperty("insert_stock_master");
							...
							//delete from stock master history
							 Hashtable hash1=corp.getHash1();
								...
								{				
									...
									query=ConnectInit.queries.getProperty("delete_stkmas_history");
			...
			if(detail_button!=null)
			{
				...
				{				
					return new ActionForward("/pages/NCorporateAction.jsp?flag=New&corpid="+corp.getCorpid());
				}
				...
				{	
					...
					if(chk_val==false) //if yes
					{
						...
						if(chk_dt==0)//if dates are equal
						{
							...
							return new ActionForward("/pages/NCorporateAction.jsp?flag=New&corp_name="+corp.getCorpid()+"&check_flg="+flg_val);
						}
						else//if not
						{
							String corp_nm=corp.getCorpid();
							...
							if(corp_nm.equals("cashdividend")) //if cashdividend then check in system configuration table
							{
								try{
									...
									if(type.equals("n"))  //no changes are made in stock parameter
									{
									    ...
									    String  qry=ConnectInit.queries.getProperty("get_resp_date_closevalue");					    
										ResultSet rs=ListTypeClass1.getResult_apply(con,qry,corp.getStid(),corp.getApply_date());
										rs.next();
										corp.setNewLTP(rs.getString("adjusted_price"));
										if(rs!=null)
											rs.close();
							...
							corp.setRatio1(ori_r1);
							corp.setRatio2(ori_r2);
							return new ActionForward("/pages/NCorporateAction.jsp?check_flg=0&corp_name="+corp.getCorpid());
						}						
					}
					else   // if no
					{
						//then alert
						corp.setNature(null);
						return new ActionForward("/pages/CorporateDetails.jsp?flag=Exist");
					}
				}
				...
				{					
					...
					if(rad_val!=null)
					{
						...
						{							
							...
							ResultSet rs=ListTypeClass1.getResult1(con,query,corp.getChk_but());
							rs.next();
					...
					return new ActionForward("/pages/NCorporateAction.jsp?flag=New&cval=c&corp_name="+corp.getCorpid()+"&check_flg="+flg_val);
				}
			}
			...
			if(hist_butt!=null)
			{
				...
				{				
					String check=request.getParameter("flag");
					if(check.equals("New"))
						return new ActionForward("/pages/NCorporateAction.jsp?flag=New&corp_name="+corp.getCorpid());
				...
				if((hist_butt.equals("Continue")))//further processing
				{
					...
					{										
						...
						//check whether this combination is present in dairy or not.
						String query2=ConnectInit.queries.getProperty("select_corporate_action_diary");
						ResultSet rs2=ListTypeClass1.getresp_cad(con,query2,corp.getCorpid(),corp.getS_stock(),corp.getApply_date());
						boolean chk_exist=rs2.next();						
						corp.setCorpid(corp_act);
	...
	//This method is used to check whether the entered value matches with the selected value
		public static boolean check_val(Corporate corp)
		{
...
//This method is used to calculate new value 	
		public static int CalApply(Connection con,Connect connect,Corporate corp)
		{			
	//		CFormula cf = new CFormula();
			CFormula cf = ConnectInit.getCFormula();
...
//				check from system configuration table whether to adjust price or not for cash dividend				
				...
				if(corp_nm.equals("cashdividend"))
				{
					 ...
					 if(type.equals("n"))
					 {
					 	corp.setNewTIS(corp.getTis());					    
					    corp.setAdjust("0");
					    query=ConnectInit.queries.getProperty("select_stock_price_detail");					    
						ResultSet rs=ListTypeClass1.getResult12(con,query,corp.getStid());
						if(rs!=null && rs.next()){
							corp.setNewLTP(rs.getString("adjusted_price"));
							rs.close();
						}
...
//	check whether ths change in tis is less than 5% or not 			
...
//This method is used to recalculate old value(for undo)		
		public static int reCalApply(Corporate corp)
		{
			int flg_val=0;
		    try{
...
// This method is use to get the currency exchange value		
		public static void get_currency(Connection con,Connect connect,Corporate corp,String index,String stock)
		{
...
//				get index currencies
				...
				ResultSet rs=ListTypeClass1.getAffected(con,qry,index);
...
//				get stock currencies
				...
				rs=ListTypeClass1.getAffected(con,qry1,stock);
				String stk_curr="ss";
				...
				else  //if not equal
				{
					...
					rs=ListTypeClass1.getResult_corp(con,qry,ind_curr,stk_curr);
					if(rs.next())
					{
					...
					{
						rs=ListTypeClass1.getResult_corp(con,qry,stk_curr,ind_curr);
						if(rs.next())
						{
							ft_curr=rs.getString("from_to_currency_id");
							flg=2;
					...
					if(flg!=0)
					{
						if(chk_dt==0)
						{
							if(butt==null)
							{
								...
								rs=ListTypeClass1.getResult12(con,query,ft_curr);
							}
							else
							{
								String query=ConnectInit.queries.getProperty("resp_undo_curr_val");
								rs=ListTypeClass1.getResult_apply(con,query,ft_curr,corp.getApplied_date());
							}
						}
						else
						{
							String query=ConnectInit.queries.getProperty("resp_undo_curr_val");
							rs=ListTypeClass1.getResult_apply(con,query,ft_curr,corp.getApply_date());
						}
						String val="";
		...
		public static void tmcv_div(Corporate corp)
		{
			try{
