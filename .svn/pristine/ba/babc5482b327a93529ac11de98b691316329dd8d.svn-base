...
<body>	
		<%
			...
			for(int i=0;i<len;i++){
			if(i==0){ind2=ComposeIndex.getIndexName(indid[0]);ind2=ind2+"(%)"; }
			if(i==1){ind3=ComposeIndex.getIndexName(indid[1]);ind3=ind3+"(%)"; }
			}
			%>
      	<table ID="sortTabletable" border="1" align="center" cellpadding="3" cellspacing="0">
				<tr> <h3> Industrywise Comaparision</h3></tr>
 			...
 			<logic:iterate id="details"  name="PortfolioReportBean" property="sectorData" >
 			<%
