...
public class FrameTestBhav extends JFrame{	
	
	public FrameTestBhav() {
		super();
		initialize();
	}
	...
	public static void main(String[] args) {
		//MyDom d1=new MyDom();
    	final JLabel BHAVDetails = new JLabel(" Bhav File details");
    	...
    	//new ClientBhav( d1.s1 ,d1.s2 ,d1.s3,source,destination).start();
    	new ClientBhav("203.199.75.115","C05023","HIS468",source,destination).start();
    	//new MktThrDB(name);
		new Timer(1000, new ActionListener(){		
					
            public void actionPerformed(ActionEvent evt) {
               String name = BHAVThread.str;
               FName.setText(name);
               
               
               long length=BHAVThread.len;
               FileSize.setText(Long.toString(length)+"Bytes");
               
               int num=BHAVThread.nofiles;
               NumFile.setText(Integer.toString(num));  
               
               String filetime=BHAVThread.rightnow;
		...
		BHAVDetails.setBounds(new java.awt.Rectangle(15,15,100,20));
		
		contentPane = new JPanel();
		contentPane.setLayout(null);
		contentPane.add(BHAVDetails, null);
        ...
        final FrameTestBhav f = new FrameTestBhav();
