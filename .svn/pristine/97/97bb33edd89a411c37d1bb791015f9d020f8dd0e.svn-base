...
public class FrameTestMess extends JFrame{	
	
	public FrameTestMess() {
		super();
		initialize();
	}
	...
	public static void main(String[] args) {
		//MyDom d1=new MyDom();
    	final JLabel MessDetails = new JLabel(" Message File details");
    	...
    	MyDom d1=new MyDom();
    	String source="/CM05/Messages/August252006/";
    	String destination="D://DownLoadMess//";
    	//String destination1="D://DownLoad1//";
    	//new ClientMess( d1.s1 ,d1.s2 ,d1.s3,source,destination).start();
    	new ClientMess("203.199.75.115","C05023","HIS468",source,destination).start();
    	//new MktThrDB(name);
		new Timer(1000, new ActionListener(){		
					
            public void actionPerformed(ActionEvent evt) {
               String name = MessThread.str;
               FName.setText(name);
               
               
               long length=MessThread.len;
               FileSize.setText(Long.toString(length)+"Bytes");
               
               int num=MessThread.nofiles;
               NumFile.setText(Integer.toString(num));  
               
               String filetime=MessThread.rightnow;
		...
		MessDetails.setBounds(new java.awt.Rectangle(15,15,100,20));
		
		contentPane = new JPanel();
		contentPane.setLayout(null);
		contentPane.add(MessDetails, null);
