...
public class AdjustDecimal {
	...
	/**
	 * method returns string having double value cinsisting of two digits after
	 * decimal point.
	 */
	public String indexcompose(String tp12) {
		...
		if ((tp12.length()) > (tp12.indexOf(46) + 3)) {
			String tsr2 = tp12.substring(tp12.indexOf(46),
					(tp12.indexOf(46) + 3));
	 ...
	 * consisting of two digits after decimal point. input parameter is double
	 * no.
	 */
	public String shareholdingpatt(double noheld2) {
		...
		if (no2.indexOf('E') == -1) {
			...
			String ts2 = no2.substring(0, no2.indexOf(46))
					+ no2.substring((no2.indexOf(46)) + 1, (no2.indexOf('E')));
			...
			if (ts2.length() >= (no2.indexOf(46) + n2)) {
				if (ts2.length() == (no2.indexOf(46) + n2)) {
					...
					no2 = ts2.substring(0, (no2.indexOf(46) + n2)) + "."
							+ ts2.substring((no2.indexOf(46) + n2));
				}
			} else {
	...
	/**
	 * method to convert double value in exponential format to double value
	 * consisting of two digits after decimal point. input parameter is String.
	 */
		...
		try {
			...
			if (no2 != null) {
				if (no2.indexOf('E') == -1) {
					...
					String ts2 = no2.substring(0, no2.indexOf(46))
							+ no2.substring((no2.indexOf(46)) + 1,
									(no2.indexOf('E')));
					flag = (no2.charAt(no2.indexOf('E') + 1) == '-');
					Logging.debug("flag in shareholdingpatt is " + flag);
					if (flag) {
						n2 = Integer
								.parseInt(no2.substring(no2.indexOf('E') + 2));
					} else {
						n2 = Integer
								.parseInt(no2.substring(no2.indexOf('E') + 1));
					}
					if (flag == true) {
						div = 10 * n2;
	...
	/**
	 * method to convert double value to double value consisting of two digits
	 * after decimal point. input parameter is double no.
	 */
	...
	/**
	 * method to convert double value to double value consisting of two digits
	 * after decimal point. input parameter is string.
	 */
	...
	public static String ArrangeAsNumeric(String str) {
		...
		if (str.indexOf(46) != -1) {
			...
			arrstr = str1.substring(0, (str1.length() - ((count) * 3))) + ","
					+ arrstr;
			arrstr = arrstr + rstr;
		} else {
			...
			arrstr = str.substring(0, (str.length() - (count * 3))) + ","
					+ arrstr;
		}
		if ((arrstr.charAt(0)) == ',')
