...
public class FillTables {
	...
	public void addStocksInSourceTable(String[] keys, Hashtable sourceTable,
		...
		Logging.debug("destinationTable.size() : " + destinationTable.size());
		// setTotalMCV();
	}

	public StringBuffer addRowsInTable(Hashtable data, String letter) {
		Logging.debug("letter in addRowsInTable is " + letter);
		System.out.println("Add rows in table *** ");
		...
		try {
			...
			// e.printStackTrace();
			Logging.debug("Error in inserting rows in Table " + e.getMessage());
	...
	public StringBuffer addRowsInSecondTable(Hashtable data, String indexType,
			HttpServletRequest request) {
		/*
		 * instead of creating object of index composition to get parent index
		 * currency id try to get the currency id from index composition form
		 * since this method is being is being called from the index composition
		 * page .
		 */
		// IndexCompositionForm icf=new IndexCompositionForm();//created object
		// of indexcomposition form class
		// String indexCurrencyId=icf.getParentCurrencyId();//get the index
		// currency id from object
		HttpSession httpSession = request.getSession();
		NewIndexForm indexForm = (NewIndexForm) httpSession
		...
		for (Enumeration e = data.keys(); e.hasMoreElements();) {
			...
			total += rs1.getMktCapital1(2, indexForm.getS_baseCurrency(),
					rs1.getCurrencyId());
		...
		for (int i = 0; i < entries.length; i++) {
			...
			/*
			 * String mkt_cap =
			 * ad.shareholdingpatt(rs.getMktCapital1(marcapiwf,indexForm
			 * .getS_baseCurrency(),rs.getCurrencyId())); String adjmkt_cap =
			 * ad.shareholdingpatt(rs.getAdjustedMarket(marcapiwf));
			 * adjmkt_cap=mkt_cap;
			 */// commented by neha 12-01-2009 for editable iwf

			String mkt_cap = ad.shareholdingpatt(rs.getMktCapitalNew(marcapiwf,
					indexForm.getS_baseCurrency(), rs.getCurrencyId()));
			String adjmkt_cap = ad.shareholdingpatt(rs
					.getAdjustedMarket(marcapiwf));

			Logging.debug("before setting iwf with index type : " + indexType);
			...
			if (!readOnly) {
				...
				buffer.append("<input type= text name ='iwf" + rs.getStockID()
						+ "'");
				buffer.append(" size='11' align='right' onblur='if((isit(indexComposition.iwf"
						+ rs.getStockID()
						+ ".value))==false){indexComposition.iwf"
						+ rs.getStockID()
						+ ".value=\"1.0\" }else{var padjmcap=(indexComposition.adjmcap"
						+ rs.getStockID()
						+ ".value); var cal"
						+ rs.getStockID()
						+ "=indexComposition.iwf"
						+ rs.getStockID()
						+ ".value*"
						+ mkt_cap
						+ ";"
						+ " indexComposition.adjmcap"
						+ rs.getStockID()
						+ ".value=cal"
						+ rs.getStockID()
						+ ";alert(indexComposition.totalMCV.value);alert((indexComposition.totalMCV.value - padjmcap));var tmcv"
						+ "=((indexComposition.totalMCV.value - padjmcap) + ((indexComposition.iwf"
						+ rs.getStockID()
						+ ".value)*"
						+ mkt_cap
						+ "));"
						+ " indexComposition.totalMCV"
						+ ".value=tmcv"
						+ ";} '  value='" + iwf + "' >");

				/*
				 * buffer.append("<input type= text name ='iwf" +
				 * rs.getStockID() + "'"); buffer .append(
				 * " size='11' align='right' onblur='if((isit(indexComposition.iwf"
				 * + rs.getStockID() + ".value))==false){indexComposition.iwf" +
				 * rs.getStockID() +
				 * ".value=\"1.0\" }else{var padjmcap=(indexComposition.adjmcap"
				 * +rs.getStockID()+".value); var cal"+rs.getStockID()+
				 * "=indexComposition.iwf"+
				 * rs.getStockID()+".value*"+mkt_cap+";"+
				 * " indexComposition.adjmcap"
				 * +rs.getStockID()+".value=cal"+rs.getStockID()+
				 * ";alert(indexComposition.totalMCV.value);alert((indexComposition.totalMCV.value - padjmcap));var tmcv"
				 * +
				 * "=((indexComposition.totalMCV.value - padjmcap) + ((indexComposition.iwf"
				 * + rs.getStockID()+".value)*"+mkt_cap+"));"+
				 * " indexComposition.totalMCV"+".value=tmcv"+
				 * ";"+"indexComposition.weight"
				 * +rs.getStockID()+".value=indexComposition.adjmcap"
				 * +rs.getStockID
				 * ()+".value/indexComposition.totalMCV.value*100"+
				 * ";      } '  value='" + iwf + "' >");
				 */
			 ...
			 * buffer.append("<td width='7%' align='right'>");
			 * buffer.append("<font face='Arial' size='1' color='blue'>");
			 * Logging.getDebug("inclusion exclusion date 4indexType : " +
			 * indexType); buffer.append(
			 * "<input type='text' readonly='true'  size='14' name='incls_date"
			 * + rs.getStockID()+" value='' >");
			 * buffer.append("<input onclick='c2.popup('incls_date" +
			 * rs.getStockID()+"');'  type='button' value='...'/>'");
			 * buffer.append("</font>"); buffer.append("</td>");
			 */
			...
			/*
			 * double
			 * mcap_calc=((double)Double.parseDouble(adjmkt_cap))*((double
			 * )Double.parseDouble(iwf)); adjmkt_cap=new
			 * Double(mcap_calc).toString();
			...
			buffer.append(" size='30' align='right'  value='" + adjmkt_cap
					+ "'>");// onfocus='
							// func("+rs.getStockID()+","+request.getParameter("iwf" + rs.getStockID())+","+adjmkt_cap+");'
							// >");
			buffer.append("</font>");
			buffer.append("</td>");
			...
			buffer.append("<font face='Arial' size='1' color='blue'>");
			/*
			 * buffer.append("<input name ='weight"+rs.getStockID()+"'");
			 * buffer.append(" size='30' align='right'  value='"+weight +"'>");
			 */

			buffer.append(weight); // commented by neha 16-01-2009
			buffer.append("</font>");
			buffer.append("</td>");
			buffer.append("</tr>");
		}
	...
	public StringBuffer addRowsInSecondTableInclsDate(Hashtable data,
			String indexType, HttpServletRequest request) {
		/*
		 * instead of creating object of index composition to get parent index
		 * currency id try to get the currency id from index composition form
		 * since this method is being is being called from the index composition
		...
		for (int i = 0; i < entries.length; i++) {
			...
			String iwf = null;
			// String mkt_cap =
			// ad.shareholdingpatt(rs.getMktCapital1(marcapiwf,indexForm.getS_baseCurrency(),rs.getCurrencyId()));
			// //commented by neha 12-01-2009 for editable iwf
			String mkt_cap = ad.shareholdingpatt(rs.getMktCapitalNew(marcapiwf,
					indexForm.getS_baseCurrency(), rs.getCurrencyId()));
			String adjmkt_cap = ad.shareholdingpatt(rs
					.getAdjustedMarket(marcapiwf));
			// adjmkt_cap=mkt_cap; //commented by neha 12-01-2009 for editable
			// iwf
			Logging.debug("before setting iwf with index type : " + indexType);
			if (request.getParameter("iwf" + rs.getStockID()) == null) {
				Logging.debug("1");
			...
			buffer.append("<input type='text' readonly='true'  size='14' name='incls_date"
					+ rs.getStockID() + " value='' >");
			buffer.append("<input onclick='c2.popup('incls_date"
					+ rs.getStockID() + "');'  type='button' value='...'/>'");
			buffer.append("</font>");
			...
			buffer.append("<font face='Arial' size='1' color='blue'>");
			Logging.debug("exclusion date 4indexType : " + indexType);
			buffer.append("<input type='text' readonly='true'  size='14' name='excls_date"
					+ rs.getStockID() + " value='' >");
			buffer.append("<input onclick='c2.popup('excls_date"
					+ rs.getStockID() + "');'  type='button' value='...'/>'");
			buffer.append("</font>");
			buffer.append("</td>");
	...
	public StringBuffer DesignBaseValuesTable(NewIndexForm form,
			HttpServletRequest request, ActionErrors actionErrors) {
		...
		try {
			...
			PreparedStatement preparedStatement = form.getDatabaseConnection()
			...
			while (rst.next()) {
				childindexname = iname + "." + rst.getString("level_name")
						+ "." + rst.getString("class_name");
				// Logging.getDebug("While putting value is "+rst.getString("industry_classification_code"));
				stringBuffer
						.append("<tr><td width=\"5%\" align=\"right\" bgcolor=\"#FFFFFF\">&nbsp;");
				stringBuffer
						.append(i
								+ "</td><td width=\"23%\" align=\"left\" bgcolor=\"#FFFFFF\"> &nbsp;&nbsp;"
								+ childindexname);
				stringBuffer.append("</td>");
				stringBuffer
						.append("<td width=\"5%\" align=\"center\" bgcolor=\"#FFFFFF\"><input type=\"text\" name=\""
								+ iname
								+ rst.getString("industry_classification_code")
								+ "\"  size=\"10\" align='left'");
				stringBuffer.append(" onblur='if((isit(baseValues." + iname
						+ rst.getString("industry_classification_code")
						+ ".value))==false){baseValues." + iname
						+ rst.getString("industry_classification_code")
						+ ".value=" + basevalue + "} '");
				...
				if (request.getParameter(iname
					...
					try {
						...
						if (value <= 0) {
							stringBuffer
									.append("<font color=\"#FF0000\">*</font>");
							if (actionErrors.isEmpty())
								actionErrors.add(null, new ActionError(
										"indexcompose.invalidBaseValue"));
						} else {
							stringBuffer.append("&nbsp;");
						}
					} catch (Exception e) {
						...
						if (actionErrors.isEmpty())
							actionErrors.add(null, new ActionError(
									"indexcompose.invalidBaseValue1"));
