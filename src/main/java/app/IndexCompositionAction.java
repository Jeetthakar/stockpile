 ...
 */
public class IndexCompositionAction extends Action {
	Logger Logging = Logger.getLogger(IndexCompositionAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		IndexCompositionForm compositionForm = (IndexCompositionForm) form;
			...
			try {
				...
				java.net.URL imgURL = ComputeIndexForm.class.getResource("ComputeIndexForm.class");
			  ...
			  Logging.debug("inside IndexComposition - user_id :"+user_id);
//			  session.setAttribute("user_id", user_id);
			  Logging.debug("user id set to session :"+user_id);
			   Logging.debug("1");
			if (compositionForm.getOperation() != null && compositionForm.getOperation().trim().equals("Submit")) {
				
				    NewIndexForm pageForm = (NewIndexForm) session.getAttribute("newIndexForm");
				    Logging.debug("2");
				if (pageForm.getS_applyClassification() == null	|| !pageForm.getS_applyClassification().equals("on")) {
			       ...
			       return new ActionForward("/pages/IndexHome.jsp?D1="	+ compositionForm.getIndexId()+ "&B2=Compute&basedate="	+ pageForm.getD_baseDate());
				} 
					...
					return new ActionForward("/pages/IndexHome.jsp?D1="	+ compositionForm.getIndexId()+ "&B2=Compute&basedate="+ pageForm.getD_baseDate());
					                 ...
					                 Logging.debug("5");
					         return new ActionForward("/pages/GetbaseValue.jsp?fromcomposition=yes");
				       }
			    }
		...
		try{
			...
			NewIndexForm idxForm=(NewIndexForm)session1.getAttribute("newIndexForm");
			// main if started
			if(flag ==true)
			{          
				   ...
				   // Logging.debug("Compliance id ======>"+compid);
					...
					//  mcap filter o/p
						  if (!mcapdata.isEmpty()) {
					          Logging.debug("Inside isEmpty  " +mcapdata.isEmpty());
						...
						// ind class filter output
						  ...
						  if (!tvoldata.isEmpty()) {
					          Logging.debug("Inside isEmpty  " +tvoldata.isEmpty());
						  ...
						  if (!tvaldata.isEmpty()) {
					          Logging.debug("Inside isEmpty  " +tvaldata.isEmpty());
						  ...
						  if (!filterdata.isEmpty()) {
					          Logging.debug("Inside isEmpty  " +filterdata.isEmpty());
						...
						// comparison of two collection main output  
						  ...
						  if (!cmpmain.isEmpty()) {
					          Logging.debug("Inside isEmpty  " +cmpmain.isEmpty());
	...
	public void createIndex(NewIndexForm pageForm,
		...
		try {
			...
			Logging
					.debug("StockDetailsCollection: Error inserting in table "
							+ e.getMessage());

			// TODO: handle exception
		}
		storeComposition(pageForm.getI_indexID(), pageForm
				.getDatabaseConnection(), ConnectInit.queries
				.getProperty("insert_into_index_composition"), compositionForm,
				request, pageForm);

		if (pageForm.getS_applyClassification() != null
				&& pageForm.getS_applyClassification().equals("on")) {
			Logging.debug("request.getParameter(\"checktocreatechild\") : "
					+ request.getParameter("checktocreatechild"));
			if (request.getParameter("checktocreatechild").equals("on")) {
				try {
					...
					cs = pageForm
							...
							.prepareCall(
									"{ call public.define_and_compose_all_child_index(?) }");

					int inid = new Integer(compositionForm.getIndexId())
							.intValue();
					cs.setInt(1, inid);
					Logging.debug("Executed procedure : " + cs.execute());
				} catch (Exception ex) {
					Logging
							.debug("Error in automatically creating child indices :"
									+ ex);
					errors.add(ActionErrors.GLOBAL_ERROR, new ActionError(
					...
					try {
						...
						Logging.info("database commited ");
						// TODO: handle exception
					}
			...
			try {
				...
				for (Enumeration e = destinationTable.keys(); e
					...
					Logging.debug("indexID : " + indexID + "  : "
							+ request.getParameter("iwf" + sd.getStockID()));
					//psmt.setInt(1, id1);
				...
				con.commit();
				pageform.getDatabaseConnection().commit();
			} catch (Exception e) {
