...
public class ActionCorp {
	...
	// This function is called when user wants to see affected index divisor and
	// tmcv
	public static void toGetaffect(Corporate corporateact) {
		try {
			...
			double oldmcv = cf.calMarketCap(Double.parseDouble(oldltp),
					Long.parseLong(corporateact.getMark_lot()),
					Double.parseDouble(corporateact.getCurr_val()),
					Long.parseLong(oldtis), Double.parseDouble(iwf));
			Logging.error("OLDMCV  " + oldmcv);

			double newmcv = cf.calMarketCap(Double.parseDouble(newltp),
					Long.parseLong(corporateact.getMark_lot()),
					Double.parseDouble(corporateact.getCurr_val()),
					Long.parseLong(newtis), Double.parseDouble(iwf));
			...
			double newdivisor = cf.newDivisorCorp(Double.parseDouble(oldtmcv),
					diff, Double.parseDouble(olddivisor));
	...
	// This function is called when user wants to see Undo affected index
	// divisor and tmcv
	public static void recaltoGetaffect(Corporate corporateact) {
		try {
			String newltp = corporateact.getNewLTP();
			String newtis = corporateact.getNewTIS();
			String iwf = corporateact.getIwfstk();
			...
			double newdivisor = cf.RecalnewDivisorCorp(
					Double.parseDouble(oldtmcv), diff,
					Double.parseDouble(olddivisor));
			corporateact.setNewdivisor(Double.toString(newdivisor));
	...
	public static void actionOnApply(Corporate corporate) {
		...
		try {
			...
			if (corp.equals("rightsoffering"))
				rightsOfferAction(ltp, tis, ratio, corporate.getAmt(),
						corporate);
			if (corp.equals("sharesbuyback"))
				repurchaseAction(ltp, tis, corporate.getAmt(),
						corporate.getShare(), corporate);
			...
			double newmcv = cf.calMarketCap(
					Double.parseDouble(corporate.getNewLTP()),
					Long.parseLong(corporate.getMark_lot()), 1,
					Long.parseLong(corporate.getNewTIS()),
					Double.parseDouble(corporate.getIwfstk()));
	...
	public static void reCalOnApply(Corporate corporate) {
		try {
			corporate.setOldtis(corporate.getTis());
			String tis = corporate.getOldtis();
			String ltp = corporate.getClose();
			...
			if (corp.equals("reversesplit"))
				recalsplitAction(ltp, tis, ratio, corporate);
			if (corp.equals("rightsoffering"))
				recalrightsOfferAction(ltp, tis, ratio, corporate.getAmt(),
						corporate);
			if (corp.equals("sharesbuyback"))
				recalrepurchaseAction(ltp, tis, corporate.getAmt(),
						corporate.getShare(), corporate);
			if (corp.equals("spin-off")) {
				if (ratio.equals(":")) {
					ratio = 1 + ":" + 1;
			...
			if (corp.equals("warrantconversion"))
				recalwarrantAction(ltp, tis, ratio, corporate.getShare(),
						corporate);
			if ((corp.equals("shareissuance")) | (corp.equals("adrissue")))
	...
	public static void reducecapAction(String oldLtp, String oldTis,
			String newTis, Corporate corporateact) {
		// CFormula cf = new CFormula();
		CFormula cf = ConnectInit.getCFormula();
		long tisnew;
		double ltpnew = Double.parseDouble(oldLtp);
		tisnew = cf
				.newTISReduce(Long.parseLong(oldTis), Long.parseLong(newTis));
	...
	public static void recalreducecapAction(String oldLtp, String oldTis,
			String newTis, Corporate corporateact) {
		// CFormula cf = new CFormula();
		CFormula cf = ConnectInit.getCFormula();
		long tisnew;
		double ltpnew = Double.parseDouble(oldLtp);
		tisnew = cf.newRecalTISReduce(Long.parseLong(oldTis),
				Long.parseLong(newTis));
		String newtis = new Long(tisnew).toString();
		String newlt = new Double(ltpnew).toString();
		long adjust = (tisnew - Long.parseLong(oldTis));
		String adj = new Long(adjust).toString();
	...
	public static void shareissueAction(String oldLtp, String oldTis,
			String newTis, Corporate corporateact) {
	...
	public static void recalshareissueAction(String oldLtp, String oldTis,
			String newTis, Corporate corporateact) {
		// CFormula cf = new CFormula();
		CFormula cf = ConnectInit.getCFormula();
		long tisnew;
		double ltpnew = Double.parseDouble(oldLtp);
		tisnew = cf.newRecalTISIssue(Long.parseLong(oldTis),
				Long.parseLong(newTis));
		String newtis = new Long(tisnew).toString();
		String newlt = new Double(ltpnew).toString();
	...
	public static void stockDividendAction(String oldLtp, String oldtis,
			String ratio, Corporate corporateact) {
		String str[] = new String[2];
		str = token(ratio);
		// CFormula cf = new CFormula();
		CFormula cf = ConnectInit.getCFormula();
		double ltpnew;
		long tisnew;
		ltpnew = cf.newLTPStkDivi(Double.parseDouble(oldLtp),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]));
		tisnew = cf.newTISStkDivi(Long.parseLong(oldtis),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]));
		String newlt = new Double(ltpnew).toString();
		String newtis = new Long(tisnew).toString();
		long adjust = (tisnew - Long.parseLong(oldtis));
	...
	public static void RecalstockDividendAction(String oldLtp, String oldtis,
			String ratio, Corporate corporateact) {
		String str[] = new String[2];
		str = token(ratio);
		CFormula cf = ConnectInit.getCFormula();
		...
		double ltpnew;
		long tisnew;
		ltpnew = cf.newRecalLTPStkDivi(Double.parseDouble(oldLtp),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]));
		tisnew = cf.newRecalTISStkDivi(Long.parseLong(oldtis),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]));
	...
	public static void cashDividendAction(String oldltp, String oldtis,
			String amt, Corporate corporateact) {
		// CFormula cf = new CFormula();
		CFormula cf = ConnectInit.getCFormula();
		double ltpnew;
		ltpnew = cf.newLTPCashDivi(Double.parseDouble(oldltp),
	...
	public static void splitAction(String oldLtp, String oldtis, String ratio,
		...
		double ltpnew;
		long tisnew;
		ltpnew = cf.newLTPSplitRev(Double.parseDouble(oldLtp),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]));
		tisnew = cf.newTISSplitRev(Long.parseLong(oldtis),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]));
		String newlt = new Double(ltpnew).toString();
		String newtis = new Long(tisnew).toString();
		long adjust = (tisnew - Long.parseLong(oldtis));
		String adj = new Long(adjust).toString();
		double nface = cf.newLTPSplitRev(
				Double.parseDouble(corporateact.getFace()),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]));
	...
	public static void recalsplitAction(String oldLtp, String oldtis,
			String ratio, Corporate corporateact) {
		String str[] = new String[2];
		str = token(ratio);
		CFormula cf = ConnectInit.getCFormula();
		...
		double ltpnew;
		long tisnew;
		ltpnew = cf.newRecalLTPSplitRev(Double.parseDouble(oldLtp),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]));
		tisnew = cf.newRecalTISSplitRev(Long.parseLong(oldtis),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]));
		...
		long adjust = (Long.parseLong(oldtis) - tisnew);
		String adj = new Long(adjust).toString();
		double nface = cf.newRecalLTPSplitRev(
				Double.parseDouble(corporateact.getFace()),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]));
		String newface = new Double(nface).toString();
		corporateact.setNewFace(newface);
	...
	public static void rightsOfferAction(String oldLtp, String oldtis,
			String ratio, String amount, Corporate corporateact) {
		String str[] = new String[2];
		str = token(ratio);
		CFormula cf = ConnectInit.getCFormula();
		// CFormula cf = new CFormula();
		double ltpnew;
		long tisnew;
		ltpnew = cf.newLTPRightsOff(Double.parseDouble(oldLtp),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]),
				Double.parseDouble(amount));
		tisnew = cf.newTISRightsOff(Long.parseLong(oldtis),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]));
	...
	public static void recalrightsOfferAction(String oldLtp, String oldtis,
			String ratio, String amount, Corporate corporateact) {
		String str[] = new String[2];
		str = token(ratio);
		...
		double ltpnew;
		long tisnew;
		ltpnew = cf.newRecalLTPRightsOff(Double.parseDouble(oldLtp),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]),
				Double.parseDouble(amount));
		tisnew = cf.newRecalTISRightsOff(Long.parseLong(oldtis),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]));
	...
	public static void repurchaseAction(String oldLtp, String oldtis,
			String amount, String percent, Corporate corporateact) {
		// CFormula cf = new CFormula();
		CFormula cf = ConnectInit.getCFormula();
		double ltpnew;
		long tisnew;

		ltpnew = cf.newLTPRepurchase(Double.parseDouble(oldLtp),
				Long.parseLong(oldtis), Long.parseLong(percent),
				Double.parseDouble(amount));
		tisnew = cf.newTISRepurchase(Long.parseLong(oldtis),
	...
	public static void recalrepurchaseAction(String oldLtp, String oldtis,
			String amount, String percent, Corporate corporateact) {
		// CFormula cf = new CFormula();
		CFormula cf = ConnectInit.getCFormula();
		double ltpnew;
		...
		ltpnew = cf.newRecalLTPRepurchase(Double.parseDouble(oldLtp),
				Long.parseLong(oldtis), Long.parseLong(percent),
				Double.parseDouble(amount));
		tisnew = cf.newRecalTISRepurchase(Long.parseLong(oldtis),
				Long.parseLong(percent));
	...
	public static void warrantAction(String oldLtp, String oldtis,
			String ratio, String percent, Corporate corporateact) {
		String str[] = new String[2];
		str = token(ratio);
		CFormula cf = ConnectInit.getCFormula();
		// CFormula cf = new CFormula();
		long tisnew;
		long value = cf.noOSharesFWar(Long.parseLong(str[0]),
				Long.parseLong(str[1]), Long.parseLong(percent));
	...
	public static void recalwarrantAction(String oldLtp, String oldtis,
			String ratio, String percent, Corporate corporateact) {
		String str[] = new String[2];
		str = token(ratio);
		CFormula cf = ConnectInit.getCFormula();
	...
	public static void spinAction(String oldLtp, String oldtis, String ratio,
			String amount, Corporate corporateact) {
		String str[] = new String[2];
		str = token(ratio);
		CFormula cf = ConnectInit.getCFormula();
		// CFormula cf = new CFormula();
		double ltpnew;
		long tisnew;
		ltpnew = cf.newLTPSpinOff(Double.parseDouble(oldLtp),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]),
				Double.parseDouble(amount));
		tisnew = cf.newTISSpinOff(Long.parseLong(oldtis),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]));// Long.parseLong(oldtis);
	...
	public static void reCalSpin(String oldLtp, String oldtis, String ratio,
			String amount, Corporate corporateact) {
		String str[] = new String[2];
		str = token(ratio);
		...
		double ltpnew;
		long tisnew;
		ltpnew = cf.newRecalLTPSpinOff(Double.parseDouble(oldLtp),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]),
				Double.parseDouble(amount));
		tisnew = cf.newRecalTISSpinOff(Long.parseLong(oldtis),
				Double.parseDouble(str[0]), Double.parseDouble(str[1]));// Long.parseLong(oldtis);
	...
	public static void recalIndex(Connection con, Connect connect,
			Corporate corporateact) {
		ResultSet rs = null;
		ResultSet rs1 = null;
		ResultSet rs2 = null;
		...
		try {
			...
			if (chk_dt == 0) {
				...
				rs1 = ListTypeClass1.getResult12(con, query1,
						corporateact.getI_index());
			} else {
				String query1 = ConnectInit.queries
						.getProperty("get_undo_index_close");
				rs1 = ListTypeClass1
						.getResult_apply(con, query1,
								corporateact.getI_index(),
			...
			rs = ListTypeClass1.getResult1(con, ind_com_query,
					corporateact.getI_index());
			...
			while (rs.next()) {
				...
				Logging.debug("apply date from line no  632 of actioncorp  "
						+ apply1);
				...
				if (chk_dt == 0)
					rs2 = ListTypeClass1.getResult12(con, query, sid);
				else
					rs2 = ListTypeClass1.getResult_apply(con, stk_qry, sid,
							corporateact.getApply_date());
				close_val = rs2.next();
					...
					if (chk_hash2 == true) {
						...
						if (close_val == true) {
							...
							if (close == null) {
								rs2.close();
								data_error
										.put(new String(sid), new String(sid));
							} else {
								if (close.equals("0")) {
									rs2.close();
									data_error.put(new String(sid), new String(
											sid));
								} else {
									...
									Object obj = null;
									for (Enumeration enumm = data2.keys(); enumm
											.hasMoreElements();) {
										String val = (String) enumm
												.nextElement();
										if (val.equals(sid)) {
											obj = data2.get(val);
											break;
						...
						if (close_val == true) {
							...
							if (close == null) {
								rs2.close();
								stock_error.put(new String(sid),
										new String(sid));
							} else {
								if (close.equals("0")) {
									rs2.close();
				...
				if (chk_hash == true) {
					...
					if (close_val == false) {
						data_error.put(new Integer(sid).toString(), new String(
								sid));
					}
					if (close_val == true) {
						String close = rs2.getString("adjusted_price");
			...
			if (hash2.isEmpty() == false) {
				...
				for (Enumeration enumm = hash2.keys(); enumm.hasMoreElements();) {
					...
					rs2 = ListTypeClass1.getResult_corp(con, qry,
							corporateact.getI_index(), id);
					boolean chk_rs = rs2.next();
					if (chk_rs == false) {

						if (chk_dt == 0)
							rs3 = ListTypeClass1.getResult12(con, query, id);
						else
							rs3 = ListTypeClass1.getResult_apply(con, stk_qry,
									id, corporateact.getApply_date());

						boolean close_val = rs3.next();
						if (close_val == false) {
							data_error.put(new Integer(id).toString(),
									new String(id));
						}
						if (close_val == true) {
							...
							if (close == null)
								...
								if (close.equals("0"))
									data_error.put(new String(id), new String(
											id));
								else {
									rs1 = ListTypeClass1.getAffected(con,
											stk_query, id);
									...
									for (Enumeration enum1 = data2.keys(); enum1
											.hasMoreElements();) {
										String val = (String) enum1
												.nextElement();
										if (val.equals(id)) {
											obj1 = data2.get(val);
											break;
									...
									double mcv = cf.calMarketCap(Double
											.parseDouble(close), Long
											.parseLong(ml), Double
											.parseDouble(corporateact
													.getCurr_val()), Long
	...
	public static void Hist_apply(Corporate corp, int flag) {
		...
		try {
			...
			Logging.debug("connection in apply is==" + Hist_con);
			if (flag == 1)// Index has a closing value for respective date
			{
				corp.setInd_comp(null);
				applyIndexDetail(Hist_con, connect, corp); // calculate index
															// parameter
				String ind_query = ConnectInit.queries
						.getProperty("select_resp_close"); // get index values
															// for particular
															// date
			...
			{
				try {
					Logging.debug("in compute");
					Logging.debug("index is==" + corp.getI_index());
					Logging.debug("apply date" + corp.getApply_date());
					...
					Logging.debug("call compute method");
					// Recompute Index Parameter
					cind.computeIndex(index, "n", "y", "yes", null, request,
							corp.getApply_date(), Hist_con);

					Logging.debug("out of compute");
					corp.setInd_comp(null);
					applyIndexDetail(Hist_con, connect, corp); // calculate
																// index
																// parameter
					String ind_query = ConnectInit.queries
							.getProperty("select_resp_close"); // get index
																// values for
																// particular
																// date
					check_affect_index(Hist_con, corp, ind_query);
			...
			{
				corp.setInd_comp("c");
				recal_applyIndexDetail(Hist_con, connect, corp);// recalculate
																// new index
																// value
				...
				for (Enumeration enum1 = hash1.keys(); enum1.hasMoreElements();) {
					...
					if (corp_val.equals("addstock")) {
						ListTypeClass1.affect_index_list(Hist_con, corp,
								child_index, stid);
					}
	...
	public static void Hist_applyindex(Corporate corp, int flag) {
		Connection Hist_con = null;
		try {
			// Connect connect = ConnectInit.getConnect();
			Connect connect = ConnectInit.getConnect();
			...
			{
				corp.setInd_comp(null);
				applyIndex(Hist_con, connect, corp); // calculate index
														// parameter
			}
			if (flag == 2) // Index has to be recompute
			{
			...
			{
				corp.setInd_comp("c");
				recalIndex(Hist_con, connect, corp); // recalculate index
														// parameter
			}
			IndexCom_Action.tmcv_div_adj(corp);
		} catch (Exception e) {
	...
	public static void applyIndex(Connection con, Connect connect,
			Corporate corporateact)// ,String query,String query1)
	{
		// queries
		/*
		 * String query =
		 * connect.queries.getProperty("select_stock_price_detail"); String
		 * stk_qry = connect.queries.getProperty("get_undo_close_value"); String
		 * query1 =
		 * connect.queries.getProperty("get_latest_index_value_index_wise");//
		 * "select_index_detail"); String ind_query =
		 * connect.queries.getProperty("select_resp_close"); // get index values
		 * for particular date String stk_query =
		 * connect.queries.getProperty("detail_stock_master"); String ind_qry =
		 * connect.queries.getProperty("index_comp_detail");
		 */
		String query = ConnectInit.queries
				.getProperty("select_stock_price_detail");
		String stk_qry = ConnectInit.queries
		...
		String dt = UpdateCorp.accept_date(); // get the current date
		String apply = corporateact.getApply_date();
		int chk_dt = ComputeIndexForm.CompareDate(apply, dt); // check for the
																// current date
																// and user's
																// entered date
			...
			try {
				if (leng > 0) {
					for (Enumeration enum1 = data.keys(); enum1
						...
						String id2 = (String) enum1.nextElement();
						if (chk_dt == 0)
							rs = ListTypeClass1.getResult12(con, query, id2);
						else
							rs = ListTypeClass1.getResult_apply(con, stk_qry,
									id2, corporateact.getApply_date());
						close_val = rs.next();
						if (close_val == false) {
							data_error.put(new Integer(id2).toString(),
									new String(id2));
						}
						if (close_val == true) {
							...
							if (close == null) {
								...
								if (close.equals("0"))
									...
									Logging.debug("mcv====" + mcv);
									if (newTmcv_del == 0)
										newTmcv_del = Double.parseDouble(tmcv)
												- mcv;
									else
						...
						if (close_val == true) {
							...
							if (close == null) {
								...
								if (close.equals("0"))
									...
									rs1 = ListTypeClass1.getResult1(con,
											cquery, cad);
									rs1.next();
									...
									if (corp_nm.equals("change iwf")) {
										...
										double mcv_new = cf.calMarketCap(Double
												...
												.parseDouble(corporateact
														.getCurr_val()), Long
												.parseLong(tis), iwf_val);
										mcv = mcv_new - mcv_old;
										if (newTmcv_del == 0)
											newTmcv_del = Double
													.parseDouble(tmcv) + mcv;
										else
											newTmcv_del = newTmcv_del + mcv;
									}
	...
	public static void undo_recal_applyIndexDetail(Corporate corporateact) {
		// Connect connect = ConnectInit.getConnect();
		Connect connect = ConnectInit.getConnect();
		Connection con = null;
		ResultSet rs = null;
		...
		try {
			...
			String query1 = ConnectInit.queries
					.getProperty("get_undo_index_close");// "select_index_detail");
			String ind_com_query = ConnectInit.queries
					.getProperty("get_composition_of_parent");
			// String
			// query=connect.queries.getProperty("select_stock_price_detail");
			String query = ConnectInit.queries
					.getProperty("get_undo_close_value");
			String stk_query = ConnectInit.queries
					.getProperty("detail_stock_master");
			...
			try {
				...
				rs = ListTypeClass1.getResult_apply(con, query1, index,
						corporateact.getApplied_date());
				rs.next();
				...
				String stock = null;
				double newtmcv = 0.0;
				rs1 = ListTypeClass1.getResult1(con, ind_com_query,
						corporateact.getI_index());
				while (rs1.next()) {
					double mcv = 0.0;
					...
					boolean close_val = false;
					rs2 = ListTypeClass1.getResult_apply(con, query, sid,
							corporateact.getApplied_date());
					close_val = rs2.next();
					if (close_val == true) {
						String close = rs2.getString("adjusted_price");
						...
						if (close == null) {
							...
							if (chk_hash == true)
								...
								hash_stk_error.put(new String(sid), new String(
										sid));
						} else {
							if (close.equals("0")) {
								rs2.close();
								if (chk_hash == true)
									hash_error.put(new String(sid), new String(
											sid));
								else
									hash_stk_error.put(new String(sid),
											new String(sid));
							} else {
								// get currency exchange value
								NCorp_Action.get_currency(con, connect,
										corporateact, index, sid);

								rs = ListTypeClass1.getAffected(con, stk_query,
										sid);
								rs.next();
								String tis = rs.getString("tis");
								...
								if (chk_hash == true) {
									...
									if (corp.equals("changeiwf")) {
										for (Enumeration enum1 = hash1.keys(); enum1
												.hasMoreElements();) {
											String id2 = (String) enum1
													.nextElement();
											String div1[] = ActionCorp
													.token(id2);
											rs = ListTypeClass1.getAffected(
													con, query2, div1[1]);
											rs.next();
											String stid = rs
													.getString("stock_id");
				...
				if (corp.equals("deletestock")) {
					for (Enumeration enum1 = hash1.keys(); enum1
						...
						Object obj = hash1.get(id2);
						String stid = obj.toString();
						rs2 = ListTypeClass1.getResult_apply(con, query, stid,
								corporateact.getApplied_date());
						...
						if (close_val == true) {
							...
							if (close == null) {
								rs2.close();
								hash_error.put(new String(stid), new String(
										stid));
							} else {
								if (close.equals("0")) {
									rs2.close();
									hash_error.put(new String(stid),
											new String(stid));
								} else {
									rs = ListTypeClass1.getAffected(con,
											stk_query, stid);
									rs.next();
									String tis = rs.getString("tis");
									String ml = rs.getString("market_lot");
	...
	public static void recal_applyIndexDetail(Connection con, Connect connect,
			Corporate corporateact) {
		String query = ConnectInit.queries
				.getProperty("select_stock_price_detail");
		String stk_qry = ConnectInit.queries
		...
		try {
			...
			double newtmcv = 0.0;
			int count = 0;
			ResultSet rs1 = ListTypeClass1.getResult1(con, ind_com_query,
					corporateact.getI_index());
			while (rs1.next()) {
				count++;
				...
				if (close_val == true) {
					...
					if (close == null) {
						...
						if (close.equals("0")) {
							...
							rs = ListTypeClass1
									.getAffected(con, stk_query, sid);
							rs.next();
							String tis = rs.getString("tis");
							String ml = rs.getString("market_lot");
							Logging.debug("corp action==" + corp + "chk_hash=="
									+ chk_hash);
							if (chk_hash == true) {
								if (corp.equals("deletestock")) {
									newtmcv = newtmcv;
			...
			if (corp.equals("addstock")) {
				...
				for (Enumeration enum1 = hash1.keys(); enum1.hasMoreElements();) {
					...
					ResultSet rs2 = null;
					if (chk_dt == 0)
						rs2 = ListTypeClass1.getResult12(con, query, stid);
					else
						rs2 = ListTypeClass1.getResult_apply(con, stk_qry,
								stid, corporateact.getApply_date());
					boolean close_val = rs2.next();
					if (close_val == true) {
						String close = rs2.getString("adjusted_price");
						if (close == null)
	...
	public static void undoapplyIndexDetail(Corporate corporateact) {
		// Connect connect = ConnectInit.getConnect();
		Connect connect = ConnectInit.getConnect();
		Connection con = null;
		ResultSet rs = null;
		...
		try {
			...
			try {
				...
				rs = ListTypeClass1.getResult_apply(con, query1, index, date);
				rs.next();
				String tmcv = rs.getString("tmcv");
				String div = rs.getString("divisor");
				...
				for (Enumeration enum1 = hash1.keys(); enum1.hasMoreElements();) {
					...
					stock = obj.toString();
					rs = ListTypeClass1
							.getResult_apply(con, query, stock, date);
					check_rs = rs.next();
					if (check_rs == false) {
						...
						if (close == null) {
							...
							if (close.equals("0"))
								...
								rs1 = ListTypeClass1.getResult12(con, query3,
										stock);
								rs1.next();
								...
								if (corp.equals("addstock")) {
									...
									if (new_tmcv == 0.0)
										new_tmcv = Double.parseDouble(tmcv)
												- mcv;
									else
				...
				if (divi == 0.0) {
					double newdivisor = (new_tmcv / Double
	...
	public static void applyIndexDetail(Connection con, Connect connect,
			Corporate corporateact) {
		ResultSet rs = null;
		ResultSet rs1 = null;
		...
		try {
			...
			if (chk_dt == 0)
				...
				rs = ListTypeClass1.getResult_apply(con, ind_query, index,
						corporateact.getApply_date()); // 1
			String tmcv = null;
			String div = null;
			...
			for (Enumeration enum1 = hash1.keys(); enum1.hasMoreElements();) {
				...
				if (div1[0].equals("false")) {
					...
					if (chk_dt == 0)
						...
						rs = ListTypeClass1.getResult_apply(con, stk_qry,
								div1[1], corporateact.getApply_date());
					check_rs = rs.next();
					if (check_rs == false)
						hash_error
						...
						if (close == null) {
							...
							if (close.equals("0")) {
								...
								if ((corp.equals("deletestock"))
										| (corp.equals("changeiwf")))
									ListTypeClass1
											.affect_index_list(con,
													corporateact, affect_index,
													div1[1]);// 2
								if (corp.equals("addstock"))
									ListTypeClass1.affect_index_list(con,
											corporateact, child_index, div1[1]);
								if (rs != null)
									rs.close();
								rs1 = ListTypeClass1.getAffected(con,
										stk_query, div1[1]);
	...
	public static void Historic_cal(Corporate corp) {
		...
		try {
			...
			// CFormula cf = new CFormula();
			CFormula cf = ConnectInit.getCFormula();
			String qry = ConnectInit.queries.getProperty("get_cname_type"); // get
			// CA
			// shortname
			String stk_qry = ConnectInit.queries
					.getProperty("get_undo_close_value");// get stock closing
															// value for
															// respective date
			String stk_tis = ConnectInit.queries
					.getProperty("detail_stock_master"); // get
															// stock
															// tis
			String up_qry = ConnectInit.queries.getProperty("update_index"); // update
																				// index
																				// composition
			Vector v = corp.getV();
			if (v.isEmpty() == false) {
				for (Enumeration enumm = v.elements(); enumm.hasMoreElements();) {
					...
					rs = ListTypeClass1.getAffected(Hist_con, qry, id);
					String type = null, fcorp_nm = null, corp_nm = null, date = null;
					boolean isData = false;
					if (rs != null)
						isData = rs.next();
					/*
					 * String type=rs.getString("corporate_action_type"); String
					 * fcorp_nm=null; String
					 * corp_nm=rs.getString("cam_shortname"); String
					...
					if (isData) {
						...
						sid = rs.getString("stock_id"); // get stock id
						corp.setStid(sid);
						corp.setStkid(sid);
					}
					if (!(corp_nm.equals("change indcurr"))) {
						corp.setApplied_date(date);
						rs1 = ListTypeClass1.getResult_apply(Hist_con, stk_qry,
								sid, date); // get stock
											// closing value
						rs1.next();
						...
						rs1 = ListTypeClass1
								.getAffected(Hist_con, stk_tis, sid); // get
																		// stock
																		// tis
						rs1.next();
						String tis = rs1.getString("tis");
					...
					if (type.equals("stock event")) {
						...
						UpdateCorp.update_undo_stkprice(Hist_con, undo_query,
								query, corp);

						if ((!(corp_nm.equals("stock dividend/bonus")))
								| (!(corp_nm.equals("cash dividend")))) {
							String ind_qry = ConnectInit.queries
									.getProperty("select_index_composition");
							rs1 = ListTypeClass1.getAffected(Hist_con, ind_qry,
									sid);
							while (rs1.next()) {
								String index = rs1.getString("index_id");
								Logging.debug("sotck aff==" + index);
								// get currency exchange value and calculate MCV
								NCorp_Action.get_currency(Hist_con, connect,
										corp, index, sid);
								...
								UpdateCorp.update_hist_index(Hist_con, up_qry,
										rs1, corp);
								...
								String query1 = ConnectInit.queries
										.getProperty("get_undo_index_close"); // get
																				...
																				// particular
																				// date
								rs2 = ListTypeClass1.getResult_apply(Hist_con,
										query1, index, date);
								rs2.next();
								...
								UpdateCorp.update_undo_index_daily(Hist_con,
										query1, query, corp);
							}
						}
			...
			if (corp.getR_type().equals("stock event")) {
				rs1 = ListTypeClass1.getResult_apply(Hist_con, stk_qry, sid,
						date); // get stock closing value
				rs1.next();
				String sclose = rs1.getString("adjusted_price");
				if (sclose == null)
					sclose = rs1.getString("stock_closing_value");
				...
				rs1 = ListTypeClass1.getAffected(Hist_con, stk_tis, sid); // get
																			// stock
																			// tis
				rs1.next();
				String tis = rs1.getString("tis");
				...
				NCorp_Action.CalApply(Hist_con, connect, corp);// calculate new
																// value
			}
			if (corp.getR_type().equals("index event")) {
				...
				if (corp_nm.equals("changeindcurr")) {
					...
					ListTypeClass1.affect_child_list(Hist_con, corp, query);
	...
	public static void hist_affect_cal(Corporate corp, String affect) {
		Connection Hist_con = null;
		ResultSet rs = null;
		try {
			// Connect connect = ConnectInit.getConnect();
			...
			String query1 = ConnectInit.queries
					.getProperty("get_undo_index_close"); // get
															...
															// particular
															// date
			rs = ListTypeClass1.getResult_apply(Hist_con, query1, affect,
					corp.getApply_date());
			rs.next();
			String tmcv = rs.getString("tmcv");
			...
			if (corp_name.equals("cashdividend")) {
				// check from system configuration table whether to adjust price
				// or not for cash dividend
				String query = ConnectInit.queries
						.getProperty("select_system_config");
				try {
					rs = ListTypeClass1.resultCorporate(Hist_con, query);
					rs.next();
				...
				// in case of stock dividend/bonus there is no change in tmcv &
				// divisor
	...
	public static void undo_ind(Connection con, Connect connect,
			Corporate corp, String stock, String index, String diary,
			String corp_nm, String flag) {
		String query1 = ConnectInit.queries.getProperty("get_undo_index_close"); // get
																					// index
																					// values
																					// for
																					// particular
		...
		try {
			...
			ResultSet rs = ListTypeClass1.getResult_apply(con, query1, index,
					date);
			rs.next();
			String tmcv = rs.getString("tmcv");
			String div = rs.getString("divisor");
			...
			{
				if ((corp_nm.equals("add stock"))
						| (corp_nm.equals("change iwf")))
					ListTypeClass1.affect_index_list(con, corp, affect_index,
							stock);
				if (corp_nm.equals("delete stock"))
					ListTypeClass1.affect_index_list(con, corp, child_index,
							stock);
			}
			// get IWF
			if ((corp_nm.equals("add stock")) | (corp_nm.equals("change iwf"))) {
				rs = ListTypeClass1.getResult_corp(con, index_comp, index,
						stock);
				rs.next();
				value = rs.getString("iwf");
			}
			if (corp_nm.equals("delete stock")) {
				rs = ListTypeClass1.getResult12(con, query3, stock);
				rs.next();
				value = rs.getString("iwf");
			}
			// get TIS
			ResultSet rs1 = ListTypeClass1.getResult12(con, query3, stock);
			rs1.next();
			String tis = rs1.getString("tis");
			String ml = rs1.getString("market_lot");
			rs1.close();
			...
			// get currency exchange value
			NCorp_Action.get_currency(con, connect, corp, index, stock);
			...
			if (corp_nm.equals("change iwf")) {
				double mcv_new = cf.calMarketCap(Double.parseDouble(close),
						Long.parseLong(ml),
						Double.parseDouble(corp.getCurr_val()),
						Long.parseLong(tis), Double.parseDouble(value));
				Logging.debug("mcv in new==" + mcv_new);
				double mcv_old = cf.calMarketCap(Double.parseDouble(close),
						Long.parseLong(ml), 1, Long.parseLong(tis),
			...
			String query = ConnectInit.queries
					.getProperty("update_undo_ind_close");
			// String
			// ind_val_daily=connect.queries.getProperty("update_resp_index_value_daily");
	...
	public static void undo_currind(Connection con, Connect connect,
			Corporate corp) {
		...
		try {
			...
			temp_update_curr(con, connect, corp, date, newcurr_val,
					ft_currency, desc);
			// get child indices
			String query = ConnectInit.queries
					.getProperty("resp_child_indcurr");
			...
			if (hash_aff.isEmpty() == false) {
				for (Enumeration enumm = hash_aff.keys(); enumm
						.hasMoreElements();) {
					String id = (String) enumm.nextElement();
	...
	public static void temp_update_curr(Connection con, Connect connect,
			Corporate corp, String date, double newcurr_val,
			String ft_currency, String desc) {
		try {
			// queries
			String stk_qry = ConnectInit.queries
					.getProperty("get_undo_close_value");// connect.queries.getProperty("get_resp_date_closevalue");//get
															// stock closing
															// value for
															// respective date
			// CFormula cf = new CFormula();
			...
			String query = ConnectInit.queries
					.getProperty("get_composition_of_parent");
			rs = ListTypeClass1.getAffected(con, query, corp.getI_index());
			while (rs.next()) {
				...
				rs1 = ListTypeClass1.getResult_apply(con, stk_qry, stock, date); // get
																					// stock
																					// closing
																					// value
				rs1.next();
				...
				double mcv = cf.calMarketCap(Double.parseDouble(sclose),
						Long.parseLong(ml), newcurr_val, Long.parseLong(tis),
			...
			ResultSet rs1 = ListTypeClass1.getAffected(con, qry, ft_currency);
			rs1.next();
			...
			rs1 = ListTypeClass1.getAffected(con, qry, corp.getI_index());
			rs1.next();
	...
	public static void check_curr(Corporate corporateact) {
		// Connect connect = ConnectInit.getConnect();
		Connect connect = ConnectInit.getConnect();
		Connection con = null;
		ResultSet rs = null;
		try {
			...
			try {
				int oldcur = 0;
				int newcur = 0;
				// String query = connect.queries.getProperty("resp_ftcurr");
				String query = ConnectInit.queries.getProperty("resp_ftcurr");
				rs = ListTypeClass1.getAffected(con, query,
						corporateact.getFtcurrency());
				int fcurr = 0;
				int tcurr = 0;
				if (rs != null && rs.next()) {
				...
				if (check == false) {
					for (Enumeration enum1 = affect.keys(); enum1
						...
						rs = ListTypeClass1.getAffected(con, query, id2);
						rs.next();
	...
	public static void check_affect_index(Connection con,
			Corporate corporateact, String query) {
		try {
			...
			if (check == false) {
				for (Enumeration enum1 = affect.keys(); enum1.hasMoreElements();) {
					...
					if (chk_dt == 0)
						...
						rs = ListTypeClass1.getResult_apply(con, query, id2,
								corporateact.getApply_date());
					rs_val = rs.next();
					if (rs_val == false) {
						rs_value = false;
	...
	public static int check_val(Corporate corporateact) {
		int flag = 0;
	...
	public static int check(String query, String query1, Hashtable hash,
			Connection con, Corporate corporateact, String get_name) {
		int flag = 0;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		ResultSet rs2 = null;
		...
		try {
			...
			for (Enumeration enumm = hash.keys(); enumm.hasMoreElements();) {
				...
				rs2 = ListTypeClass1.getAffected(con, get_name, corp);
				rs2.next();
				...
				rs1 = ListTypeClass1.getResult_apply(con, query1, id, date1);
				if (!(rs1.next())) {
					flag = 5;
					break;
				}
				...
				if (itemp1 == null)
					...
					if (((itemp1.equals("Add Stock"))
							| (itemp1.equals("Delete Stock")) | (itemp1
								.equals("Change IWF")))
							& corp_name.equals("Rebasing")) {
						flag = 1;
						break;
					}
					if (((itemp1.equals("Rebasing")) & ((corp_name
							.equals("Add Stock"))
							| (corp_name.equals("Delete Stock")) | (corp_name
								.equals("Change IWF"))))) {
						flag = 1;
						break;
					}
	...
	public static int check_cond(String corp, String leng, String value,
			Corporate corporateact) {
		int flag = 0;
		try {
			CorporateAction error = new CorporateAction();
	...
	public static void hist_affect_ind(Corporate corp, String affect) {
		try {
			// Connect connect = ConnectInit.getConnect();
			Connect connect = ConnectInit.getConnect();
			...
			if (corpnm.equals("changeindcurr")) {
				...
				if (chk.equals("c"))
					recal_affect_value(con, connect, corp, affect); // recalculate
																	// new value
			}
			if (con != null) {
				con.close();
	...
	public static void cal_curr_ind(Connection con, Connect connect,
			Corporate corp, String index) {
		ResultSet rs = null;
		ResultSet rs1 = null;
		try {
			// CFormula cf = new CFormula();
			...
			if (chk_dt == 0) {
				...
				// String query =
				// connect.queries.getProperty("get_undo_index_close");
				String query = ConnectInit.queries
						.getProperty("get_undo_index_close");
				rs = ListTypeClass1.getResult_apply(con, query, index,
						corp.getApply_date());
			}
			String tmcv = null;
			String div = null;
			...
			query = ConnectInit.queries
					.getProperty("get_composition_of_parent");
			rs = ListTypeClass1.getAffected(con, query, index);
			while (rs.next()) {
				double newmcv = 0.0;
				String stock = rs.getString("stock_id");
				String iwf = rs.getString("iwf");
				String qry = ConnectInit.queries
						.getProperty("detail_stock_master");
				rs1 = ListTypeClass1.getAffected(con, qry, stock);
				String tis = null;
				String ml = null;
				if (rs1 != null && rs1.next()) {
					tis = rs1.getString("tis");
				...
				if (flg == 0) {
					newmcv = cf.calMarketCap(Double.parseDouble(close),
	...
	public static StringBuffer affected_index_detail(Connection con,
			Corporate corporateact, String check, String query) {
		StringBuffer str = new StringBuffer();
		ResultSet rs = null;
		try {
			Hashtable affect = corporateact.getHash_affind();
			...
			if (chk_affect == false) {
				...
				while (it.hasNext()) {
					...
					rs = ListTypeClass1.getAffected(con, query, element);
					rs.next();
					...
					if (check == null) {
						...
						if (val == -1)
							str.append("<option value=" + element + ">" + iname
									+ "</option>");
					} else {
						int val = str.indexOf(element);
						if (element.equals(check)) {
							if (val == -1)
								str.append("<option selected value=" + element
										+ ">" + iname + "</option>");
						} else {
							if (val == -1)
	...
	public static void undo_recal_affect_value(Corporate corporateact,
			String index) {
		// Connect connect = ConnectInit.getConnect();
		Connect connect = ConnectInit.getConnect();
		Connection con = null;
		...
		try {
			...
			try {
				String date = corporateact.getApplied_date();
				rs1 = ListTypeClass1.getResult_apply(con, query1, index,
						corporateact.getApplied_date());
				rs1.next();
				...
				rs = ListTypeClass1.getResult1(con, ind_com_query, index);
				double tmcv = 0.0, count = 0;
				// CFormula cf = new CFormula();
				CFormula cf = ConnectInit.getCFormula();
				while (rs.next()) {
					...
					if (chk_hash == true) {
						...
						if (chk_stk == false) {
							rs2 = ListTypeClass1.getResult_apply(con, query,
									sid, date);
							rs2.next();
							String close = rs2.getString("adjusted_price");
							if (close == null)
				...
				if (corp.equals("deletestock")) {
					for (Enumeration enum1 = hash1.keys(); enum1
						...
						rs = ListTypeClass1.getAffected(con, stk_query, stid);
						rs.next();
						String tis = rs.getString("tis");
						String ml = rs.getString("market_lot");
						String iwf = rs.getString("iwf");
	...
	public static void recal_affect_value(Connection con, Connect connect,
		...
		try {
			...
			while (rs.next()) {
				...
				String iwf = rs.getString("iwf");
				// get currency exchange value
				NCorp_Action.get_currency(con, connect, corporateact, index,
						sid);
	...
	public static void undoaffected_index_value(Corporate corporateact,
			String index) {
		// Connect connect = ConnectInit.getConnect();
		Connect connect = ConnectInit.getConnect();
		Connection con = null;
			...
			try {
				...
				if (affect_val == true) {
					...
					for (Enumeration enum1 = hash1.keys(); enum1
						...
						Object obj = hash1.get(id2);
						stock = obj.toString();
						rs = ListTypeClass1.getResult_apply(con, query, stock,
								date);
						check_rs = rs.next();
						if (check_rs == true) {
							close = rs.getString("adjusted_price");
	...
	public static void affected_index_value(Corporate corporateact, String index) {
		// Connect connect = ConnectInit.getConnect();
		Connect connect = ConnectInit.getConnect();
		Connection con = null;
		ResultSet rs = null;
		...
		try {
			...
			try {
				...
				String index_base = ConnectInit.queries
						.getProperty("select_index_name");
				String index_comp_val = ConnectInit.queries
						.getProperty("select_index_comp");
				String stk_query = ConnectInit.queries
				...
				if (affect_val == true) {
					...
					for (Enumeration enum1 = hash1.keys(); enum1
						...
						String div1[] = token(id2);
						if (div1[0].equals("false")) {
							for (Enumeration enumm = hash_affect.keys(); enumm
									.hasMoreElements();) {
						...
						if (div1[0].equals("true")) {
							Logging.debug("-----------in true------------");
							rs = ListTypeClass1.getAffected(con, query2,
									div1[1]);
							String val = null;
							if (rs != null && rs.next()) {
								stock = rs.getString("stock_id");
	...
	public static void hist_affected_index_value(Corporate corporateact,
			String index) {
		// Connect connect = ConnectInit.getConnect();
		Connect connect = ConnectInit.getConnect();
		Connection con = null;
		...
		try {
			...
			try {
				...
				if (div == 0.0) {
					...
					diff = cf.diffTMCV(Double.parseDouble(tmcv), new_tmcv);
					newdivisor = cf.newDivisorCorp(Double.parseDouble(tmcv),
							diff, Double.parseDouble(divi));
					String newdivisor1 = adj.shareholdingpatt(newdivisor);
					;
					corporateact.setNewdivisor1(newdivisor1);
				}
	...
	public static void hist_recal_affect_index(Corporate corporateact,
			String index) {
		Connection con = null;
		try {
			// Connect connect = ConnectInit.getConnect();
			Connect connect = ConnectInit.getConnect();
			con = connect.getConnectionForHistTransaction();
	...
	public static void recal_affect_index(Connection con, Connect connect,
			Corporate corporateact, String index) {
		ResultSet rs = null;
		ResultSet rs1 = null;
		ResultSet rs2 = null;
		...
		try {
			...
			if (hash2.isEmpty() == false) {
				...
				for (Enumeration enumm = hash2.keys(); enumm.hasMoreElements();) {
					String id = (String) enumm.nextElement();
					rs2 = ListTypeClass1.getResult_corp(con, qry, index, id);
					boolean chk_rs = rs2.next();
					if (chk_rs == false) {
						rs3 = null;
						if (chk_dt == 0)
			...
			Logging.error("error in recal==" + e.getMessage());
		} finally {
			try {
				if (rs != null) {
					rs.close();
	...
	public static void affect_comp_index(Connection con, Connect connect,
			Corporate corporateact, String index) {
		ResultSet rs = null;
		ResultSet rs1 = null;
		try {
				...
				if (!(index.equals("Affected Child Index"))) {
					if (affect_rs == true) {
						...
						if (chk_dt == 0)
							...
							rs = ListTypeClass1.getResult_apply(con, ind_query,
									index, corporateact.getApply_date());
						if (rs != null && rs.next()) {
							tmcv = rs.getString("tmcv");
							t = rs.getDouble("tmcv");
							corporateact.setTmcv1(tmcv);
						...
						for (Enumeration enum1 = hash1.keys(); enum1
							...
							if (chk_exist == true) {
								if (chk_dt == 0)
									...
									rs = ListTypeClass1.getResult_apply(con,
											stk_qry, id2,
											corporateact.getApply_date());
								check_rs = rs.next();
								if (check_rs == true) {
						...
						double out = corporateact.getOutstanding();
						for (Enumeration enum1 = hash2.keys(); enum1
								.hasMoreElements();) {
							String id2 = (String) enum1.nextElement();
						...
						if (div == 0.0) {
							double newdivisor = newTmcv_del / index_close;
							corporateact.setNewdivisor1(Double
									.toString(newdivisor));
						} else {
							double diff = cf.diffTMCV(Double.parseDouble(tmcv),
									newTmcv_del);
	...
	public static int check_iwfval(Corporate corporateact) {
		int flag = 0;
