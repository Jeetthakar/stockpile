...
{
	Logger Logging = Logger.getLogger(DynamicTable.class);
	Connection con = null;
	...
	{
		{
			Logging.getDebug("at action."+sqlquery);
			Connection con = null;
			Connect c = ConnectInit.getConnect();
			ResultSet rst = null;
			PreparedStatement pst = null;
			...
			try{
				...
				temp_store = "";
				Logging.getDebug("Column count is : "+numberOfColumns);
			...
			Logging.getError("Error in DynamicTableAction 2: "+ew);
			}
			finally{
				try{
					if(rst != null)
	...
	/*public void makeQuery(String sqlquery)
	{
			...
			Logging.getDebug("query in saving : "+sqlquery);
