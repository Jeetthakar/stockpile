...
import app.ActionForwardParameters;
import app.Connect;

import com.harrier.initializeation.ConnectInit;


public final class FixedIncomeDefineIndexAction extends Action {
	Logger Logging = Logger.getLogger(FixedIncomeDefineIndexAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		FixedIncomeDefineIndexForm pageForm = (FixedIncomeDefineIndexForm) form;
			...
			if (pageForm.getB_isIndexIsChildOrCustomized() != null
				...
				if (b) {
					...
					new ActionForward(response
							.encodeURL("/pages/fixedincome/FixedIncomeIndexHome.jsp?D1="
		...
		if (((pageForm.getB_isCaptured()).equals("n"))
			...
			if (pageForm.getS_indexType().equals("5")) {
				...
				if (errors.isEmpty()) {
					return (new ActionForward(response
							.encodeURL("/pages/fixedincome/FixedIncomeIndexHome?D1="
									+ pageForm.getI_indexID()
									+ "&B2=Compute&basedate="
									+ pageForm.getD_baseDate())));
				} else {
			...
			return (new ActionForward(response
					.encodeURL("/pages/fixedincome/FixedIncomeIndexComposition.jsp?from=create")));
		}
		return (mapping.getInputForward());
	}

	public boolean capturedIndex(FixedIncomeDefineIndexForm pageForm, ActionMapping mapping,
		...
		if (!pageForm.getB_isCaptured().equals("n")) {
			...
			try {
				...
				QueryClass.fixedIncomeInsertIntoIndexMaster(query, pageForm, use_user,
						user_id);
	...
	public boolean copyOfAnotherIndex(FixedIncomeDefineIndexForm pageForm,
		...
		if (pageForm.getB_isIndexIsChildOrCustomized() != null
			...
			QueryClass.fixedIncomeStoreCopyOfParent(query, pageForm.getS_parentIndex(),
	...
	public void createIndex(FixedIncomeDefineIndexForm pageForm, HttpServletRequest request,
			String use_user) {
		Properties prop = new Properties();
