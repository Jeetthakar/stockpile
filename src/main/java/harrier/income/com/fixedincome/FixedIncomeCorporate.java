...
public class FixedIncomeCorporate extends ActionForm{
	Logger Logging = Logger.getLogger(FixedIncomeCorporate.class);
	...
	/**
	 * @param bc_end The bc_end to set.
	 */
	...
	/**
	 * @param bc_start The bc_start to set.
	 */
	...
	/**
	 * @param nc_end The nc_end to set.
	 */
	...
	/**
	 * @param nc_start The nc_start to set.
	 */
	public void setNc_start(String nc_start) {
		this.nc_start = nc_start;
	}
	private String stid, ratio,ratio1,ratio2, corpid, percent="" , amt , newLTP,
			newTIS, adjust, affect,share,cad_id,leng,i_index,values,fdate=null,tdate,bc_start,amt_per,
			bc_end,nc_start,nc_end,ltp,description,symbol,series,newTmcv,event_id,button,corp_name,type,chk_but,r_type=null,c_Cad,c_Cad1,upflg,ncorp_button,compo_button,ind_comp,
			check_type,exc_stk,n_share,s_exc,time=null,s_stock=null,s_ratio1,s_ratio2,exist,
			new_corp_but=null,s_affectedIndex,id,ind_div,ftcurrency,currencyid,curr_val,mark_lot=null,
		...
		try{
			...
			if(hash.isEmpty())
			{
				...
				if(val!=0)
				{
					...
					{
						String stk=stock[i];
						hash.put(new Integer(stk).toString(),new String(corpid)); 																							
					}
				}
				setHash(hash);
				hash=getHash();
			   ...
			   for(Enumeration enum1 =hash.keys();enum1.hasMoreElements();)
					{
						...
						int cid= FixedIncomeListTypeClass1.check_corp_name1(rs2,corpid);						    
						rs2.close();								
						String query=ConnectInit.queries.getProperty("select_index_corpdiary");
						ResultSet rs= FixedIncomeListTypeClass1.check_dairy_exist(con,query,Integer.toString(cid),i_index,id2,apply_date);						
					if(rs!=null)
							flag=rs.next();
						if(flag==true)
						{
							  ...
							  while(rs.next())
							  {
								...
								sl=new stocklist(tid,rs.getString("index_name"),rs.getString("stock_name"),rs.getString("cam_name"),rs.getString("apply_on_date"),rs.getString("values"),rs.getString("status"));
								Pp.add(sl);	
							  }//while
						}//flag true
						else
						{
							query=ConnectInit.queries.getProperty("select_stock_corp_index");
							ResultSet rs1= FixedIncomeListTypeClass1.getresp_cad1(con,query,Integer.toString(cid),id2,i_index);
							rs1.next();			
							String tid="false:"+id2;
							sl=new stocklist(tid,rs1.getString("index_name"),rs1.getString("stock_name"),rs1.getString("cam_name"),"","","n");
							Pp.add(sl);
						}//flag false
					}//for end
		}catch(Exception e){
	...
	public ArrayList getAlllist() {
		...
		try{
			...
			{				
				ResultSet rs=FixedIncomeListTypeClass1.check_type_diary(this,fdate,tdate,r_type);
				while(rs!=null && rs.next())
				{
					String cid=rs.getString("cad_id");		
					String query1=null;
					query1=ConnectInit.queries.getProperty("cad_id_details");	
					ResultSet rs1= FixedIncomeListTypeClass1.getResult1(con,query1,cid);
					rs1.next();	
					sl=new stocklist(rs.getString("cad_id"),rs1.getString("cam_name"),rs1.getString("stock_ex_name"),rs1.getString("stock_name"),rs1.getString("apply_on_date"),rs1.getString("applied_date"),rs1.getString("amount"),rs1.getString("percentage"),rs1.getString("ratio_for_shares"),rs1.getString("ratio_shares_offered"),rs1.getString("values"),rs1.getString("status"));
					Pp.add(sl);	
				}	
			}
			if(r_type.equals("index event"))
			{
				...
				while(rs!=null && rs.next())
				{
					String cid=rs.getString("cad_id");				
					String query1=null;
					query1=ConnectInit.queries.getProperty("index_cad_details");	
					ResultSet rs1= FixedIncomeListTypeClass1.getResult1(con,query1,cid);
					rs1.next();
					String srtname=rs1.getString("cam_shortname").toLowerCase().trim();					
					sl=new stocklist(rs.getString("cad_id"),rs1.getString("cam_name"),rs1.getString("index_name"),rs1.getString("exchange_name"),rs1.getString("stock_name"),rs1.getString("apply_on_date"),rs1.getString("applied_date"),rs1.getString("base_date"),rs1.getString("values"),rs1.getString("status"));
					Pp.add(sl);
					hash.put(new String(cid),new String(srtname));
					if(rs1.getString("status").equals("y") & srtname.equals("change iwf"))
						hash2.put(new String(cid),new String(rs1.getString("values")));
			...
			if(r_type.equals("event"))
			{
				...
				while(rs!=null && rs.next())
				{
					...
					String query1=ConnectInit.queries.getProperty("select_stock_name");
					ResultSet rs1= FixedIncomeListTypeClass1.getAffected(con,query1,stock_id);
					rs1.next();
					String stock_name=rs1.getString("stock_name");
					String eid=rs1.getString("stock_exchange_id");
					rs1.close();
					query1=ConnectInit.queries.getProperty("get_stock_exchange_name");
					rs1= FixedIncomeListTypeClass1.getAffected(con,query1,eid);
					rs1.next();
					...
					sl=new stocklist(event,ex_name,stock_name,rs.getString("apply_on_date"),rs.getString("amount"),rs.getString("percentage"),rs.getString("ratio_for_shares"),rs.getString("ratio_shares_offered"),rs.getString("description"),rs.getString("values"),"n");
					Pp.add(sl);
				}
			}
...
//		AcessControl asc=new AcessControl();
		...
		Logging.debug(" Inside getAffindCollection(): Affected Child Indices ="+AffChildInd);
		try{
			int chk_dt= FixedIncomeComputeIndexForm.CompareDate(apply_date,UpdateCorp.accept_date());	//check for the current date and user's entered date
			Connect c = ConnectInit.getConnect();
			if(chk_dt==0)
				con = c.getdbConnection();
			else
					...
					Collections.sort(v);
					    ...
					    while(it.hasNext())
						{
							...
							String div[]=ActionCorp.token2(ele);
							 ...
							 String qry1=ConnectInit.queries.getProperty("index_name");
							 ResultSet rs= FixedIncomeListTypeClass1.getAffected(con,qry1,element);
							 rs.next();
							 String iname=rs.getString("index_name");
							 
							 if(affect!=null)
							 	if((affect.equals(""))|(affect.equals(AffChildInd)))
							 		affect=null;
							 if(affect==null)							 							 	
	...
	/**
	 * @param affindCollection The affindCollection to set.
	 */
...
//		Change by Manoj Adekar for Dynamic connection using getdbConnection() instead of  getConnection()
		try{
			...
			if(corpid!=null)			
				if((corpid.equals("Select Action"))|(corpid.equals("")))
					corpid=null;
		   String query =ConnectInit.queries.getProperty("get_corporate_list_index");   
		   ...
		   if(corpid==null)
			{
				while(rs.next())
				{
					...
					roles.add(new LabelValueBean(rs.getString("cam_name"),corp_val));
				}
			}
			else
			{
	...
	/**
	 * @param indcorpCollection The indcorpCollection to set.
	 */
	...
	/**
	 * @param newmcv The newmcv to set.
	 */
	...
	private String tmcv, divisor, indexval, newtmcv, newdivisor,tmcv1,divisor1,newtmcv1,newdivisor1;
	...
	private int val;
	private String apply_date,ex_date,record_date,announce_date,applied_date,status,newFace;
	...
	/**
	 * @param newFace The newFace to set.
	 */
	public void setNewFace(String newFace) {
		this.newFace = newFace;
	}
	private String i_name="",i_cdate="",i_basedate="",i_basevalue="",m_start_time="",n_stop_time="",
	compute_interval="",is_capture="",cap_from="",o_ric="",cancel,close;
	private String is_child="",parent="",currency="",is_customized="",industry_class_code="",industry_class="",b_natureindex="",cur;
	...
	public void setResult2() {
		...
		if(con == null){
         ...
         ResultSet rs =  FixedIncomeListTypeClass1.getResult12(con,query12,i_index);         
		boolean flag=true;
		try {
			if (rs != null)
				flag=rs.next();
		...
		try {
			...
			if (rs.getString(15) != null) {
				industry_class_code = rs.getString("industry_classification_code");
			...
			if (rs.getString(19) != null) {
				industry_class = rs.getString("industry_classification_name");
	...
	/**
	 * @param newtmcv The newtmcv to set.
	 */
	...
	/**
	 * @param ratio1 The ratio1 to set.
	 */
	...
	/**
	 * @param ratio2 The ratio2 to set.
	 */
	...
	/**
	 * @param cad_id The cad_id to set.
	 */
	...
	/**
	 * @param hash12 The hash12 to set.
	 */
	...
	/**
	 * @param announce_date The announce_date to set.
	 */
	...
	/**
	 * @param i_index The i_index to set.
	 */
	...
	/**
	 * @param is_customized The is_customized to set.
	 */
	...
	/**
	 * @param b_natureindex The b_natureindex to set.
	 */
	...
	/**
	 * @param cap_from The cap_from to set.
	 */
	...
	/**
	 * @param compute_interval The compute_interval to set.
	 */
	...
	/**
	 * @param i_basedate The i_basedate to set.
	 */
	...
	/**
	 * @param i_basevalue The i_basevalue to set.
	 */
	...
	/**
	 * @param i_cdate The i_cdate to set.
	 */
	...
	/**
	 * @param industry_class The industry_class to set.
	 */
	...
	/**
	 * @param industry_class_code The industry_class_code to set.
	 */
	...
	/**
	 * @param is_capture The is_capture to set.
	 */
	...
	public void reset_indevent()
	{
		...
		ResultSet rs1= FixedIncomeListTypeClass1.resultCorporate(con,qry);
		rs1.next();
	...
	public void selectdate()
	{
		...
		try{
			...
			String tdate=Integer.toString(ds.getDate())+"-"+mnt+"-"+yer;			
			setFdate(tdate);
			...
			String tdate1=Integer.toString(ds1.getDate())+"-"+mnt1+"-"+yer1;
			setTdate(tdate1);		
	...
	public int check_stkstatus(Connection con,Connect connect,String stock,String date)
	{
		int merge_chk=-1;
		try{
			String stk_qry=ConnectInit.queries.getProperty("check_stock_status");								
			ResultSet rs= FixedIncomeListTypeClass1.getAffected(con,stk_qry,stock);
			if(rs.next())
			{
				String mdate=rs.getString("applied_date");
				merge_chk= FixedIncomeComputeIndexForm.CompareDate(date,mdate);
			}
		}catch(Exception e){
	...
	/**
	 * @param hash_error The hash_error to set.
	 */
	...
	/**
	 * @param hash_affind The hash_affind to set.
	 */
	...
	/**
	 * @param index_type_name The index_type_name to set.
	 */
		...
		try{
			...
			{		
				...
				if(button.equals("Action"))
				{
					ind_comp=null;
					errors.clear();
					if((i_index.equals(""))|(i_index.equals("Select Index")))				
						errors.add("i_index",new ActionError("Error.message.Index"));	
				}
				if(button.equals("Commit"))
				{
					...
					if(butt==null)
					{
						...
						{
							String qry=ConnectInit.queries.getProperty("get_corporate_list_index");
							int cid=0;							
							ResultSet rs= FixedIncomeListTypeClass1.resultCorporate(con,qry);
							cid= FixedIncomeListTypeClass1.check_corp_name1(rs,request.getParameter("corpid"));
							rs.close();		
								
							String query=ConnectInit.queries.getProperty("check_diarycurr_exist");
							String date=UpdateCorp.accept_date();
							rs= FixedIncomeListTypeClass1.getresp_cad1(con,query,getI_index(),Integer.toString(cid),date);
							if(rs.next())
							{
								errors.add("Values",new ActionError("Error.message.ActionApplied"));								
							}
						}
						else
						{
							...
							int cid=0;				
								...
								String query=ConnectInit.queries.getProperty("check_index_action");
								String date=UpdateCorp.accept_date();
								String stk_val[]=request.getParameterValues("stock");								
								//if(stk_val==null){
								//	errors.add("Values",new ActionError("Error.message.ActionApplied"));
								//}else{
									...
									for(int i=0;i<l1;i++)
									{
										...
										rs= FixedIncomeListTypeClass1.check_dairy_exist(con,query,Integer.toString(cid),i_index,sid,date);
					...
					else
					{
						...
						{		
							...
							try{
								...
								ResultSet rs= FixedIncomeListTypeClass1.getAffected(con,query,getC_Cad());
								rs.next();
				...
				if(button.equals("Apply"))
				{
					...
					if(butt==null)
					{
						...
						if((apply_date==null)|(apply_date.equals("")))
							errors.add("i_index",new ActionError("Error.message.ApplyDate"));
						else
						{	
							String dt=UpdateCorp.accept_date();   //get the current date
							String apply=apply_date;
							chk_dt= FixedIncomeComputeIndexForm.CompareDate(apply_date,dt);//check for the current date and user's entered date
							if(chk_dt > 0)
							{
								flg=1;
								errors.add("chk",new ActionError("Error.message.GreaterDate"));
							}
						}
						if(flg==0)
						{
							...
							{
								flg=1;
								errors.add("i_index",new ActionError("Error.message.CorpAction"));
							}
							else
							{
								...
								int type_id=0;
								try{							
									String query=ConnectInit.queries.getProperty("select_index_type");
									ResultSet rs= FixedIncomeListTypeClass1.getResult1(con,query,i_index);
									if(rs!=null && rs.next())
									type_id=rs.getInt("index_type_id");
								}catch(Exception e){Logging.error("error="+e.getMessage());}
								if((type_id==1 & corp_val.equals("changeiwf"))|(type_id==3 & corp_val.equals("changeiwf"))|(type_id==4 &((corp_val.equals("changeiwf"))|(corp_val.equals("addstock"))|(corp_val.equals("deletestock"))))|(type_id==5 & ((corp_val.equals("addstock"))|(corp_val.equals("deletestock"))|(corp_val.equals("changeiwf"))|(corp_val.equals("rebasing")))))
								{
									flg=1;
									errors.add("chk_but",new ActionError("Error.message.TypeCheck"));
								}
							}	
						}						
						if(flg==0)
						{
							if((corpid.equals("addstock"))|(corpid.equals("deletestock"))|(corpid.equals("changeiwf")))
								{
									String stock=request.getParameter("stock");									
									if(stock==null)
									{
										flg=1;
										errors.add("stock",new ActionError("Error.message.Stock"));
									}
								...
								{								
									...
									{
										flg=1;
										errors.add("Values",new ActionError("Error.message.Value"));
									}
									else
									{
								...
								if(flg==0)
								{
									if(chk_dt!=0)
									{
										if(!(corpid.equals("changeindcurr")))
										{
											...
											if(len > 1)
											{
												errors.add("Values",new ActionError("Error.message.Histmultiple"));
											}
										}
									}
								}
								if(flg==0)
								{
									if(chk_dt!=0)
									{
										...
										{
											String qry=ConnectInit.queries.getProperty("check_hist_add_del");
											String cam_name="delete stock";
											String stk=stock[0];
											ResultSet rs=ListTypeClass1.check_dairy_exist(con,qry,i_index,stk,apply_date,cam_name);
											boolean chk_rs=rs.next();
											if(chk_rs==true)
											{
												flg=1;
												errors.add("chk",new ActionError("Error.message.Histadd"));
											}												
										}
										if(corpid.equals("deletestock"))
										{
											...
											String stk=stock[0];
											ResultSet rs= FixedIncomeListTypeClass1.check_dairy_exist(con,qry,i_index,stk,apply_date,cam_name);
											boolean chk_rs=rs.next();
											if(chk_rs==true)
											{
												flg=1;
												errors.add("chk",new ActionError("Error.message.Histdel"));
											}
										}
										if(corpid.equals("changeiwf"))
										{
											String qry=ConnectInit.queries.getProperty("check_hist_iwf");
											String stk=stock[0];
											ResultSet rs= FixedIncomeListTypeClass1.getresp_cad(con,qry,i_index,stk,apply_date);
											rs.next();
											String ca_name=rs.getString("cam_shortname");
											if(ca_name.equals("add stock"))
											{
												flg=1;
												errors.add("chk",new ActionError("Error.message.Histiwf"));
											}
										}
									}
								}							
								if(corpid.equals("changeindcurr"))
								{
									if(ftcurrency.equals("")| ftcurrency.equals("Select Currency"))
									{
										flg=1;
										errors.add("Values",new ActionError("Error.message.Currency"));
									}									
									if(ind_div==null)
									{
										flg=1;
										errors.add("Values",new ActionError("Error.message.Radio"));
									}
									if(flg==0)
									{
										String qry=ConnectInit.queries.getProperty("get_corporate_list_index");
									...
									if(flg==0)
									{
										String query=ConnectInit.queries.getProperty("resp_child_indcurr");
										 FixedIncomeListTypeClass1.affect_child_list(con,this,query);
										ActionCorp.check_curr(this);
										if(isCheck_curr()==true)
										{
											flg=1;
											errors.add("Values",new ActionError("Error.message.Currerror"));
										}
									}
									if(flg==0)
									{
										if(chk_dt==0)
										{
											...
											{
												String query=ConnectInit.queries.getProperty("select_index_detail");
												ResultSet rs=ListTypeClass1.getResult12(con,query,getI_index());
												if(rs.next())
												{
													...
													{
														flg=1;
														errors.add("chk",new ActionError("Error.message.IndexCloseParent"));
													}
													else
													{
														if(close.equals("0"))
												...
												ActionCorp.check_affect_index(con,this,query);
												if(isCheck()==false)
												{
													flg=1;
													errors.add("chk",new ActionError("Error.message.Childerr"));
												}
											}
										}
									}
								...
								if(flg==0 & (!corpid.equals("changeindcurr")))
								{
									String qry=ConnectInit.queries.getProperty("get_corporate_list_index");
									int cid=0;							
										ResultSet rs=ListTypeClass1.resultCorporate(con,qry);
										cid=ListTypeClass1.check_corp_name1(rs,corpid);
										if(rs!=null)
											rs.close();
										String query=ConnectInit.queries.getProperty("check_index_action");
										String date=UpdateCorp.accept_date();
										...
										if(stk_val!=null){
											...
											{
												String sid=stk_val[i];
												rs=ListTypeClass1.check_dairy_exist(con,query,Integer.toString(cid),i_index,sid,date);
												if(rs.next())
												{
												//	flg=1;
													//errors.add("Values",new ActionError("Error.message.ActionApplied"));
...
//								check stock status //ok
								String stk_val[]=(String[])request.getParameterValues("stock"); //here is the problem
								if(stk_val!=null){
									int l1=stk_val.length;
									for(int i=0;i<l1;i++)
									{
										String sid=stk_val[i];								
										int merge_chk=check_stkstatus(con,connect,sid,apply_date);
										Logging.debug("merger chk ==="+merge_chk);
										if(merge_chk > 0 || merge_chk==0)
										{
											flg=1;
											errors.add("chk",new ActionError("Error.message.HistStkStatus"));
											break;
										}
									} 	
								}
			...
			{				
				if(chk_but==null)
					errors.add("chk_but",new ActionError("Error.message.Check"));
				else
				{
					...
					if(corp_id!=null)
					{
						...
						{
							String chkbut[]=request.getParameterValues("chk_but");
							int l1=chkbut.length;
							if(l1 > 1)
							{								
								errors.add("chk_but",new ActionError("Error.message.MultipleSelect"));
							}
						}
					}
				}
			...
			if(new_ind_but!=null)
			{
				if(new_ind_but.equals("Action"))
				{
					...
					try{
						...
						String query=ConnectInit.queries.getProperty("select_index_type");
						ResultSet rs=ListTypeClass1.getResult1(con,query,i_index);
						rs.next();
						type_id=rs.getInt("index_type_id");
					}catch(Exception e){Logging.error("error="+e.getMessage());}
					if((type_id==1 & corp_val.equals("changeiwf"))|(type_id==3 & corp_val.equals("changeiwf"))|(type_id==4 &((corp_val.equals("changeiwf"))|(corp_val.equals("addstock"))|(corp_val.equals("deletestock"))))|(type_id==5 & ((corp_val.equals("addstock"))|(corp_val.equals("deletestock"))|(corp_val.equals("changeiwf"))|(corp_val.equals("rebasing")))))
					...
					if(corp_val.equals("changeindcurr"))
						errors.add("chk_but",new ActionError("Error.message.NewIndCur"));
					
				}
				if(new_ind_but.equals("Submit"))
				{					
					if(i_index!=null)
						if((i_index.equals("Select Index"))|(i_index.equals("")))
							errors.add("i_index",new ActionError("Error.message.Index"));
					if(corpid==null)
						errors.add("corpid",new ActionError("Error.message.CorpAction"));
					if(corpid!=null)
					{
						if((corpid.equals("Select Action"))|(corpid.equals("")))
						{
						...
						if((corpid.equals("addstock"))|(corpid.equals("changeiwf"))|(corpid.equals("deletestock")))
						{
							String stock=request.getParameter("stock");
							if(stock==null)
								errors.add("stock",new ActionError("Error.message.Stock"));
						...
						if(corpid.equals("changeiwf"))
						{
							...
							{
								if(val.equals(""))
									errors.add("val",new ActionError("Error.message.Value"));								
								else
								{
					...
					{						
						String query=ConnectInit.queries.getProperty("check_corp_adddel_exist");
						int len=stock.length;		
						hash=getHash();
						hash.clear();
						setHash(hash);
						...
						for(int i=0;i<len;i++)
						{
							...
							try{
								rs= FixedIncomeListTypeClass1.getresp_cad(con,query,Integer.toString(cid),getI_index(),stk);
								if(rs.next())
								{	
									flag=1;
								}
						...
						if(flag==1)
						{							
							errors.add("val",new ActionError("Error.message.AlreadyStock"));
						}
					}			
					}catch(Exception e){Logging.error("error=="+e.getMessage());}
//					check stock status
					...
					for(int i=0;i<len;i++)
					{						
						int merge_chk=check_stkstatus(con,connect,stk[i],apply_date);
					    if(merge_chk > 0 || merge_chk==0)
						{						
							errors.add("chk",new ActionError("Error.message.HistStkStatus"));
							break;
			...
			if(ap_co_but2!=null)
			{
				if(ap_co_but2.equals("YDelete"))
				{
					...
					if(chk==null)
						errors.add("chk",new ActionError("Error.message.Check"));
				}
				if(ap_co_but2.equals("Update"))
				{	
					if(hash.isEmpty())
						errors.add("hash",new ActionError("Error.message.Hash"));
					else
					{
					...
					{	
						...
						else
						{
							...
							if(len==1)
							{
								...
								ResultSet rs= FixedIncomeListTypeClass1.getAffected(con,query,cid);
								rs.next();
								String apply_date=rs.getString("apply_on_date");
								String stock=rs.getString("stock_id");
								rs.close();
//								check stock status
								int merge_chk=check_stkstatus(con,connect,stock,apply_date);
								Logging.debug("merger chk==="+merge_chk);
							    if(merge_chk > 0 || merge_chk==0)
									errors.add("chk",new ActionError("Error.message.HistStkStatus"));								
							}
							...
							{		
								...
								Logging.debug("cad id in for==="+chk[i]);
								String query1=ConnectInit.queries.getProperty("check_applyon_date");
								String get_name=ConnectInit.queries.getProperty("get_corp_name");						
								String check_compo=ConnectInit.queries.getProperty("index_comp_detail");
								
								ResultSet rs= FixedIncomeListTypeClass1.getAffected(con,query,chk[i]);
								rs.next();
								String apply_date=rs.getString("apply_on_date");
								String stock=rs.getString("stock_id");
								String corp=rs.getString("cam_id");
								int iid=rs.getInt("index_id");
								ResultSet rs1= FixedIncomeListTypeClass1.getAffected(con,get_name,corp);
								 rs1.next();
								 String corp_name=rs1.getString("cam_shortname").toLowerCase().trim();
								 rs1.close();
								...
								{
									if(!(chk_apply.equals(apply_date)))
									{										
										errors.add("date",new ActionError("Error.message.DifferentApply"));
										break;
									}
								}
								Logging.debug("stock in for="+stock+"aappl="+apply_date);
//								check stock status
								int merge_chk=check_stkstatus(con,connect,stock,apply_date);
								Logging.debug("merger chk==="+merge_chk);
							    if(merge_chk > 0 || merge_chk==0)
								{									
								 ...
								 {
								 	rs1= FixedIncomeListTypeClass1.getResult_corp(con,check_compo,Integer.toString(iid),stock);
								 	if(rs1.next())
								 		errors.add("chk",new ActionError("Error.message.Alreadyadd"));	
								 }
								 if((corp_name.equals("delete stock"))|(corp_name.equals("change iwf")))
								 {
								 	rs1= FixedIncomeListTypeClass1.getResult_corp(con,check_compo,Integer.toString(iid),stock);
								 	if(!rs1.next())
								 		errors.add("chk",new ActionError("Error.message.Alreadydelete"));
								 }
								 String date1=UpdateCorp.accept_date();
								 if(corp_name.equals("change iwf"))
								 {
								 	String qry=ConnectInit.queries.getProperty("check_index_action");
									rs= FixedIncomeListTypeClass1.check_dairy_exist(con,qry,corp,Integer.toString(iid),stock,date1);
									if(rs.next())
										errors.add("chk",new ActionError("Error.message.ActionApplied"));
								 }												 
										    
								
								int chk_dt= FixedIncomeComputeIndexForm.CompareDate(apply_date,dt);	//check for the current date and user's entered date
								if(chk_dt > 0)
								{
									errors.add("chk",new ActionError("Error.message.GreaterDate"));	
									break;
								...
								if(chk_dt < 0)
								{
									if(len > 1)
									{
										errors.add("chk",new ActionError("Error.message.Histmultiple"));	
										break;
									}
								...
								{ 
									 ...
									 else
									 {
									 	if(((itemp1.equals("add stock"))|(itemp1.equals("delete stock"))|(itemp1.equals("change iwf")))& corp_name.equals("rebasing"))
									 		errors.add("chk",new ActionError("Error.message.CombCorp"));
									 	if(((itemp1.equals("rebasing"))&((corp_name.equals("add stock"))|(corp_name.equals("delete stock"))|(corp_name.equals("change iwf")))))
									 		errors.add("chk",new ActionError("Error.message.CombCorp"));
									 }									 
									 setI_index(rs1.getString("index_id"));
									 if(index==0)
									 	index=iid;
									 else
									  	if(index!=iid)
									 		errors.add("chk",new ActionError("Error.message.SameIndex"));
								
									if(len!=1)
									 	if(itemp1.equals(corp_name))
									 		if(itemp1.equals("rebasing"))
									 			errors.add("chk",new ActionError("Error.message.MultipleSelect"));
								}
								}catch(Exception e){
									Logging.debug("Error="+e.getMessage());
									}
...
//								check stock status
								...
								ResultSet rs= FixedIncomeListTypeClass1.getAffected(con,query,cad);
								rs.next();
								int merge_chk=check_stkstatus(con,connect,rs.getString("stock_id"),rs.getString("applied_date"));
								rs.close();
							    if(merge_chk > 0 || merge_chk==0)
								{									
									errors.add("chk",new ActionError("Error.message.HistStkStatus"));
			...
			if(ind_comp_but!=null)
			{
				...
				if(ind_comp_but.equals("Commit"))
				{
					...
					if(!(hash3.isEmpty()))
					{
						 for(Enumeration enum1 =hash3.keys();enum1.hasMoreElements();)
						 {
						 	...
						 	rs.close();
						 	query=ConnectInit.queries.getProperty("check_index_action");
							String date=apply_date;//UpdateCorp.accept_date();
							rs= FixedIncomeListTypeClass1.check_dairy_exist(con,query,camid,index_id,id,date);
						 	if(rs.next())
						 	{
						 		flag=1;
						 		errors.add("chk",new ActionError("Error.message.ActionApplied"));						 		
					...
					if(flag==0)
					{
						if(!(hash4.isEmpty()))
						{
							 for(Enumeration enum1 =hash4.keys();enum1.hasMoreElements();)
							 {
							 	String id=(String)enum1.nextElement();
				...
				if(ind_comp_but.equals("Apply"))
				{
					...
					if(chk_dt!=0)
					{
						...
						for(Enumeration enum1 =data.keys();enum1.hasMoreElements();)
						{
							...
							String qry=ConnectInit.queries.getProperty("check_hist_add_del");
							String cam_name="add stock";
							ResultSet rs= FixedIncomeListTypeClass1.check_dairy_exist(con,qry,i_index,id2,apply_date,cam_name);
							boolean chk_rs=rs.next();
							if(chk_rs==true)
							{
								errors.add("chk",new ActionError("Error.message.Histdel"));
			...
			if(new_corp!=null)
			{
				if((new_corp.equals("Submit"))|(new_corp.equals("NewSubmit")))
				{
					int flg=0;					
					if((exc.equals("Select Exchange"))|(exc.equals("")))
					{
						flg=1;
						errors.add("exc",new ActionError("Error.message.StockExchName"));
					}	
					if((s_stock.equals("Select Stock"))|(s_stock.equals("")))
					{
						flg=1;
						errors.add("exc",new ActionError("Error.message.Stock"));
					}
					if((corpid.equals("Select Action"))|(corpid.equals("")))
					{
						flg=1;
						errors.add("exc",new ActionError("Error.message.CorpAction"));
					}
					else
					{
						...
						if((corpid.equals("stockdividend/bonus"))|(corpid.equals("split"))|(corpid.equals("reversesplit"))|(corpid.equals("rightsoffering"))|(corpid.equals("warrantconversion")))
							if((r1.equals(""))|(r2.equals("")))
							{
								flg=1;
								errors.add("r1",new ActionError("Error.message.Ratio"));
							}
						if((corpid.equals("cashdividend"))|(corpid.equals("rightsoffering"))|(corpid.equals("sharesbuyback"))|(corpid.equals("spin-off"))|(corpid.equals("specialdividend")))
						{							
							if(amt_per==null)
							{
								flg=1;
								errors.add("amt_per",new ActionError("Error.message.Radio"));
							}
							if(amt.equals(""))
							{
								flg=1;
								errors.add("amt",new ActionError("Error.message.Amount"));
							}
							else
							{
								...
								{
									flg=1;
									errors.add("amt",new ActionError("Error.message.NegativeAmount"));
								}
							}
						}						
						if(corpid.equals("sharesbuyback")|(corpid.equals("warrantconversion"))|(corpid.equals("shareissuance"))|(corpid.equals("capitalreduce"))|(corpid.equals("adrissue")))
							if(share.equals(""))
							{
								flg=1;
								errors.add("share",new ActionError("Error.message.Share"));
							}
							else
							{
								...
								{
									flg=1;
									errors.add("share",new ActionError("Error.message.NegativeShare"));
								}
							}
					}
					if(flg==0)
					{							
						//check stock status
						int merge_chk=check_stkstatus(con,connect,getS_stock(),apply_date);							
						if(merge_chk > 0 || merge_chk==0)
						{
							flg=1;
							errors.add("chk",new ActionError("Error.message.HistStkStatus"));
					...
					if(flg==0)
					{
						...
						{							
							if(r_val1 > r_val2)		
							{
								errors.add("share",new ActionError("Error.message.Ratiosplit"));
							}
						}
						if(corpid.equals("reversesplit"))
						{
							if(r_val2 > r_val1)
							{
								errors.add("share",new ActionError("Error.message.Ratiorevsplit"));
							}
						}
					}
				}//submit or newsubmit
			...
			if(ap_but1!=null)
			{
				if(ap_but1.equals("Update"))
				{
					...
					else
					{
						...
						int len=str.length;
						if(len > 1)
							errors.add("chk",new ActionError("Error.message.Update"));
					}
				}
					...
					{						
						...
						if(butt!=null)
							...
							else
							{
								...
								{								
									String query=ConnectInit.queries.getProperty("select_rep_cad");																
									ResultSet rs= FixedIncomeListTypeClass1.getAffected(con,query,str[i]);
									rs.next();
									String apply_date=rs.getString("apply_on_date");
									String stock=rs.getString("stock_id");
									...
									ResultSet rs1=null;
									String query1=ConnectInit.queries.getProperty("check_applyon_date");
									rs1= FixedIncomeListTypeClass1.getResult_apply(con,query1,str[i],date1);
									...
									{									
										if(temp==null & temp1==null & apply==null)
										{
											temp=cam;
											temp1=sid;
											apply=apply_date;
										...
										{
											if((!(temp.equals(cam))) & (!(temp1.equals(sid))))
											{
												flg=1;
												errors.add("chk",new ActionError("Error.message.Corp1"));
											}
											else
											{
												if(!(temp.equals(cam)))
												{
													flg=1;
													errors.add("chk",new ActionError("Error.message.Corp2"));
												}
												else
												{	
													if(!(temp1.equals(sid)))
													{
														flg=1;
														errors.add("chk",new ActionError("Error.message.Corp3"));
													}
												}
											}
											if(flg==0)
											{
												if(!(apply.equals(apply_date)))
												{
													flg=1;
													errors.add("chk",new ActionError("Error.message.DifferentApply"));													
												}
											}
											if(flg==0)
											{			
								...
								if(len==1)
								{
									String str1=request.getParameter("c_Cad");
									String query=ConnectInit.queries.getProperty("select_rep_cad");																
									ResultSet rs= FixedIncomeListTypeClass1.getAffected(con,query,str1);
									rs.next();
									String apply_date=rs.getString("apply_on_date");
									String stock=rs.getString("stock_id");
									rs.close();									
									//check stock status
									int merge_chk=check_stkstatus(con,connect,stock,apply_date);
									Logging.debug("merge_chk==="+merge_chk);														
									if(merge_chk > 0 || merge_chk==0)
									{
										flg=1;
										errors.add("chk",new ActionError("Error.message.HistStkStatus"));
								...
								if(len > 1)
								{
									String cam_name=ActionCorp.getcorpname(con,get_name,cam);
									cam_name=cam_name.toLowerCase().trim();
									if((cam_name.equals("spin-off"))|(cam_name.equals("stock dividend/bonus"))|(cam_name.equals("special dividend"))|(cam_name.equals("split"))|(cam_name.equals("reverse split"))|(cam_name.equals("rights offering"))|(cam_name.equals("warrant conversion")))
										errors.add("chk",new ActionError("Error.message.MultipleSelect"));
								}
							}
							}
							if(butt!=null)
							{
								...
								else
								{
									...
									{
										String str1=request.getParameter("c_Cad");																				
										String query=ConnectInit.queries.getProperty("undo_merger");
										ResultSet rs= FixedIncomeListTypeClass1.getAffected(con,query,str1);
										if(rs.next())
											errors.add("chk",new ActionError("Error.message.UndoMerge"));										
									}								
			...
			{				
				if(ncorp.equals("Apply"))
				{
					...
					{						
						...
						boolean chk_rs=rs.next();
						if(chk_rs==false)
							errors.add("chk",new ActionError("Error.message.StockCloseDate"));
						else
						{
							...
							else
							{
								...
								if(corp.equals("sharesbuyback")|(corp.equals("warrantconversion"))|(corp.equals("shareissuance"))|(corp.equals("capitalreduce"))|(corp.equals("adrissue")))
								{
									...
									{
										flg=1;
										errors.add("chk",new ActionError("Error.message.NegativeShare"));
									}
									else
									{
										if((corp.equals("sharesbuyback"))|(corp.equals("capitalreduce")))
										{
											float shr=Float.parseFloat(share);
											String query=ConnectInit.queries.getProperty("detail_stock_master");
											ResultSet rs1= FixedIncomeListTypeClass1.getAffected(con,query,s_stock);
											rs1.next();
											float tis=rs1.getFloat("tis");											
											if(shr >= tis)
											{
												flg=1;												
												errors.add("share",new ActionError("Error.message.Greatershare"));												
											}
										}
									}
								}
								if((corp.equals("cashdividend"))|(corp.equals("rightsoffering"))|(corp.equals("sharesbuyback"))|(corp.equals("spin-off"))|(corp.equals("specialdividend")))
								{
									String amt=request.getParameter("amt");//request.getParameter("amount");
									setAmt(amt);
									boolean chk_amt=amt.startsWith("-");
									if(chk_amt==true)
									{
										flg=1;
										errors.add("share",new ActionError("Error.message.NegativeAmount"));
									}
									else
									{
										...
										{
											flg=1;
											errors.add("share",new ActionError("Error.message.Greateramount"));
										}
									}
								}
							  }	
							...
							{
								String corp=request.getParameter("corpid");
								qry=ConnectInit.queries.getProperty("get_corporate_list_stock");
								int cid=0;							
								rs= FixedIncomeListTypeClass1.resultCorporate(con,qry);
								cid= FixedIncomeListTypeClass1.check_corp_name1(rs,corp);
								rs.close();							
								String query=ConnectInit.queries.getProperty("check_action");
								String apply=UpdateCorp.accept_date();
								String stock=request.getParameter("s_stock");
								ResultSet rs1= FixedIncomeListTypeClass1.getresp_cad(con,query,Integer.toString(cid),stock,apply);
								if(rs1.next())
								{
									setAmt(request.getParameter("amt"));
									setShare(request.getParameter("share"));
									setRatio1(request.getParameter("ratio1"));
									setRatio2(request.getParameter("ratio2"));
									errors.add("chk",new ActionError("Error.message.ActionApplied"));
								}						
							}
						}
						...
						{														
							...
							boolean chk_rs=rs.next();
							if(chk_rs==false)
								errors.add("chk",new ActionError("Error.message.StockClose"));
							else
							{
								String adj=rs.getString("adjusted_price");								
								if(adj==null)
				...
				if(ncorp.equals("NApply"))
				{
					...
					if(getExc().equals("Select Exchange"))
						errors.add("chk",new ActionError("Error.message.SelectStockExc"));					
					if((apply_date.equals(""))|(apply_date==null))
						errors.add("chk",new ActionError("Error.message.ApplyDate"));
					String stock=request.getParameter("s_stock");					
					if((stock.equals("Select Stock"))|(stock.equals("")))								
						errors.add("chk",new ActionError("Error.message.Stock"));
					else
					{						
						if(apply_date!=null)
						{
							...
							int ap_chk= FixedIncomeComputeIndexForm.CompareDate(apply_date,dt);							
							...
							if(flg==0)
							{
								...
								if(ap_chk < 0)
								{
									...
									Logging.debug("close in corp-=="+getClose());
								}
								if(ap_chk==0)
								{									
									if(flg_val==0)
									{
										String qry=ConnectInit.queries.getProperty("get_latest_stock_closing_value_tis");
										PreparedStatement  stmt = con.prepareStatement(qry);
										stmt.setString(1,stock);
					...
					if((corp.equals("Select Action"))|(corp.equals("")))
						errors.add("chk",new ActionError("Error.message.CorpAction"));
					else
					{
						if((corp.equals("stockdividend/bonus"))|(corp.equals("split"))|(corp.equals("reversesplit"))|(corp.equals("rightsoffering"))|(corp.equals("warrantconversion"))|(corp.equals("specialdividend")))
						{
						...
						if(corp.equals("sharesbuyback")|(corp.equals("warrantconversion"))|(corp.equals("shareissuance"))|(corp.equals("capitalreduce"))|(corp.equals("adrissue")))
						{
							...
							else
							{
								...
								{
										flg=1;
										errors.add("amt",new ActionError("Error.message.NegativeShare"));
						...
						if(flg==0)
						{
							...
							if(corp.equals("split"))
							{
								...
								{
									flg=1;
									errors.add("ratio",new ActionError("Error.message.Ratiosplit"));
								}
							}
							if(corp.equals("reversesplit"))
							{
								...
								{
									flg=1;
									errors.add("ratio",new ActionError("Error.message.Ratiorevsplit"));
								}
							}
						}
						if(flg==0)
						{
							...
							String qry=ConnectInit.queries.getProperty("get_corporate_list_stock");
							int cid=0;	
							try{
							ResultSet rs= FixedIncomeListTypeClass1.resultCorporate(con,qry);
							cid= FixedIncomeListTypeClass1.check_corp_name1(rs,corp);
							...
							ResultSet rs1= FixedIncomeListTypeClass1.getresp_cad(con,query,Integer.toString(cid),stock,apply_date);
							if(rs1.next())
								errors.add("chk",new ActionError("Error.message.ActionApplied"));
							}catch(Exception e){Logging.error("error="+e.getMessage());}
						}
				...
				if(ncorp.equals("Commit"))
				{
					...
					{	
						if(butt==null)
						{
							...
							{
								String qry=ConnectInit.queries.getProperty("get_latest_index_value_index_wise");
								try{
									for(Enumeration enumm=hash.keys();enumm.hasMoreElements();)
									{
										String id=(String)enumm.nextElement();
										ResultSet rs= FixedIncomeListTypeClass1.getResult12(con,qry,id);
										if(rs!=null && rs.next()){ 
											String ind_val=rs.getString("index_closing_value");
											if(ind_val==null)
											{
												errors.add("chk",new ActionError("Error.message.AffectIndex"));
												flg_val=1;
												break;
											}
											else
					...
					else
					{
						if(hash.isEmpty()==false)
						{
							...
							try{
								...
								{
									String id=(String)enumm.nextElement();
									ResultSet rs= FixedIncomeListTypeClass1.getResult_apply(con,qry,id,apply_date);
									boolean chk_rs=rs.next();
									if(chk_rs==true)
									{
										String ind_val=rs.getString("index_closing_value");
			...
			if(ebutton!=null)
			{
				...
				{
					if(c_Cad==null)
						errors.add("chk",new ActionError("Error.message.Radio"));
				}
			...
			if(event_but!=null)
			{
				if(event_but.equals("Amt"))
				{
					...
					if(corp!=null)
					{
						if((corp.equals("Select Action"))|(corp.equals(""))){					
							...
							{
								percent="";	
								errors.add("chk",new ActionError("Error.message.Amount"));
							}
						}
						else
						{
				...
				if(event_but.equals("Submit"))
				{
					...
					if((corp.equals("stockdividend/bonus"))|(corp.equals("split"))|(corp.equals("reversesplit"))|(corp.equals("rightsoffering"))|(corp.equals("warrantconversion")))
						if((r1.equals(""))|(r2.equals("")))
							errors.add("r1",new ActionError("Error.message.Ratio"));
					if((corp.equals("cashdividend"))|(corp.equals("rightsoffering"))|(corp.equals("sharesbuyback"))|(corp.equals("spin-off"))|(corp.equals("specialdividend")))
					{
					...
					if(ap_date!=null)
						if(ap_date.equals(""))
							errors.add("chk",new ActionError("Error.message.ApplyDate"));
					//check stock status
					int merge_chk=check_stkstatus(con,connect,getStid(),ap_date);
				    if(merge_chk > 0 || merge_chk==0)
						errors.add("chk",new ActionError("Error.message.HistStkStatus"));						
				}				
			}	
			...
			if(radbutton!=null & new_ind_but==null)
			{
				...
				if(chk_dt==0)
				{
					...
					{					
						String query=ConnectInit.queries.getProperty("select_index_detail");
						//added by sonali to get the closing value of affected indices
						...
						while(it.hasNext()){
							...
							ResultSet rs=FixedIncomeListTypeClass1.getResult12(con,query,indexVal);
							if(rs!=null && rs.next())
							{
								String close=rs.getString("index_closing_value");
								if(close==null)
								...
								else
								{
									if(close.equals("0"))
									{
										...
										i++;
										closingFlag=1;
										//errors.add("chk",new ActionError("Error.message.IndexCloseParent"+noClosing));
									}
								}
							}
							else
							...
							if(hash_affind.isEmpty()==false)
							{
								if(isCheck()==false)
								{
									...
									i++;
									closingFlag=1;
									//errors.add("chk",new ActionError("Error.message.IndexCloseChild"));
								}
							}
						}
						if(closingFlag==1){
				...
				{
					String query=ConnectInit.queries.getProperty("Historic_compo_parent");
					ResultSet rs=FixedIncomeListTypeClass1.getResult_apply(con,query,i_index,apply_date);
					if(!(rs.next()))
					{
						ind_comp=null;
						errors.add("chk",new ActionError("Error.message.HistindComp"));
					}
				}
			}
			...
			if(detail_button!=null)
			{
				...
				if(detail_button.equals("Radio"))
				{
					if(nature!=null)
					{
						...
						{
							String query=ConnectInit.queries.getProperty("select_rep_cad");
							ResultSet rs=FixedIncomeListTypeClass1.getResult1(con,query,chk_but);
							rs.next();
							String amt=rs.getString("amount");
						...
						//	float tis=rs.getFloat("tis");
							...
							{								
								float shrval=Float.parseFloat(shr);
								query=ConnectInit.queries.getProperty("detail_stock_master");
								ResultSet rs1=FixedIncomeListTypeClass1.getAffected(con,query,s_stock);
								rs1.next();
								float tis=rs1.getFloat("tis");
								if(shrval >= tis)
	...
	/**
	 * @param chk_but The chk_but to set.
	 */
	...
	/**
	 * @param amt_per The amt_per to set.
	 */
	...
	/**
	 * @param ncorp_button The ncorp_button to set.
	 */
	...
	/**
	 * @param compo_button The compo_button to set.
	 */
	...
	/**
	 * @param ind_comp The ind_comp to set.
	 */
	...
	/**
	 * @param hash_stock_error The hash_stock_error to set.
	 */
	...
	/**
	 * @param exc_stk The exc_stk to set.
	 */
	...
	/**
	 * @param excCollection The excCollection to set.
	 */
...
//			for inactive stock display
			...
			if(exch==null)
			{
				...
				Logging.debug("1");
					...
					{
						Logging.debug("2");
						String query=ConnectInit.queries.getProperty("select_from_stock_master");
						rs=FixedIncomeListTypeClass1.resultCorporate(con,query);
					}
					else
					{
						String query=ConnectInit.queries.getProperty("select_index_wise_stock_id");
						rs=FixedIncomeListTypeClass1.getAffected(con,query,i_index);
					}
			...
			else
			{
				...
				ResultSet rs=FixedIncomeListTypeClass1.getAffected(con,query,exch);				
				roles.add(new LabelValueBean("Select Stock",""));				
				if(stkid==null)
				{
					while(rs.next())
	...
	/**
	 * @param stkCollection The stkCollection to set.
	 */
	...
	/**
	 * @param n_share The n_share to set.
	 */
...
//		AcessControl asc=new AcessControl();
		...
		Logging.debug(" Inside getS_stkCollection: select Stock ="+SelExch);
	...
	public Collection getS_stkCollection() {
		...
		try{
			...
			AcessControl asc = ConnectInit.getAcessControl();	
	//		AcessControl asc=new AcessControl();
			String SelStock=asc.getLangValues("StockDetailFromDate.Stock");
			String SelExch=asc.getLangValues("Masters.SelectExch");
			Logging.debug(" Inside getS_stkCollection: select Stock ="+SelStock);
			...
			if(exc!=null)
				...
				if(s_stock!=null)
					...
					if((!(corpid.equals("addstock")))& (!(corpid.equals("deletestock"))) & (!(corpid.equals("changeiwf"))))
					{						
						String query =ConnectInit.queries.getProperty("fixed_income_stock_list");					    
						ResultSet rs=FixedIncomeListTypeClass1.getAffected(con,query,exc);		
						if(s_stock==null)
						{
							while(rs.next())
							{
					...
					{						
						...
						if(corpid.equals("addstock"))
						{
							String query=ConnectInit.queries.getProperty("select_resp_stock");
							rs=FixedIncomeListTypeClass1.getResult_corp(con,query,i_index,exc);
						}
						if((corpid.equals("deletestock"))|(corpid.equals("changeiwf")))
						{
							String query=ConnectInit.queries.getProperty("select_stock_index");
							rs=FixedIncomeListTypeClass1.getResult1(con,query,i_index);
						}
						if(s_stock==null)
						{
							while(rs.next())
	...
	/**
	 * @param collection The s_stkCollection to set.
	 */
	...
	public Collection getCorpCollection() {
		...
		try{
			...
			AcessControl asc = ConnectInit.getAcessControl();	
	//		AcessControl asc=new AcessControl();
			String SelAction=asc.getLangValues("indCompliance.Select_Action");
			Logging.debug(" Inside getIndcorpCollection: select Action ="+SelAction);
	...
	/**
	 * @param corpCollection The corpCollection to set.
	 */
	...
	/**
	 * @param s_ratio1 The s_ratio1 to set.
	 */
	...
	/**
	 * @param s_ratio2 The s_ratio2 to set.
	 */
	...
	/**
	 * @param new_corp_but The new_corp_but to set.
	 */
	...
	public Collection getStkmulCollection() {
		...
		try{
			...
			{	
				...
				if((corpid.equals("addstock"))|(corpid.equals("changeiwf")))
				{
					String query=ConnectInit.queries.getProperty("select_stock_index");
					rs=FixedIncomeListTypeClass1.getResult1(con,query,i_index);
				}
	...
	/**
	 * @param stkmulCollection The stkmulCollection to set.
	 */
	...
	//		AcessControl asc=new AcessControl();
			...
			Logging.debug(" Inside getS_stkCollection: select Stock ="+AffectedInd);
			
			String qry=ConnectInit.queries.getProperty("fixed_income_affected_index_by_ca");			
			rs = FixedIncomeListTypeClass1.getResult1(con,qry,s_stock);
			roles.add(new LabelValueBean(AffectedInd,""));
			if(s_affectedIndex!=null)
			{
				if((s_affectedIndex.equals(""))|(s_affectedIndex.equals(AffectedInd)))
					s_affectedIndex=null;
			}			
			if(s_affectedIndex==null)				
			{			
	...
	/**
	 * @param affstkCollection The affstkCollection to set.
	 */
	...
	public ArrayList getStocklist() {
		...
		try{
		    ...
		    String corp_val=getCorpid();
		   	String query =ConnectInit.queries.getProperty("get_corporate_list_stock");
			ResultSet rs = FixedIncomeListTypeClass1.resultCorporate(con,query);
			int cid=FixedIncomeListTypeClass1.check_corp_name1(rs,corp_val);
			rs.close();
			
			query=ConnectInit.queries.getProperty("select_stock_corpdiary");
			rs=FixedIncomeListTypeClass1.getresp_cad(con,query,Integer.toString(cid),s_stock,apply_date);
		   	while(rs.next())
		   	{			
		   		setId(rs.getString("cad_id"));		   		
				sl=new stocklist(rs.getString("cad_id"),rs.getString("cam_name"),rs.getString("stock_name"),rs.getString("apply_on_date"),rs.getString("amount"),rs.getString("ratio_for_shares"),rs.getString("ratio_shares_offered"),rs.getString("percentage"),rs.getString("values"));
				Pp.add(sl);
		   	}		
		}catch(Exception e){
			Logging.error("Error="+e.getMessage());
...
//			Check no CA applied on this stock, 
			...
			{	
				...
				ResultSet rs=FixedIncomeListTypeClass1.check_dairy_exist(con,query,c_Cad,stid,applied_date,time);
				boolean chk_rs=rs.next();
				if(chk_rs==true)//if applied
				{
					...
					while(rs.next())
					{
						count++;
						String cid=rs.getString("cad_id");			
						String qry=ConnectInit.queries.getProperty("cad_stkind_details");
						ResultSet rs1=FixedIncomeListTypeClass1.getresp_cad(con,qry,cid,cid,cid);
						rs1.next(); 					
						sl=new stocklist(Integer.toString(count),rs1.getString("cam_name"),rs1.getString("stock_ex_name"),rs1.getString("stock_name"),rs1.getString("index_name"),rs1.getString("apply_on_date"),rs1.getString("applied_date"),rs1.getString("amount"),rs1.getString("percentage"),rs1.getString("ratio_for_shares"),rs1.getString("ratio_shares_offered"),rs1.getString("values"),rs1.getString("status"));
						Pp.add(sl);
					}
				}//chk_rs true
				if(chk_rs==false)
				{
					...
					query=null;
					query=ConnectInit.queries.getProperty("select_index_composition");
					rs=FixedIncomeListTypeClass1.getAffected(con,query,stid);
					...
					if(chk_stk==true)//if stock belongs to any index
					{
						...
						while(rs.next())
						{
							...
							String qry=null;
							qry=ConnectInit.queries.getProperty("check_ind_exist");						
							ResultSet rs1=FixedIncomeListTypeClass1.getResult_event2(con,qry,c_Cad,index_id,applied_date,time);
							...
							if(chk_ind==true)//if applied on index
							{
								...
								while(rs1.next())
								{
									...
									String qry1=ConnectInit.queries.getProperty("cad_stkind_details");
									ResultSet rs2=FixedIncomeListTypeClass1.getresp_cad(con,qry1,cadid,cadid,cadid);
									rs2.next();							
									sl=new stocklist(Integer.toString(count),rs2.getString("cam_name"),rs2.getString("stock_ex_name"),rs2.getString("stock_name"),rs2.getString("index_name"),rs2.getString("apply_on_date"),rs2.getString("applied_date"),rs2.getString("amount"),rs1.getString("percentage"),rs2.getString("ratio_for_shares"),rs2.getString("ratio_shares_offered"),rs2.getString("values"),rs2.getString("status"));
									Pp.add(sl);		
									flag=1;
								}//while rs1
							}//chk_ind true
						...
						if(flag==0)
						{
							...
							setHash(hash);							
							FixedIncomeListTypeClass1.corp_detail(this);																
							query =ConnectInit.queries.getProperty("stock_details_for_ca");
					        rs = FixedIncomeListTypeClass1.getResult12(con,query,s_stock);
						    try{
								setResult(rs);
					        }catch (Exception e) {
			...
			if(r_type.equals("index event"))
			{
				String query=ConnectInit.queries.getProperty("select_undo_list_index");		
				ResultSet rs=FixedIncomeListTypeClass1.check_dairy_exist(con,query,c_Cad,i_index,applied_date,time);
				boolean chk_rs=rs.next();
				if(chk_rs==true)//if applied
				{
					rs.beforeFirst();
				...
				if(chk_rs==false)
				{
					...
					query=null;
					query=ConnectInit.queries.getProperty("check_ind_exist_stock");
					rs=FixedIncomeListTypeClass1.check_dairy_exist(con,query,c_Cad,applied_date,time,i_index);
					boolean chk_stk=rs.next();
					if(chk_stk==true)//if stock belongs to any index
					{
						rs.beforeFirst();
						...
						while(rs.next())
						{
							...
							ResultSet rs2=FixedIncomeListTypeClass1.getresp_cad(con,qry1,cadid,cadid,cadid);
							rs2.next();							
							sl=new stocklist(Integer.toString(count),rs2.getString("cam_name"),rs2.getString("stock_ex_name"),rs2.getString("stock_name"),rs2.getString("index_name"),rs2.getString("apply_on_date"),rs2.getString("applied_date"),rs2.getString("amount"),rs.getString("percentage"),rs2.getString("ratio_for_shares"),rs2.getString("ratio_shares_offered"),rs2.getString("values"),rs2.getString("status"));
							Pp.add(sl);		
							flag=1;
						}//while
					}//if chk_stk
					if(chk_stk==false)
					{
						 setLeng("1");
						 setCad_id(c_Cad);
						 FixedIncomeListTypeClass1.stock_corpdetail(this);
					}//if chk_stk false
				}//if chk_rs is false
			}//if index event
		}catch(Exception e){
	...
	/**
	 * @param ind_div The ind_div to set.
	 */
	...
	/**
	 * @param ftcurrency The ftcurrency to set.
	 */
	...
	//		AcessControl asc=new AcessControl();
			...
			Logging.debug(" Inside getS_stkCollection: select Stock ="+SelCurr);
			...
			String query=ConnectInit.queries.getProperty("resp_frto_currency");
			ResultSet rs=FixedIncomeListTypeClass1.getResult12(con,query,currencyid);
			if(ftcurrency==null)
			{
				while(rs.next())
				{					
					String count=rs.getString("from_to_currency_id");
					roles.add(new LabelValueBean(rs.getString("description"),count));
				}
			}
			else
			{
	...
	/**
	 * @param ftCollection The ftCollection to set.
	 */
	...
	/**
	 * @param curr_val The curr_val to set.
	 */
	public void setCurr_val(String curr_val) {
		...
		/**
		 * @param check_curr The check_curr to set.
		 */
	...
	public ArrayList getHistoriclist() {
		Connect c = ConnectInit.getConnect();
		con = c.getConnection();
		Logging.getDebug("in history list");
		Logging.getDebug("r type   in history="+r_type);
		...
		try{
			...
			if(r_type.equals("stock event"))
			{
				String query=c.queries.getProperty("historic_list");		
				ResultSet rs=ListTypeClass1.hist_list(con,query,s_stock,apply_date);
				boolean chk_rs=rs.next();
				if(chk_rs==true)//if applied
				{
					flg=1;
			...
			if(r_type.equals("index event"))
			{
				Logging.getDebug("in index event diary");
				boolean chk_rs=false;
				ResultSet rs=null;
				Logging.getDebug("corpid in index eve=="+corpid);
				if(corpid.equals("changeindcurr"))
				{
					String query=c.queries.getProperty("Hist_currind_list");
					rs=ListTypeClass1.getResult_apply(con,query,i_index,apply_date);
					chk_rs=rs.next();
				}
				else
				{
					String query=c.queries.getProperty("historic_list_index");
					Logging.getDebug("in index event diary"+stock[0]);
					String stk=stock[0];
					rs=ListTypeClass1.hist_list1(con,query,i_index,apply_date,stk);
					chk_rs=rs.next();
				}
				if(chk_rs==true)//if applied
				{
			...
			{
				Logging.getDebug("in index event diary");
				String query=c.queries.getProperty("historic_list_index1");
				ResultSet rs=ListTypeClass1.hist_list(con,query,i_index,apply_date);
				boolean chk_rs=rs.next();
				if(chk_rs==true)//if applied
				{
					flg=1;
			...
			Logging.getDebug("vactor after hist=="+v);
			if(v.isEmpty()==false)
			{
				int count=0;
				for(Enumeration enumm=v.elements();enumm.hasMoreElements();)
				{
					...
					String qry=c.queries.getProperty("cad_stkind_details");
					ResultSet rs1=ListTypeClass1.getresp_cad(con,qry,id,id,id);
					rs1.next();		
					...
					String ap_date=rs1.getString("apply_on_date");
					 ...
					 if(val==null)
					 	val="-";
					sl=new stocklist(Integer.toString(count),rs1.getString("cam_name"),stkexname,sname,iname,ap_date,rs1.getString("applied_date"),amt,per,r1,r2,val,rs1.getString("status"));
					Pp.add(sl);
				}
			}
			Logging.getDebug("r type in history=="+r_type);
			if(r_type.equals("stock event"))
			{				
				Logging.getDebug("close in historic=="+getClose());
				if(flg==0)
				{											
					String corp_act=corpid;
					String chk_flg=check_type;  //check action is from diary or action page
					if(chk_flg!=null)
						if(chk_flg.equals(""))
							chk_flg=null;
					...
					if(chk_flg!=null)
					{
						...
						{
							try{
							String qry=c.queries.getProperty("get_corporate_list_stock");							
							ResultSet rs=ListTypeClass1.resultCorporate(con,qry);
							ListTypeClass1.check_corp_name(rs,corp_act,this);
							rs.close();
							
							//check whether this combination is present in dairy or not.
							String query2=c.queries.getProperty("select_corporate_action_diary");
							ResultSet rs2=ListTypeClass1.getresp_cad(con,query2,corpid,s_stock,apply_date);
							...
							{
								int flg_val=NCorp_Action.CalApply(con,c,this);   	//calculate the new values
								setCommit_butt("1");
								setLeng("2");
							}
							}catch(Exception e){
			...
			if(r_type.equals("index event"))
			{
				...
				{
					try{						
						//check whether this action is persent in diary or not
						String corpnm=corpid;
						if(corpnm.equals("changeindcurr"))
						{
								String chk=ind_div;							
						//check whether the action is for only tmcv change or both tmcv & divisor							
								if(chk.equals("td"))//for tmcv&divisor
								{									
									//calculate new value
										ActionCorp.cal_curr_ind(con,c,this,i_index);																											
								}
								else  //for tmcv only
								{
									//calculate new value
									ActionCorp.cal_curr_ind(con,c,this,i_index);
									//for change in tmcv only newdivisor is equals to olddivisor
									if(ind_div.equals("t"))
									{
										setNewdivisor(getDivisor());
									}																	
								}	
								IndexCA_Action.tmcv_div_adj(this);//remove E power
								String query=c.queries.getProperty("resp_child_indcurr");
								ListTypeClass1.affect_child_list(con,this,query);
								setLeng("2");
								setCommit_butt("1");
							}
						else
						{
							boolean flag=IndexCA_Action.check_exist(i_index,this,corpid,stock);
							corpid=corpnm;
							if(flag==true)
							{
							...
							{								
								IndexCA_Action.insert_hash(stock,this);             //insert into hash all selected stock
			...
			if(r_type.equals("index event diary"))
			{
				Logging.getDebug("flg in index event diary=="+flg);
				if(flg==0)
				{
					int flg_val=0;
					flg_val=IndexCA_Action.check_close(con,c,this);
	...
	*/
	...
	/**
	 * @param mark_lot The mark_lot to set.
	 */
	...
	/**
	 * @param succ_butt The succ_butt to set.
	 */
	...
	/**
	 * @param copy_hash The copy_hash to set.
	 */
	...
	/**
	 * @param stk_flag The stk_flag to set.
	 */
	...
	/**
	 * @param affIndexNoClosing The affIndexNoClosing to set.
	 */
