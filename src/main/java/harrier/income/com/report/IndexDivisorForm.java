 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class IndexDivisorForm extends ActionForm{
	Logger Logging = Logger.getLogger(IndexDivisorForm.class);
	IndexDivisorForm indform =null;
	private String check;
	private String selchart;
	
	private String avgSpan=null,roleId_div;
...
//								Change by Manoj adekar
								int role_id2=Integer.parseInt(roleId_div);
								if(role_id2!=1)
								{
...
//		Change by Manoj Adekar for Dynamic connection using getdbConnection() instead of  getConnection()
	...
	public StringBuffer display_Divisor(String indIdS,String FrDt,String toDt)
	{
		...
		try
				{	
					tableDataNewS=loadTableDataNew(indIdS,FrDt,toDt);
					
					int count = -1;
					String style=null;
					
					String bcolor="";
					for(int i=0;i<tableDataNewS.size();i++)
					{
						count++;
						if((count==0))
						{
						...
						{			
								...
								else
								{
									...
									indform = (IndexDivisorForm)tableDataNewS.get(i);
									count++;
	...
	public String getRoleId_div() {
		return roleId_div;
	}

	public void setRoleId_div(String roleId_div) {
		this.roleId_div = roleId_div;
	}
