 ...
 */
// changed by Sonali to on 07-Aug-2006 to 
// 1) Replace the static connections 
// 2) Check the resultset for null before using 
...
/**
 * @author pranoti TODO To change the template for this generated type comment
...
public class UpdateCorp {
	...
	public static void updateAction(Connection con, String query12,
			Corporate corporateact) {
		try {
	...
	public static void update_resp_Action(Connection con, String query12,
			Corporate corporateact) {
		try {
			PreparedStatement stmt = con.prepareStatement(query12);
			stmt.setDouble(1, Double.parseDouble(corporateact.getNewLTP()));
	...
	public static void update_undo_stkprice(Connection con, String undo_qry,
			String query, Corporate corporateact) {
		try {
			ResultSet rs = ListTypeClass1.getResult_apply(con, undo_qry,
					corporateact.getStkid(), corporateact.getApplied_date());
			String stkpri_id = null;
			if (rs != null && rs.next()) {
				stkpri_id = rs.getString("stock_price_daily_id");
			}
			if (rs != null)
				rs.close();
	...
	public static void updatecad(Connection con, String query, String query1,
			Corporate corporateact) {
		int amut = 0;
		...
		try {
			for (Enumeration enmm = hash1.keys(); enmm.hasMoreElements();) {
				...
				if (query1 != null) {
					ResultSet rs1 = ListTypeClass1.getAffected(con, query1, id);
	...
	public static void update_hist_cad(Connection con, String query,
			String query1, Corporate corporateact, String id) {
		try {
			ResultSet rs1 = ListTypeClass1.getAffected(con, query1, id);
			rs1.next();
			PreparedStatement stmt = con.prepareStatement(query);
	...
	public static void insert_into_cad(Connection con, String query,
			String nextval, Corporate newcorporateact, String flag) {
		try {
			String cad_val = null;
			if (flag == null) {
				int cad = ListTypeClass1.Select_nextval(con, nextval);
	...
	public static void insert_into_cad_with_time(Connection con, String query,
			String nextval, Corporate newcorporateact, String flag) {
		try {
			String cad_val = null;
			if (flag == null) {
	...
	public static void reset(Corporate newcorporateact) {
	...
	public static void updatestkmaster(Connection con, String query,
			Corporate corporateact, String face) {
		try {
			PreparedStatement stmt = con.prepareStatement(query);
			...
			PreparedStatement statement = con
					.prepareStatement("update corporate_action_diary set final_tis = (select tis from stock_master where stock_id = ? ) where cad_id = ?");
			statement.setString(1, corporateact.getStkid());
			statement.setString(2, corporateact.getCad_id());
	...
	public static void update_stkmas_hist(Connection con, String query,
			Corporate corp) {
		try {
			PreparedStatement stmt = con.prepareStatement(query);
	...
	public static void update_indcomp_history(Connection con, String query,
			Corporate corp) {
		try {
			PreparedStatement stmt = con.prepareStatement(query);
	...
	public static void insertstkmaster(Connection con, String query,
			String query1, Corporate corporateact, String face) {
		try {
			PreparedStatement stmt = con.prepareStatement(query);
			...
			if (rs != null && rs.next()) {
				...
				if (hash1.isEmpty() == false) {
					for (Enumeration enmm = hash1.keys(); enmm
							.hasMoreElements();) {
						String id = (String) enmm.nextElement();
						String qry = "";
	...
	public static void update_index(Connection con, String query, ResultSet rs,
			Corporate corporateact) {
		try {
			...
			while (rs.next() && rs != null) {
				...
				NCorp_Action.get_currency(con, connect, corporateact,
						rs.getString("index_id"), rs.getString("stock_id"));
				double mcv = 0.0;
				mcv = cf.calMarketCap(
						Double.parseDouble(corporateact.getNewLTP()),
						Long.parseLong(corporateact.getMark_lot()),
						Double.parseDouble(corporateact.getCurr_val()),
						Long.parseLong(corporateact.getNewTIS()),
						Double.parseDouble(corporateact.getIwfstk()));

				PreparedStatement pstmt = con.prepareStatement(query);
				pstmt.setDouble(1, rs.getDouble("iwf"));
	...
	public static void update_hist_index(Connection con, String query,
			ResultSet rs, Corporate corporateact) {
		try {
			PreparedStatement pstmt = con.prepareStatement(query);
	...
	public static void check_date(Corporate newcorporateact) {
	...
	public static void check_value(Corporate corporateact) {
		try {
			 ...
			 * corporateact.setRatio1("1"); else corporateact.setRatio1(null); }
			 * if(corporateact.getRatio2().equals("")) {
			 * if(corporateact.getCorpid().equals("8"))
			 * corporateact.setRatio2("1"); else corporateact.setRatio2(null); }
			 * if(corporateact.getAmt().equals("")) corporateact.setAmt(null);
			 */
			if (corporateact.getShare() != null) {
				if (corporateact.getShare().equals(""))
					corporateact.setShare(null);
			} else // (corporateact.getShare().equals(""))
			{
				corporateact.setShare(null);
			}

			if (corporateact.getRatio1() != null) {
				if (corporateact.getRatio1().equals("")) {
					if (corporateact.getCorpid().equals("8"))
			...
			if (corporateact.getRatio2() != null) {
				if (corporateact.getRatio2().equals("")) {
					if (corporateact.getCorpid().equals("8"))
	...
	public static void checkvalue(Corporate newcorporateact) {
	...
	public static void updateAction1(Connection con, String query12,
			Corporate corporateact) {
		try {
			PreparedStatement stmt = con.prepareStatement(query12);
	...
	public static void chagecurrupd(Connection con, Connect connect,
			Corporate corp) {
		try {
			// as this method is used both for historic and general
			// calculation,so there should be date comparison
			...
			ResultSet rs2 = ListTypeClass1.resultCorporate(con, corp_query);
			ListTypeClass1.check_corp_name(rs2, corpval, corp);
			if (rs2 != null)
				rs2.close();
	...
	public static void update_affectInd_curr(Connection con, Connect connect,
			Corporate corp) {
		try {
			int oldcur = 0;
			int newcur = 0;
	...
	public static void update_hist_divisor(Connection con, String query,
			Corporate corporateact, String id) {
		try {
			PreparedStatement stmt = con.prepareStatement(query);
	...
	public static void updateDivisor(Connection con, String query11,
			String query12, Hashtable hash, Corporate corporateact) {
		try {
			Connect connect = ConnectInit.getConnect(); // connection
			for (Enumeration enmm = hash.keys(); enmm.hasMoreElements();) {
				String id = (String) enmm.nextElement();
				ResultSet rs1 = ListTypeClass1.getResult12(con, query11, id);
				if (rs1.next()) {
					...
					String apply1 = corporateact.getApply_date();
					NCorp_Action.get_currency(con, connect, corporateact, id,
							corporateact.getStid());
					// CFormula cf = new CFormula();
					CFormula cf = ConnectInit.getCFormula();
					double oldmcv = cf.calMarketCap(
							Double.parseDouble(corporateact.getOldltp()),
							Long.parseLong(corporateact.getMark_lot()),
							Double.parseDouble(corporateact.getCurr_val()),
							Long.parseLong(corporateact.getOldtis()),
							Double.parseDouble(corporateact.getIwfstk()));
					double newmcv = cf.calMarketCap(
							Double.parseDouble(corporateact.getNewLTP()),
							Long.parseLong(corporateact.getMark_lot()),
							Double.parseDouble(corporateact.getCurr_val()),
							Long.parseLong(corporateact.getNewTIS()),
					...
					double newdivisor = cf.newDivisorCorp(oldtmcv, diff,
							olddivi);
	...
	public static void update_hist_value(Corporate corp) {
		try {
			...
			if (hash1.isEmpty() == true) {
				...
				if (chk == null) {
					...
					ListTypeClass1.check_corp_name(rs, corp_act, corp);
					query = ConnectInit.queries
							.getProperty("insert_into_cad_values_with_time");
					String nextval = ConnectInit.queries
							.getProperty("get_sequence_cad_id");
					UpdateCorp.insert_into_cad_with_time(con, query, nextval,
							corp, null);
				} else {
					// Logging.getDebug("chk in update=="+chk);
					String query = ConnectInit.queries
							.getProperty("update_cad");
					hash1 = corp.getHash1();
					hash1.clear();
					corp.setHash1(hash1);
					hash1 = corp.getHash1();
			...
			if (flag == 1) {
				...
				double newmcv = cf.calMarketCap(
						Double.parseDouble(corp.getNewLTP()),
						Long.parseLong(corp.getMark_lot()), 1,
						Long.parseLong(corp.getNewTIS()),
						Double.parseDouble(corp.getIwfstk()));
				...
				if (!(corp_act.equals("stockdividend/bonus"))) {
					...
					query = ConnectInit.queries
							.getProperty("update_undo_ind_close");
					UpdateCorp.hist_update_Divisor(con, qry, query,
							corp.getHash(), corp);
				}
				// update index composition
				query = ConnectInit.queries
						.getProperty("select_index_composition");
				ResultSet rs1 = ListTypeClass1.getAffected(con, query,
						corp.getStid());
				String qry = ConnectInit.queries.getProperty("update_index");
	...
	public static void Update_hist_stkprdaily(Connection con, Connect connect,
			Corporate corp) {
		try {
			// CFormula cf=new CFormula();
			CFormula cf = ConnectInit.getCFormula();
			String up_query = ConnectInit.queries
					.getProperty("update_undo_stock_price");
			String query = ConnectInit.queries
					.getProperty("select_stkprice_historic");
			ResultSet rs = ListTypeClass1.getResult_apply(con, query,
					corp.getStid(), corp.getApply_date());
			while (rs != null && rs.next()) {
				String close = rs.getString("adjusted_price");
				if (close == null)
					close = rs.getString("stock_closing_value");
				double mcv = 0.0;
				mcv = cf.calMarketCap(Double.parseDouble(close),
						Long.parseLong(corp.getMark_lot()), 1,
						Long.parseLong(corp.getNewTIS()),
						Double.parseDouble(corp.getIwfstk()));
				...
				stmt.setString(3, rs.getString("stock_price_daily_id"));
				stmt.executeUpdate();
			}
		} catch (Exception e) {
			Logging.error("Error=" + e.getMessage());
	...
	public static void reapply_action(Corporate corp) {
		try {
			Connect connect = ConnectInit.getConnect();
			connect = ConnectInit.getConnect();
			...
			CFormula cf = ConnectInit.getCFormula();
			// queries
			String stk_close = ConnectInit.queries
					.getProperty("get_undo_close_value");// get stock closing
															// value for
															// respective date
			...
			for (int i = (vlen - 1); i >= 0; i--) {
				...
				ResultSet rs1 = ListTypeClass1.getAffected(con, qry, id);
				rs1.next();
				String cam = rs1.getString("cam_id");
				String stock = rs1.getString("stock_id");
				...
				ResultSet rs = ListTypeClass1.getAffected(con, query, cam);
				String corp_nm = null;
				String type = null;
				if (rs != null && rs.next()) {
				...
				if (type.equals("stock event")) {
					...
					rs = ListTypeClass1.getResult_apply(con, stk_close, stock,
							date);
					if (rs != null && rs.next()) {
						...
						corp.setClose(close);
						// get stock detail
						rs = ListTypeClass1.getAffected(con, stk_qry, stock);
						rs.next();
					...
					NCorp_Action.CalApply(con, connect, corp);
					...
					if (face == null)
						face = corp.getFace();
					String up_stkqry = ConnectInit.queries
							.getProperty("update_tis_stockmaster");
					...
					query = ConnectInit.queries
							.getProperty("update_undo_stock_price");
					UpdateCorp.update_undo_stkprice(con, undo_query, query,
							corp);
					...
					if (!(corp_nm.equals("stock dividend/bonus"))) {
						...
						rs = ListTypeClass1.getResult1(con, qry1, stock);
						boolean chk_rs = rs.next();
						if (chk_rs == true) // if stock belong to any index
						{
							rs.beforeFirst();
							while (rs != null && rs.next()) {
								String index = rs.getString("index_id"); // get
																			// index
																			// id
								// get currency exchange rate
								String apply1 = corp.getApply_date();
								Logging.debug("apply date from line no 1320 Updatecorp.java "
										+ apply1);
								NCorp_Action.get_currency(con, connect, corp,
										index, stock);
								mcv = cf.calMarketCap(
										Double.parseDouble(corp.getNewLTP()),
										Long.parseLong(corp.getMark_lot()),
										Double.parseDouble(corp.getCurr_val()),
										Long.parseLong(corp.getNewTIS()),
										Double.parseDouble(corp.getIwfstk()));
								corp.setNewmcv(mcv);
								// update index composition
								String up_qry = ConnectInit.queries
										.getProperty("update_index"); // update
																		// index
																		// composition
								UpdateCorp.update_hist_index(con, up_qry, rs,
										corp);

								String query1 = ConnectInit.queries
										.getProperty("get_undo_index_close"); // get
																				// index
																				// values
								ResultSet rs2 = ListTypeClass1.getResult_apply(
										con, query1, index, date);
								rs2.next();
								corp.setDivisor(rs2.getString("divisor"));
								corp.setTmcv(rs2.getString("tmcv"));

								ActionCorp.toGetaffect(corp);// calculate index
																// new values

								// //update index value daily
								query = ConnectInit.queries
										.getProperty("update_undo_ind_close");
								UpdateCorp.hist_update_Divisor(con, query1,
										query, corp.getHash(), corp);
					...
					query = ConnectInit.queries
							.getProperty("update_stmas_history");
				...
				if (type.equals("index event")) // recalculate index
				{
					...
					Hashtable hash_aff = corp.getHash_affind(); // refresh
																// affect index
					hash_aff.clear();
					...
					if (corp_act.equals("changeindcurr")) {
						...
						ActionCorp.temp_update_curr(con, connect, corp, date,
								new_val, ft_curr, desc);
						// get child indices
						query = ConnectInit.queries
						...
						if (hash_aff.isEmpty() == false) {
							for (Enumeration enmm = hash_aff.keys(); enmm
									.hasMoreElements();) {
								String id1 = (String) enmm.nextElement();
								String div[] = ActionCorp.token2(id1);
								corp.setI_index(div[1]);
								ActionCorp.temp_update_curr(con, connect, corp,
										date, new_val, ft_curr, desc);
							}
						}
					}// if change index currency
					else {
						hist_reapply_action(con, connect, corp, date); // reapply
																		// parent
																		// index
	...
	public static void hist_reapply_action(Connection con, Connect connect,
			Corporate corp, String date) {
		...
		try {
			...
			String qry = ConnectInit.queries
					.getProperty("update_indcomp_history");

			// get latest index closing value
			String query1 = ConnectInit.queries
					.getProperty("get_latest_index_value_index_wise");
			ResultSet rs = ListTypeClass1.getResult12(con, query1,
					corp.getI_index());
			...
			String ind_comp = ConnectInit.queries
					.getProperty("get_composition_of_parent");// get all stock's
																// list
			ResultSet rs2 = ListTypeClass1.getAffected(con, ind_comp,
					corp.getI_index());
			while (rs != null && rs2.next()) {
				String stk = rs2.getString("stock_id");
				String iwf = rs2.getString("iwf");

				rs = ListTypeClass1.getResult_apply(con, stk_close, stk, date);
				String close = null;
				if (rs != null && rs.next()) {
					close = rs.getString("adjusted_price");
					if (close == null)
				...
				// get stock detail
				rs = ListTypeClass1.getAffected(con, stk_qry, stk);
				String tis = null;
				String ml = null;
				String iwf_val = null;
				...
				// get currency exchange value
				String apply1 = corp.getApply_date();
				Logging.debug("apply date from line no 1461 Updatecorp.java "
						+ apply1);
				NCorp_Action.get_currency(con, connect, corp,
						corp.getI_index(), stk);
				...
				if (stk.equals(stock)) {
					...
					if (corp_act.equals("changeiwf")) {
						mcv = cf.calMarketCap(Double.parseDouble(close),
								...
								Double.parseDouble(corp.getCurr_val()),
								Long.parseLong(tis),
								Double.parseDouble(corp.getValues()));
						corp.setNewmcv(mcv);
						tmcv = tmcv + mcv;
						// update index composition
			...
			{
				rs = ListTypeClass1
						.getResult_apply(con, stk_close, stock, date);
				String close = null;
				if (rs != null && rs.next()) {
					close = rs.getString("adjusted_price");
					if (close == null)
				...
				// get currency exchange value
				String apply1 = corp.getApply_date();
				Logging.debug("apply date from line no 1527 Updatecorp.java "
						+ apply1);
				NCorp_Action.get_currency(con, connect, corp,
						corp.getI_index(), stock);
	...
	public static void recalupdateDivisor(Connection con, String query11,
			String query12, Hashtable hash, Corporate corporateact) {
		...
		for (Enumeration enmm = hash.keys(); enmm.hasMoreElements();) {
			String id = (String) enmm.nextElement();
			try {
				ResultSet rs1 = ListTypeClass1.getResult_apply(con, query11,
						id, date);
				rs1.next();
				...
				double diff = cf.diffTMCV(oldtmcv, newtmcv);
				double newdivisor = cf.RecalnewDivisorCorp(oldtmcv, diff,
						olddivi);
				PreparedStatement stmt = con.prepareStatement(query12);
	...
	public static void undoupdateDivisor(Connection con, String query11,
			String query12, Hashtable hash, Corporate corporateact) {
	...
	public static void hist_update_Divisor(Connection con, String query11,
			String query12, Hashtable hash, Corporate corporateact) {
	...
	public static void update_Divisor(Connection con, String query11,
			String query12, Hashtable hash, Corporate corporateact) {
		String date = corporateact.getApplied_date();// Applied_date();
		Connect connect = ConnectInit.getConnect();
		for (Enumeration enmm = hash.keys(); enmm.hasMoreElements();) {
	...
	public static void recal_assign_index_detail(Connection con,
			Connect connect, Corporate corporateact) {
		try {
			// CFormula cf=new CFormula();
			CFormula cf = ConnectInit.getCFormula();
			String query1 = ConnectInit.queries.getProperty("select_rep_cad");
			String update_cad = ConnectInit.queries
					.getProperty("update_index_cad_with_time");
			String corp_query = ConnectInit.queries
					.getProperty("get_corporate_list_index");
			String insert_cad = ConnectInit.queries
					.getProperty("insert_index_cad");
			String query3 = ConnectInit.queries
					.getProperty("select_stock_detail");
			String index_comp_val = ConnectInit.queries
			...
			// as this method is used both for historic and general CA, there
			...
			for (Enumeration enmm = hash.keys(); enmm.hasMoreElements();) {
				...
				if (div[0].equals("true")) {
					...
					ResultSet rs = ListTypeClass1.getAffected(con, query1,
							div[1]);
					rs.next();
					corporateact
							.setAnnounce_date(rs.getString("announce_date"));
				...
				if (div[0].equals("false")) {
					...
					if ((corp.equals("addstock"))
							| (corp.equals("deletestock")))
						corporateact.setValues(null);
					if (corp.equals("changeiwf"))
						corporateact.setValues(val);
					ResultSet rs2 = ListTypeClass1.resultCorporate(con,
							corp_query);
					ListTypeClass1.check_corp_name(rs2, corp, corporateact);
					if (rs2 != null)
						rs2.close();
				...
				if (rs2 != null && rs2.next()) {
					...
					if (corpid.equals("deletestock")) {
						ResultSet rs = ListTypeClass1.getResult_corp(con,
								index_comp_val, corporateact.getI_index(),
								stock);
						if (rs != null && rs.next()) {
				...
				if (chk_hash == false) {
					...
					if (chk_dt == 0) {
						...
						rs1 = ListTypeClass1.getResult12(con, stock_price,
								stock);
					} else {
						String stock_price = ConnectInit.queries
								.getProperty("get_undo_close_value");// get
																		// stock
																		// closing
																		// value
																		// for
																		// respective
																		// date
						rs1 = ListTypeClass1.getResult_apply(con, stock_price,
								stock, corporateact.getApply_date());
					}
					...
					// get currency exchange value
					NCorp_Action.get_currency(con, connect, corporateact,
							corporateact.getI_index(), stock);

					double mcv = cf.calMarketCap(Double.parseDouble(close),
							Long.parseLong(ml),
							Double.parseDouble(corporateact.getCurr_val()),
							Long.parseLong(tis), Double.parseDouble(value));
	...
	public static void undocurr_ind(Corporate corp, int flag) {
		try {
			Connect connect = ConnectInit.getConnect();
			Connection con = null;
			if (con == null) {
			...
			query = null;
			query = ConnectInit.queries.getProperty("update_indmaster");
			update_curr_indmaster(con, query, corp); // done upto this on 8th
														// august sonali
			...
			String del_qry = ConnectInit.queries
					.getProperty("delete_indmas_history");
	...
	public static void undoassign_index_detail(Corporate corporateact) {
		Connect connect = ConnectInit.getConnect();
		Connection con = null;
		if (con == null) {
			con = connect.getdbConnection();
		...
		try {
			...
			for (Enumeration enmm = hash.keys(); enmm.hasMoreElements();) {
				...
				corporateact.setCad_id(div[1]);
				ResultSet rs = ListTypeClass1.getAffected(con, query1, div[1]);
				rs.next();
				...
				ResultSet rs1 = ListTypeClass1.getResult_apply(con,
						stock_price, stock, date);
				rs_cond = rs1.next();
				if (rs_cond == true) {
					close = rs1.getString("adjusted_price");
					if (close == null)
					...
					if (close != null) {
						rs1.close();
						rs1 = ListTypeClass1.getAffected(con, stk_query, stock);
						rs1.next();
						...
						update_index_cad_with_time(con, update_cad,
								corporateact);
						corporateact.setCorpid(corp_value);
						corporateact.setApplied_date(date);
						...
						if (corp_val.equals("deletestock")) {
							rs1 = ListTypeClass1.getAffected(con, stk_query,
									stock);
							if (rs1 != null && rs1.next())
						...
						if (corp_val != null) {
							if (corp_val.equals("addstock")) {
								mcv = cf.calMarketCap(
										Double.parseDouble(close), Long
												.parseLong(ml), Double
												.parseDouble(corporateact
														.getCurr_val()), Long
							...
							if (corp_val.equals("deletestock")) {
								mcv = cf.calMarketCap(
										Double.parseDouble(close), Long
												...
												.parseDouble(value));
								corporateact.setNewmcv(mcv);
								insert_index_comp(con, insert_index_comp,
										corporateact);
							}
							if (corp_val.equals("changeiwf")) {
								...
								if (rs != null)
									rs.close();
								double mcv_new = cf.calMarketCap(Double
										.parseDouble(close),
										Long.parseLong(ml), Double
												.parseDouble(corporateact
														.getCurr_val()), Long
												.parseLong(tis), Double
												.parseDouble(val));
				...
				String query = ConnectInit.queries
						.getProperty("delete_indxcomp_history");
	...
	public static void assign_index_detail(Corporate corporateact) {
		try {
			Connect connect = ConnectInit.getConnect();
			Connection con = null;
			if (con == null) {
			...
			String query1 = ConnectInit.queries.getProperty("select_rep_cad");
			// String
			// update_cad=connect.queries.getProperty("update_index_cad");
			String update_cad = ConnectInit.queries
					.getProperty("update_index_cad_with_time");
			String insert_cad = ConnectInit.queries
					.getProperty("insert_index_cad");
			...
			String query3 = ConnectInit.queries
					.getProperty("select_stock_detail");
			String index_detail = ConnectInit.queries
					.getProperty("get_latest_index_value_index_wise");// "select_index_detail");
			String select_corp = ConnectInit.queries
					.getProperty("select_corp_name");
			String corp_query = ConnectInit.queries
					.getProperty("get_corporate_list_index");
			String get_cam = ConnectInit.queries.getProperty("select_from_cam");
			String select_is_cad = ConnectInit.queries
					.getProperty("select_index_stock_cad");
			String index_comp_val = ConnectInit.queries
					.getProperty("select_index_comp");
				...
				for (Enumeration enmm = hash.keys(); enmm.hasMoreElements();) {
					...
					if (div[0].equals("true")) {
						...
						ResultSet rs = ListTypeClass1.getAffected(con, query1,
								div[1]);
						rs.next();
						corporateact.setAnnounce_date(rs
								.getString("announce_date"));
						corporateact.setEx_date(rs.getString("ex_date"));
						corporateact
								.setRecord_date(rs.getString("record_date"));
						corporateact.setApply_date(rs
								.getString("apply_on_date"));
						String date = accept_date();
						corporateact.setApplied_date(date);
						corporateact.setStatus("y");
						...
						ResultSet rs1 = ListTypeClass1.getResult12(con,
								stock_price, stock);
						rs_cond = rs1.next();
						if (rs_cond == true) {
							close = rs1.getString("adjusted_price");
							...
							ResultSet rs2 = ListTypeClass1.resultCorporate(con,
									corp_query);
							ListTypeClass1.check_corp_name(rs2, corp_val,
									corporateact);
							rs2.close();
							String corpid = corporateact.getCorpid();
							update_index_cad_with_time(con, update_cad,
					...
					if (div[0].equals("false")) {
						...
						if (rs_cond == true) {
							...
							ResultSet rs2 = ListTypeClass1.resultCorporate(con,
									corp_query);
							ListTypeClass1.check_corp_name(rs2, corp,
									corporateact);
							if (rs2 != null)
								rs2.close();
							String nextval = ConnectInit.queries
									.getProperty("get_sequence_cad_id");
							insert_index_cad(con, insert_cad, nextval,
									corporateact);
							corporateact.setCorpid(corp);
							corporateact.setValues(val);
						}
					...
					String corpid = corporateact.getCorpid();
					if (corpid.equals("addstock")) {
						ResultSet rs2 = ListTypeClass1.getResult12(con, query3,
								stock);
						rs2.next();
						corporateact.setValues(rs2.getString("iwf"));
					}
					...
					if (rs_cond == true) {
						...
						if (corpid != null) {
							if (corpid.equals("changeiwf")) {
								ResultSet rs = ListTypeClass1.getResult_corp(
										con, index_comp_val,
										corporateact.getI_index(), stock);
								String val = null;
								if (rs != null && rs.next()) {
							...
							if (corpid.equals("deletestock")) {
								...
								try {
									...
									stmt.setString(1, corporateact.getI_index());
									stmt.setString(2, corporateact.getStid());
									// stmt.executeQuery(); //commented by
									// sonali 05-08-2006
									stmt.execute();
									// added by sonali
								} catch (Exception e) {
	...
	public static void hist_assign_index_detail(Corporate corporateact) {
		try {
			// commented by smaiksha starts
			// Connect connect = ConnectInit.getConnect();
			// Connection Hist_con = connect.getConnectionForHistTransaction();
			// Hist_con.commit();
			// ends
			...
			for (Enumeration enmm = hash.keys(); enmm.hasMoreElements();) {
				...
				if (div[0].equals("true")) {
					...
					int corp = rs.getInt("cam_id");
					ResultSet rs1 = ListTypeClass1.getResult_apply(con,
							stock_price, stock, corporateact.getApply_date());
					rs_cond = rs1.next();
					if (rs_cond == true) {
						close = rs1.getString("adjusted_price");
						if (close == null)
				...
				if (corpid.equals("deletestock")) {
					ResultSet rs = ListTypeClass1.getResult_corp(con,
							index_comp_val, corporateact.getI_index(), stock);
					rs.next();
					corporateact.setValues(rs.getString("iwf"));
				}
			...
			UpdateCorp.update_affect_index(con, connect, corporateact); // update
																		// child
																		// indices
		} catch (Exception e) {
			Logging.error("Exception in second updation" + e.getMessage());
	...
	public static void assign_index_cad(Connection con, Connect connect,
			Corporate corporateact) {
		int val = 0;
		try {
			String query = ConnectInit.queries
			...
			String insert_index_comp = ConnectInit.queries
					.getProperty("insert_into_index_composition");
			String query1 = ConnectInit.queries.getProperty("select_rep_cad");
			String qry_detail = ConnectInit.queries
					.getProperty("select_stock_price_detail");
			String stk_qry = ConnectInit.queries
					.getProperty("get_undo_close_value");
			String index_comp_his = ConnectInit.queries
					.getProperty("insert_index_com_his");
			String index_comp_detail = ConnectInit.queries
					.getProperty("index_comp_detail");
			String delete_index_comp = ConnectInit.queries
					.getProperty("delete_index_comp");
			...
			// as this method is used both for historic and general CA , so
			// there should be date comparison
			String dt = UpdateCorp.accept_date(); // get the current date
			String apply = corporateact.getApply_date();
			int chk_dt = ComputeIndexForm.CompareDate(apply, dt); // check for
																	// the
			...
			for (Enumeration enmm = data.keys(); enmm.hasMoreElements();) {
				...
				ResultSet rs = ListTypeClass1.getAffected(con, query1,
						obj.toString());
				rs.next();
				for (Enumeration enum1 = data_error.keys(); enum1
						.hasMoreElements();) {
				...
				if (val == 0) {
					...
					corporateact.setApplied_date(date);
					corporateact.setStatus("y");
					ResultSet rs3 = ListTypeClass1.getAffected(con, get_name,
							rs.getString("cam_id"));
					rs3.next();
					...
					int size = data2.size();
					if (size != 0) {
						for (Enumeration enm = data2.keys(); enm
								.hasMoreElements();) {
						...
						ResultSet rs1 = null;
						if (chk_dt == 0)
							rs1 = ListTypeClass1.getResult12(con, qry_detail,
									sid);
						else
							rs1 = ListTypeClass1.getResult_apply(con, stk_qry,
									sid, corporateact.getApply_date());
						rs1.next();
						close = rs1.getString("adjusted_price");
						if (close == null)
							close = rs1.getString("stock_closing_value");
						rs1.close();
						rs1 = ListTypeClass1.getAffected(con, stk_query, sid);
						rs1.next();
						tis = rs1.getString("tis");
						ml = rs1.getString("market_lot");
						rs1.close();
					...
					if (corp.equals("addstock")) {
						...
						ResultSet rs2 = ListTypeClass1.resultCorporate(con,
								corp_query);
						ListTypeClass1.check_corp_name(rs2, corpval,
								corporateact);
						rs2.close();
						String corpid = corporateact.getCorpid();
					...
					if (corp.equals("deletestock")) {
						...
						rs1.close();
						String corpval = corporateact.getCorpid();
						ResultSet rs4 = ListTypeClass1.resultCorporate(con,
								corp_query);
						ListTypeClass1.check_corp_name(rs4, corpval,
								corporateact);
						rs4.close();
						String corpid = corporateact.getCorpid();
						ResultSet rs2 = ListTypeClass1.getResult_apply(con,
								index_comp_detail, corporateact.getI_index(),
								corporateact.getStid());
						rs2.next();
						corporateact.setValues(rs2.getString("iwf"));
						double mcv = cf.calMarketCap(Double.parseDouble(close),
								...
								Double.parseDouble(corporateact.getCurr_val()),
								Long.parseLong(tis),
								Double.parseDouble(rs2.getString("iwf")));
						corporateact.setNewmcv(mcv);
						try {
							PreparedStatement stmt = con
									.prepareStatement(delete_index_comp);
							stmt.setString(1, corporateact.getI_index());
							stmt.setString(2, corporateact.getStid());
							stmt.executeQuery();
						} catch (Exception e) {
					...
					if (corp.equals("changeiwf")) {
						double mcv_new = cf.calMarketCap(
								Double.parseDouble(close), Long.parseLong(ml),
								Double.parseDouble(corporateact.getCurr_val()),
								Long.parseLong(tis), Double.parseDouble(iwf));
						ResultSet rs2 = ListTypeClass1.getResult_apply(con,
								index_comp_detail, corporateact.getI_index(),
						...
						rs2.close();
						double mcv_old = cf
								.calMarketCap(Double.parseDouble(close), Long
										.parseLong(ml),
										Double.parseDouble(corporateact
												.getCurr_val()), Long
												.parseLong(tis), Double
						...
						String corpval = corporateact.getCorpid();
						rs2 = ListTypeClass1.resultCorporate(con, corp_query);
						ListTypeClass1.check_corp_name(rs2, corpval,
								corporateact);
						rs2.close();
						String corpid = corporateact.getCorpid();
			...
			} else // update apply date index value
			{
				...
				if (rad_val == null) {
					String ind_val_daily = ConnectInit.queries
							.getProperty("update_resp_index_value_daily");
					...
					String ind_val_daily = ConnectInit.queries
							.getProperty("update_undo_ind_close");
					update_undo_index_daily(con, qry, ind_val_daily,
							corporateact);
				}
			}
			Logging.info("Complete");
	...
	public static void insert_index_comp(Connection con, String query,
			Corporate corporateact) {
		try {
			Statement stmt1 = con.createStatement();
			PreparedStatement stmt = con.prepareStatement(query);
	...
	public static void insert_index_mas_his(Connection con, String query,
			String query1, Corporate corporateact) {
		try {
			ResultSet rs = ListTypeClass1.getAffected(con, query,
					corporateact.getI_index());
			rs.next();
	...
	public static void insert_index_comp_his(Connection con, String query,
			Corporate corporateact) {
		try {
	...
	public static void update_curr_indcomp(Corporate corp) {
		try {
			Connect connect = ConnectInit.getConnect();
			Connection con = null;
			if (con == null) {
			...
			while (rs != null && rs.next()) {
				...
				String qry = ConnectInit.queries
						.getProperty("detail_stock_master");
				ResultSet rs1 = ListTypeClass1.getAffected(con, qry, stock);
				rs1.next();
				...
				rs1 = ListTypeClass1.getResult12(con, qry1, stock);
				rs1.next();
				String close = rs1.getString("adjusted_price");
				if (close == null)
					close = rs1.getString("stock_closing_value");

				double newmcv = cf.calMarketCap(Double.parseDouble(close),
						Long.parseLong(ml), currval, Long.parseLong(tis),
	...
	public static void update_undo_curr_indcomp(Corporate corp) {
		try {
			Connect connect = ConnectInit.getConnect();
			Connection con = null;
			if (con == null) {
			...
			while (rs != null && rs.next()) {
				...
				String stk_qry = ConnectInit.queries
						.getProperty("get_undo_close_value");// get stock
																// closing value
																// for
																// respective
																// date
				// get stock closing value for particular date
				rs1 = ListTypeClass1.getResult_apply(con, stk_qry, stock,
						corp.getApplied_date()); // get stock closing value
				rs1.next();
				...
				// calculate MCV
				double mcv = cf.calMarketCap(Double.parseDouble(sclose),
						Long.parseLong(ml),
						Double.parseDouble(corp.getCurr_val()),
						Long.parseLong(tis), Double.parseDouble(iwf));
				corp.setNewmcv(mcv);
	...
	public static void update_index_comp(Connection con, String query,
			Corporate corporateact) {
		try {
			PreparedStatement stmt = con.prepareStatement(query);
			stmt.setString(1, corporateact.getValues());
	...
	// changed by neha 26 sep 2008
	public static void update_index_daily(Connection con, String query,
			Corporate corporateact) {
		try {
			...
			if (corporateact.getNewTmcv().equals("")) {
				...
				double newtmcv = Double.parseDouble(tmcv1);
			...
			if (corporateact.getNewdivisor().equals("")) {
				...
				String div1 = corporateact.getNewdivisor();
				double divisor = Double.parseDouble(div1);
	...
	// changed by neha 26 sep 2008
	public static void undo_update_index_daily(Connection con, String qry,
			String query, Corporate corporateact) {
		try {
			ResultSet rs = ListTypeClass1.getResult_apply(con, qry,
					corporateact.getI_index(), corporateact.getApplied_date());
			rs.next();
	...
	public static void update_resp_index_daily(Connection con, String query,
			Corporate corporateact) {
		try {
			String tmcv_val = corporateact.getNewTmcv();
			String div_val = corporateact.getNewdivisor();
	...
	// changed by neha 26 sep 2008
	public static void update_undo_index_daily(Connection con, String qry,
			String query, Corporate corp) {
		PreparedStatement stmt=null;
		try {
			ResultSet rs = ListTypeClass1.getResult_apply(con, qry,
					corp.getI_index(), corp.getApplied_date());
			rs.next();
			...
			String div1 = corp.getNewdivisor();
			if (!div1.equalsIgnoreCase("") || !div1.isEmpty()) {
				double divisor = Double.parseDouble(div1);
				stmt.setDouble(1, divisor);
			}
			
			String tmcv1 = corp.getNewTmcv();
			if (!div1.equalsIgnoreCase("") || !div1.isEmpty()) {
				double newtmcv = Double.parseDouble(tmcv1);
				stmt.setDouble(2, newtmcv);
			}
	...
	public static void update_undo_curr_index(Connection con, String query,
			Corporate corporateact) {
		try {
			PreparedStatement stmt = con.prepareStatement(query);
	...
	public static void update_index_cad_with_time(Connection con, String query,
			Corporate corporateact) {
		String cad = null;
		try {
	...
	public static void update_index_cad(Connection con, String query,
			Corporate corporateact) {
		String cad = null;
		try {
			cad = corporateact.getCad_id();
	...
	public static void insert_curr_diary(Connection con, String query,
			String nextval, Corporate corporateact) {
		try {
			int cad = ListTypeClass1.Select_nextval(con, nextval);
	...
	public static void update_curr_indmaster(Connection con, String query,
			Corporate corporateact) {
		try {
			PreparedStatement stmt = con.prepareStatement(query);
			stmt.setString(1, corporateact.getValues());
	...
	public static void insert_index_cad(Connection con, String query,
			String nextval, Corporate corporateact) {
		try {
			int cad = ListTypeClass1.Select_nextval(con, (nextval + 1));
			PreparedStatement stmt = con.prepareStatement(query);
			stmt.setInt(1, cad);
	...
	public static void check_index_value(Corporate corporateact) {
		if (corporateact.getAnnounce_date().equals(""))
	...
	public static void delete_event(Connection con, String query,
			Corporate corporateact) {
		try {
			PreparedStatement stmt = con.prepareStatement(query);
	...
	public static void insert_into_event(Connection con, String query,
			String nextval, Corporate newcorporateact, String flag) {
		try {
			String event_val = null;
			if (flag == null) {
				int event = ListTypeClass1.Select_nextval(con, nextval);
	...
	public static void recal_update_affect_value(Corporate corporateact) {
		...
		try {
			...
			for (Enumeration enum1 = hash1.keys(); enum1.hasMoreElements();) {
				...
				ActionCorp.recal_affect_value(con, connect, corporateact,
						corporateact.getI_index());
				...
				for (Enumeration enmm = hash.keys(); enmm.hasMoreElements();) {
					...
					if (stock.equals(ind_stk)) {
						...
						if (chk_stk == false) {
							...
							rs1.close();
							// get currency exchange value
							NCorp_Action.get_currency(con, connect,
									corporateact, corporateact.getI_index(),
									stock);
							...
							if (corp_value.equals("changeiwf")) {
								rs1 = ListTypeClass1.getResult_corp(con,
										index_comp_detail,
										corporateact.getI_index(), stock);
								rs1.next();
							...
							if (div[0].equals("false"))
								get_name(con, get_cam, select_is_cad,
										corporateact, ori_index);
							if (div[0].equals("true"))
								corporateact.setCad_id(div[1]);
							insert_index_comp_his(con, index_his, corporateact);
						}
	...
	public static void undoupdate_affect_index(Corporate corporateact) {
		Connect connect = ConnectInit.getConnect();
		Connection con = null;
		if (con == null) {
			con = connect.getdbConnection();
		...
		try {
			...
			if (affect_val == true) {
				...
				for (Enumeration enum1 = hash1.keys(); enum1.hasMoreElements();) {
					...
					String rad_val = corporateact.getInd_comp();
					if (rad_val != null)
						ActionCorp.undo_recal_affect_value(corporateact,
								corporateact.getI_index());
					else
						ActionCorp.undoaffected_index_value(corporateact,
								corporateact.getI_index());
					...
					for (Enumeration enmm = hash.keys(); enmm.hasMoreElements();) {
						...
						if (rs_cond == true) {
							...
							if (close != null) {
								...
								if (corp_value.equals("addstock")) {
									mcv = cf.calMarketCap(Double
											.parseDouble(close), Long
											.parseLong(ml), Double
											.parseDouble(corporateact
													.getCurr_val()), Long
											.parseLong(tis), Double
								...
								if (corp_value.equals("changeiwf")) {
									...
									mcv = mcv_new - mcv_old;
									corporateact.setNewmcv(mcv);
									update_index_comp(con, index_comp,
											corporateact);
								}
							}
						}
	...
	public static void update_affect_index(Connection con, Connect connect,
			Corporate corporateact) {
		try {
			String stk_query = ConnectInit.queries
					.getProperty("detail_stock_master");
				...
				for (Enumeration enum1 = hash1.keys(); enum1.hasMoreElements();) {
					...
					for (Enumeration enmm = hash.keys(); enmm.hasMoreElements();) {
							...
							if (rs != null && rs.next()) {
								...
								if (chk_stk == true) {
									...
									if (chk_dt == 0) {
										...
										String stock_price = ConnectInit.queries
												.getProperty("select_stock_price_detail");
										rs1 = ListTypeClass1.getResult12(con,
												stock_price, stock);
									} else {
										String stock_price = ConnectInit.queries
												.getProperty("get_undo_close_value");
						...
						if (div[0].equals("false")) {
							...
							if (chk_stk == true) {
								...
								if (chk_dt == 0) {
									...
									String stock_price = ConnectInit.queries
											.getProperty("get_undo_close_value");
									rs1 = ListTypeClass1.getResult_apply(con,
											stock_price, stock, apply);
								}
								rs_cond = rs1.next();
								if (rs_cond == true) {
									close = rs1.getString("adjusted_price");
						...
						if (chk_stk == true) {
							String corpid = corporateact.getCorpid();
							if (corpid.equals("addstock")) {
								ResultSet rs2 = ListTypeClass1.getResult12(con,
										query3, stock);
								if (rs2 != null && rs2.next())
									corporateact
											.setValues(rs2.getString("iwf"));
							...
							if (rs_cond == true) {
								...
								if (corpid != null) {
									...
									if (chk_dt == 0) {
										String index_detail = ConnectInit.queries
												.getProperty("get_latest_index_value_index_wise");// get
																									// latest
																									// index
																									// values
										...
										String index_detail = ConnectInit.queries
												.getProperty("get_undo_index_close"); // get
																						...
																						// particular
																						// date
										rs = ListTypeClass1.getResult_apply(
												con, index_detail, ind_id,
												apply);
									}
									rs_val = rs.next();
									if (rs_val == true) {
										...
										if (rs != null)
											rs.close();
										if (corpid.equals("changeiwf")) {
											rs = ListTypeClass1.getResult_corp(
													con, index_comp_val,
													corporateact.getI_index(),
													stock);
											rs.next();
							...
							get_name(con, get_cam, select_is_cad, corporateact,
									ori_index);
							insert_index_comp_his(con, index_his, corporateact);
						}// chk_stk
					}// inner for stk
					...
					if (div == 0.0) {
						double newdivisor = (newtmcv / Double
								.parseDouble(index_close));
	...
	public static void recal_update_affect_index_comp(Corporate corporateact) {
		String ori_index = corporateact.getI_index();
		String ori_tmcv = corporateact.getTmcv();
		String ori_newtmcv = corporateact.getNewTmcv();
		String ori_div = corporateact.getDivisor();
		...
		try {
			...
			String qry_detail = ConnectInit.queries
					.getProperty("select_stock_price_detail");
			String stk_qry = ConnectInit.queries
					.getProperty("get_undo_close_value"); // get stock_closing
															// value for
															// particular date
			...
			// CFormula cf=new CFormula();
			CFormula cf = ConnectInit.getCFormula();
			// as this method is used both for historic nad general CA, so there
			// should be date comparison
			String dt = UpdateCorp.accept_date(); // get the current date
			String apply = corporateact.getApply_date();
			int chk_dt = ComputeIndexForm.CompareDate(apply, dt); // check for
			...
			if (chk_hash1 == false) {
				...
				for (Enumeration enum1 = hash1.keys(); enum1.hasMoreElements();) {
					...
					ActionCorp.recal_affect_index(con, connect, corporateact,
							ind_id);
					corporateact.setTmcv(corporateact.getTmcv1());
					corporateact.setDivisor(corporateact.getDivisor1());
					...
					for (Enumeration enmm = data.keys(); enmm.hasMoreElements();) {
						...
						if (sid.equals(ind_stk))
							...
							chk_stk = false;
						if (chk_stk == true) {
							for (Enumeration enum2 = data_error.keys(); enum2
									.hasMoreElements();) {
								String id1 = (String) enum1.nextElement();
								if (sid.equals(id1)) {
									val = 1;
							...
							if (val == 0) {
								...
								if ((corp.equals("addstock"))
									...
									if (corp.equals("changeiwf")) {
										double mcv_new = cf.calMarketCap(Double
												...
												.parseLong(tis), Double
												.parseDouble(iwf));
										rs1 = ListTypeClass1.getResult_corp(
												con, index_comp_detail,
												corporateact.getI_index(), sid);
										rs1.next();
								...
								if (corp.equals("addstock")) {
									...
									insert_index_comp(con, insert_index_comp,
											corporateact);
									insert_index_comp_his(con, index_comp_his,
											corporateact);
								}
								if (corp.equals("deletestock")) {
									try {
	...
	public static void update_affect_index_comp(Corporate corporateact) {
		String ori_index = corporateact.getI_index();
		String ori_tmcv = corporateact.getTmcv();
		String ori_newtmcv = corporateact.getNewTmcv();
		String ori_div = corporateact.getDivisor();
		...
		try {
			...
			String index_detail = ConnectInit.queries
					.getProperty("get_latest_index_value_index_wise");// "select_index_detail");
			String ind_query = ConnectInit.queries
					.getProperty("select_resp_close"); // get index values for
														// particular date
			String get_name = ConnectInit.queries.getProperty("get_corp_name");
			String corp_query = ConnectInit.queries
					.getProperty("get_corporate_list_index");
			...
			if (check_rs == true) {
				...
				if (chk_hash1 == false) {
					for (Enumeration enum1 = hash1.keys(); enum1
						...
						corporateact.setI_index(ind_id);
						int val = 0;
						for (Enumeration enmm = data.keys(); enmm
								.hasMoreElements();) {
							String sid = (String) enmm.nextElement();
							obj = data.get(sid);
							String cad_id = obj.toString();
							...
							if (chk_stk == true) {
								...
								if (val == 0) {
									...
									ResultSet rs2 = null;
									if (chk_dt == 0)
										rs2 = ListTypeClass1.getResult12(con,
												index_detail, ind_id);
									else
										rs2 = ListTypeClass1.getResult_apply(
												con, ind_query, ind_id,
												corporateact.getApply_date());
									rs2.next();
									...
									if (corp.equals("deletestock")) {
										...
										corporateact.setNewmcv(mcv);
										if (newtmcv == 0.0)
											newtmcv = Double.parseDouble(tmcv)
													- mcv;
										else
											newtmcv = newtmcv - mcv;
									}
									...
									if (corp.equals("addstock")) {
										...
										String iwf_val = corporateact
												.getValues();
										insert_index_comp(con,
												insert_index_comp, corporateact);
										insert_index_comp_his(con,
												index_comp_his, corporateact);
									}
									if (corp.equals("deletestock")) {
										try {
											PreparedStatement stmt = con
						...
						if (divi1 == 0.0) {
							...
							corporateact.setNewdivisor(Double
									.toString(newdivisor));
						} else {
							double diff = cf.diffTMCV(Double.parseDouble(tmcv),
									newtmcv);
						...
						if (chk_dt == 0) {
							...
							String ind_val_daily = ConnectInit.queries
									.getProperty("update_resp_index_value_daily");
							update_resp_index_daily(con, ind_val_daily,
									corporateact);
						}
	...
	// this method is used to calclulate Index parameter after Historic date
	// till current date
	public static void update_hist_current_index(Corporate corp) {
		try {
			...
			Date dd = new Date(
					new Integer(apply.trim().substring(6, 10)).intValue(),
					new Integer(apply.trim().substring(3, 5)).intValue(),
					new Integer(apply.trim().substring(0, 2)).intValue());
			...
			Logging.debug("month len==" + month.length() + "day len=="
					+ day.length());
	...
	public static void update_historic(Corporate corp) {
		try {
			Connect connect = ConnectInit.getConnect();
			Connection Hist_con = connect.getConnectionForHistTransaction();
			Hist_con.commit();
	...
	public static void update_hist_diary(Corporate corp) {
		try {
			Connect connect = ConnectInit.getConnect();
			Connection Hist_con = connect.getConnectionForHistTransaction();
			Hist_con.commit();
	...
	public static void get_name(Connection con, String query, String query1,
			Corporate corporateact, String index) {
		try {
			String id = null;
			String date = accept_date();
			ResultSet rs = ListTypeClass1.resultCorporate(con, query);
	...
	public static void insert_corp(Connection con, String query,
			String corp_name, String nextval, Corporate corporate) {
		try {
			String corpid = corporate.getCorpid();
			ResultSet rs = ListTypeClass1.resultCorporate(con, corp_name);
			ListTypeClass1
					.check_corp_name(rs, corporate.getCorpid(), corporate);
	...
	public static void update_corp(Connection con, String query,
			String corp_name, Corporate corporate) {
		try {
			String corpid = corporate.getCorpid();
			ResultSet rs = ListTypeClass1.resultCorporate(con, corp_name);
			ListTypeClass1
	...
	public static void rebasingAction(Corporate corp) {
		String newBaseDate = corp.getBase_date();
		String newBaseValue = corp.getB_natureindex();
	}

	public static double get_usd_exchange(Connection con, Connect connect,
			Corporate corp) {
		double exc_rate = 0.0;
		try {
			...
			String qry1 = ConnectInit.queries
					.getProperty("detail_stock_master");
			// commented by pranoti 11Aug05 as per IISL requirement(no cursor
			// exception)
			/*
			 * stmt = con.prepareStatement(qry1); stmt.setString(1,stock); rs =
			 * stmt.executeQuery();
			 */
			...
			// commented by pranoti 11Aug05 as per IISL requirement(assuming
			// that from system_setup/scrips USD currency_id will come)
			/*
			 * String query=connect.queries.getProperty("select_system_config");
			 * rs=ListTypeClass1.resultCorporate(con,query); rs.next();
			 * rs.close();
			 */
			String other_curr = "1";
			if (stk_curr.equals(other_curr)) // if both r equal
				...
				String qry = ConnectInit.queries
						.getProperty("currency_combination");
				// commented by pranoti 11Aug05 as per IISL requirement(no
				// cursor exception)
				/*
				 * stmt = con.prepareStatement(qry); stmt.setString(1,ind_curr);
				 * stmt.setString(2,stk_curr); rs = stmt.executeQuery();
				 */
				rs = ListTypeClass1.getResult_corp(con, qry, other_curr,
						stk_curr);
				if (rs.next()) {
					...
					rs.close();
					rs = ListTypeClass1.getResult_corp(con, qry, stk_curr,
							other_curr);
					if (rs.next()) {
						ft_curr = rs.getString(1);
						rs.close();
				...
				if (flg != 0) {
					...
					rs = ListTypeClass1.getAffected(con, query, ft_curr);
					rs.next();
