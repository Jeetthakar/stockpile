...
//	CFormula formula = new CFormula();
	...
	public double getAdjustedMarket(int indextype) {
		...
		//adjustedMarket = formula.calMarketCap1(ltp,1,1,tis,adjustedIWF,indextype);  commented by neha 15-01-2009
		adjustedMarket =(formula.calMarketCapt(ltp,1,1,tis,indextype))*iwf;
	...
	public double getMktCapital(int indextype) {
		...
		// mktCapital =formula.calMarketCap1(ltp,1,1,tis,iwf,indextype) //commented  by neha 15-01-2009 for editable iwf
		mktCapital =(formula.calMarketCapt(ltp,1,1,tis,indextype))*iwf;
		return mktCapital;
	}
	/**
	 * @return Returns the mktCapital.
	...
	//	AdjustDecimal ad=new AdjustDecimal();
		...
		//mktCapital = formula.calMarketCap1(ltp,market_lot,d,tis,iwf,indextype);//commented  by neha 15-01-2009 for editable iwf
        mktCapital = (formula.calMarketCapt(ltp,market_lot,d,tis,indextype))*iwf;
		return mktCapital;
	}
	
	/** written  by neha for editable iwf 15-01-2009 **/
	public double getMktCapitalNew(int indextype,String parentCurrencyId,String stkCurrencyid) {
		//compute currency rate and pass it as 3rd variable
		CFormula formula = ConnectInit.getCFormula();
	//	AdjustDecimal ad=new AdjustDecimal();
		AdjustDecimal ad = ConnectInit.getAdjustDecimal();
        ...
        Logging.debug("market_lot is "+market_lot);
		//mktCapital = formula.calMarketCap1(ltp,market_lot,d,tis,iwf,indextype);//commented  by neha 15-01-2009 for editable iwf
        mktCapital = formula.calMarketCapt(ltp,market_lot,d,tis,indextype);
		return mktCapital;
	}
	...
	public double getWightage() {
		 wightage = (adjustedMarket/totalMktCapital)*100;
