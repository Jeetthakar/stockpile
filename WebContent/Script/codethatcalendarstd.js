		...
		if (ua.ie4up) {
			...
			if (d.forms.item) {
				for (var i = 0; d.forms.item(i) != null; ++i) {
					if (d.forms.item(i).namedItem
							&& d.forms.item(i).namedItem(f)) {
...
CTc.create = function(d, ctl) {
	...
	d.write("<table " + (Def(this.ifr) ? "align=\"center\"" : "")
			+ " cellspacing=0 cellpadding=0 " + acts + " width=" + def.width
			+ " border=" + (def.border_width || 0) + " bordercolor='"
	...
	if (i) {
		...
		if (def.headerstyle.type == "buttons") {
			...
			if (def.headerstyle.type == "comboboxes") {
				this.createCombos(d);
			}
		}
	}
...
CTc.createMonth = function(d) {
	...
	for (w = 0; w < 6; ++w) {
		...
		for (var day = 0; day < 7; ++day) {
			...
			d.write("<td align=center width=14% " + this.getCss(css) + "><A "
					+ this.getCss(css) + this.getRef() + ">");
...
CTc.createButtons = function(d, i) {
	...
	if (dh.imgprevy && !i) {
		d.write("<td width=14% align=left><a href='javascript:CodeThatSetMY("
				+ this.date.getMonth() + "," + this.date.getFullYear()
				+ ",3)'><IMG border=0 src='" + dh.imgprevy + "'</IMG></A>");
	}
	if (dh.imgprevm && !i) {
		d.write("<td width=14% align=left><a href='javascript:CodeThatSetMY("
				+ this.date.getMonth() + "," + this.date.getFullYear()
				+ ",1)'><IMG border=0 src='" + dh.imgprevm + "'</IMG></A>");
	}
	d.write("<td colspan=" + col + " align=center " + this.getCss("css", dh)
...
CTc.createCombos = function(d, l) {
	...
	for (; i < 12; ++i) {
		d.write("<option "
				...
				+ " value='" + i + "'>" + this.def.monthnames[i] + "</option>");
	}
	d.write("</select>");
	d
...
function changeDisplay(elementId, setTo) {
	...
	if (!theElement) {
		 ...
		 * document.getElementById or document.all but cannot actually use
		 * either
		 */
		return;
	}
